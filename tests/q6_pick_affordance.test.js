...
const PAGE = `
  <table id="t">
    <thead><tr><th id="h0">幣別</th><th id="h1">買入</th></tr></thead>
    <tbody>
...
test('F2-1 離開選取模式時指標與表頭提示都要還原', async () => {
  ...
  pm.exitPickMode()
  const b = await boot()
  move(b.win, b.doc.getElementById('h1'))
  b.pm.exitPickMode()
  assert.equal(b.doc.body.style.cursor, '', `實得 ${JSON.stringify(b.doc.body.style.cursor)}`)
  assert.equal(b.doc.getElementById('h1').hasAttribute('title'), false, '提示是我們加的，要收乾淨')
  void doc; void win
})
...
test('F2-2 完成與取消也是真的按鈕，Tab 走得到', async () => {
  const { doc, pm } = await boot()
  for (const sel of ['[data-af-done]', '[data-af-cancel]']) {
