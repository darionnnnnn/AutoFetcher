...
test('右鍵「設定此站台登入」會開設定視窗並帶上 origin 與分頁', async () => {
  ...
  // AF-10：站台設定與任務設定用同一個載體（side panel），參數走 storage.session
  const opened = c.__calls.find(x => x.api === 'sidePanel.open')
  assert.ok(opened, '要開站台設定面板')
  assert.equal(opened.args[0].tabId, tab.id, '面板要綁在那個分頁上')
  const setOpts = c.__calls.find(x => x.api === 'sidePanel.setOptions')
  assert.match(String(setOpts.args[0].path), /site\.html/)
  const stored = await chrome.storage.session.get(`panel:${tab.id}`)
  const entry = stored[`panel:${tab.id}`]
  assert.equal(entry.kind, 'site')
  assert.equal(entry.origin, ORIGIN, `要帶上 origin,實得 ${JSON.stringify(entry)}`)
  assert.equal(entry.tabId, tab.id, `要帶上分頁 id,實得 ${JSON.stringify(entry)}`)
})
