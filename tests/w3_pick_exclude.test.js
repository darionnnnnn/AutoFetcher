...
test('表尾提示只在真的加進清單時說：Ctrl 點取消已選的整欄不得說「已自動排除表尾」', async () => {
  const { pm, doc } = await setup()
  enter(pm, doc, { purpose: 'repick', preselect: [{ block: { axis: 'col', index: 1, headerText: '點金靈' } }] })
  hover(doc, 'h1')
  doc.getElementById('h1').dispatchEvent(new globalThis.MouseEvent('click', { bubbles: true, ctrlKey: true }))
...
test('右鍵排除之後按 Ctrl+Z 不得跳回排除之前的快照（排除也是改動，快照要失效）', async () => {
  ...
  click(trimTail(doc))
  menu(doc, 'c1-2', 'exclude')
  key(doc, 'z', { ctrlKey: true })
  confirm(doc)
  const picks = lastMsg(c).picks
  assert.deepEqual(picks[0].block.exclude, [{ index: 3, header: '合計' }, { index: 1, header: '10.0.0.2' }],
    '排除的改動被 Ctrl+Z 一起丟掉了')
  assert.equal(picks.length, 3, 'Ctrl+Z 退回「移除最後一項」：整欄值＋去尾後剩下 3 格，再移除 1 格')
})
...
test('連續選兩次：排除、去頭去尾狀態與計數不得殘留到下一次', async () => {
  ...
  assert.deepEqual(picks[0].block.exclude, [{ index: 3, header: '合計' }], '上一次右鍵排除的第 2 列不得殘留')
})
