...
// AF-9：單值預設改成「數字 + 折線」——只給數字卡的話，使用者存完看到一個
// 沒有脈絡的數字，不會知道要自己去加折線才看得到趨勢
test('number 模式預設勾數字與折線', async () => {
...
test('存檔後在所選儀表板加入對應卡片', async () => {
  ...
  assert.equal(cards.length, 2, `number 模式預設數字與折線兩張卡，實得 ${cards.length}`)
