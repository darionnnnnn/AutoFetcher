...
test('pointercancel(觸控被瀏覽器接管、視窗失焦)要清乾淨,不可留幽靈與高亮', () => {
  const src = setup()
  const tgt = makeTarget('t', [200, 200, 300, 300])
  let dropped = 0
  DND.createDragSource(src, () => ({ taskId: 't1' }))
  ...
  down(src, 0, 0)
  move(250, 250)
  document.dispatchEvent(pointer('pointercancel', 250, 250))
  assert.equal(DND.isDragging(), false)
  assert.equal(document.querySelector('[data-dnd-ghost]'), null, '幽靈會卡在畫面上')
  assert.equal(tgt.hasAttribute('data-drop-active'), false, '高亮會留在卡片上')
  up(250, 250)
  assert.equal(dropped, 0, '取消之後放開不算數')
})

test('第二根手指放開不可吃掉第一根手指的拖曳', () => {
  const src = setup()
  const tgt = makeTarget('t', [200, 200, 300, 300])
  let dropped = 0
  DND.createDragSource(src, () => ({ taskId: 't1' }))
  ...
  down(src, 0, 0)
  move(250, 250)
  document.dispatchEvent(pointer('pointerup', 500, 500, { pointerId: 2 }))
  assert.equal(dropped, 0, '別的指標放開不該觸發投放')
  assert.equal(DND.isDragging(), true, '原本的拖曳要還在')
  up(250, 250)
  assert.equal(dropped, 1)
})

test('拖曳結束後狀態當場歸零(不是靠下一次 pointerdown 補救)', () => {
  const src = setup()
  const tgt = makeTarget('t', [200, 200, 300, 300])
  DND.createDragSource(src, () => ({ taskId: 't1' }))
  DND.registerDropTarget(tgt, { accepts: () => true, onDrop: () => {} })
  down(src, 0, 0)
  move(250, 250)
  up(250, 250)
  assert.equal(DND.isDragging(), false)
...
test('第二根手指移動不可改變第一根手指的投放目標', () => {
  const src = setup()
  const a = makeTarget('a', [200, 200, 300, 300])
  const b = makeTarget('b', [600, 600, 700, 700])
  const hit = []
  DND.createDragSource(src, () => ({ taskId: 't1' }))
  DND.registerDropTarget(a, { accepts: () => true, onDrop: () => hit.push('a') })
  DND.registerDropTarget(b, { accepts: () => true, onDrop: () => hit.push('b') })
  down(src, 0, 0)
  move(250, 250)
  assert.equal(a.hasAttribute('data-drop-active'), true)
  document.dispatchEvent(pointer('pointermove', 650, 650, { pointerId: 2 }))
  assert.equal(a.hasAttribute('data-drop-active'), true, '別根手指移動不可把高亮搶走')
  assert.equal(b.hasAttribute('data-drop-active'), false)
  up(250, 250)
