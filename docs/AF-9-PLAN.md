...
- 使用說明：**不做獨立教學頁**。原本要在 popup 放一行「怎麼用？」連結，**實作時推翻**：
  使用者已明確表示不看說明文字，連結只會變成另一個沒人點的入口；
  教學責任全部由「主要入口按鈕 + 狀態驅動的一句指令」承擔（已寫進 BACKLOG，附觸發條件）。
...
- 排程區（在「多久抓一次」卡內）：
  ...
  - 星期改成 chip **外觀**（DOM 仍是 `<input type="checkbox">`）：
    改成 `button[aria-pressed]` 會失去原生勾選語意、且既有多處測試依賴 checkbox，
    收益只有樣式、風險是無障礙倒退，因此只做 CSS chip 化。
...
- 任務頁排程欄改用 `describeSchedule`；**popup 原本沒有排程文字**（只有下次執行時刻），
  改為在任務列的 `title` 顯示同一句白話（久沒用回來看得出這是每天還是每十分鐘一次）。
...
- 多值清單每列：序號、名稱輸入、位置說明（「列標題 · 欄標題」或位置定位字樣）、測試結果格（未測 `—`；成功綠值；失敗紅 `—` 且 `title` 為錯誤句）。立即測試回來以每個值的 `key` 對應（`res.fields[key]`，dryRun 的實際回傳形狀；
  規劃時誤寫成紀錄用的 `values[].name`，實作以程式碼事實為準）。
...
| 發現 | 判定 | 處置 |
|---|---|---|
| `updateSchedulePreview` 讀錯 `validateForm` 回傳形狀，錯誤分支是死碼 | 成立（自行核對 `validateForm` 回 `{ok, errors}`） | 已修並補兩條測試（星期全不選、間隔沒填分鐘），突變會紅 |
| 摘要卡不跟著列/欄定位、數值類型、聚合方式更新 | 成立 | 已補綁定與測試 A1-5 |
| `applyDefaultCardTypes` 會把使用者勾的卡片型別改回預設（既有缺陷，本輪預設值改動放大影響） | 成立 | 加「使用者動過就不覆蓋」守衛，補測試 A7-1 |
| 非表格上點「單格」給的理由是「這個用途一次只選一個」，與同一畫面的「非表格：抓整個元素」矛盾 | 成立 | 理由改為對到真正判定；停用分支改為**一律解除鎖定**，補測試 C9-2b / C9-2c |
| 面板閃避會來回彈跳 | 成立 | 加閂鎖（換角後要離開才再換），補測試 C13-1b |
| 時刻 chip 掛的 `.chip` 類別沒有定義 | 成立 | 補 `picker.html` 樣式，星期一併 chip 化 |
| `exitPickMode` 漏清 `pendingPreselect` / `kindCache` | 成立 | 已補 |
| `applyFieldResults` 對 `null`／空值會顯示空白或 `"null"` | 成立 | 改為一律 `—` |
| `renderFieldList` 連續呼叫 `updateFieldListState` 兩次 | 成立 | 移除一次 |
| 測試 A4-1 兩條分支都會命中、A5 只比對被刪掉的字面值、C15 面板位置恆為 `16px` | 成立（三條假通過） | 三條全部改寫，突變後皆會紅 |
| 文件宣稱 popup 使用 `describeSchedule` 但實際沒有 | 成立 | 讓 popup 任務列的 `title` 真的用它，三處文件敘述一併對齊 |
| SPEC 寫「面板第一行是指令句」，實作在最後一行 | 成立（文件與碼不一致） | 指令句緊鄰動作列比較合理，改 SPEC 敘述 |
| 「工具列**啟用**分支未解除鎖定」 | **不成立** | `updateToolbar` 在非表格時三段全停用，而 `lockedEl` 只可能在非表格時設定，啟用分支不可達；該行是死程式碼，已於作業 C 移除，改驗停用分支 |
| 星期 chip（`aria-pressed`）、popup「怎麼用？」連結未做 | 成立（定案未回填） | 兩條定案都已在上面改寫並說明理由，BACKLOG 有對應條目 |
| 欄位列的上／下移按鈕 `disabled` 也是靜默無回饋 | 成立但不在本輪範圍 | 寫進 BACKLOG |
| `RUN_TASK` 的 `msg.__testOpts`（AF-1 留下的正式碼測試旁路） | 成立但不在本輪範圍 | 寫進 BACKLOG |
...
- 尚未實機操作驗證(使用者實測)，建議先看:設定視窗版面與摘要卡、選取模式點整欄、
  間隔時段的觸發預覽、popup 的「在這個頁面選取」。

## 規劃項目逐條比對（使用者要求，Claude 親做）

對 PLAN 的每一條定案與改動去 `src/`、`tests/` 找對應實作，結果如下。

**已實作且有測試（無落差）**：`shared/describe.js` 四種句型契約、`schedule-math.js` 搬家與
`scheduler.js` 只 re-export、時刻 chip（去重／排序／`#times` 為事實來源）、時段勾選才展開、
取消勾選清殘值、編輯既有 window 任務回填、觸發預覽（含跨午夜只列今天、不合法顯示錯誤句）、
任務頁沿用 `describeSchedule`、單格升級取代、取代可復原（含 `Ctrl+Z` 順序與快照清空時機）、
停用提示與 `pendingMode`、動作列建一次、面板閃避（含焦點與遲滯）、狀態指令句四種、
`exitPickMode` 全部重設、頁面不寫死寬度、摘要卡三行、多值列位置與逐值結果、一鍵命名、
儀表板預設與卡片數字＋折線、`pickerDefaults` 存 `dashboardId`／`cardTypes`、儲存回饋、
popup 主要入口與 `chrome://` 提示、版本 0.8.0、SPEC／BACKLOG 更新。

**本次比對抓到的兩條漏實作，已補**：

| 定案 | 落差 | 處置 |
|---|---|---|
| 「有已選時『完成』用主色」 | 動作列建一次的改寫把它做成**一律主色**，未選時（`aria-disabled`）也長得像主要動作 | 改為由 `updatePanelActions` 依已選數切換，補測試 C12b，突變會紅 |
| 卡 3 的「卡片型別 chip」 | 只做了星期的 chip 化，卡片型別仍是裸 checkbox | 併入同一組樣式 |

**刻意偏離規劃，理由如下（非漏做）**：

| 定案原文 | 實作 | 理由 |
|---|---|---|
| 卡 1「抓什麼」內含即時預覽與立即測試 | 獨立成第二張卡「先試抓看看」 | 「試抓」是一個動作，不是「抓什麼」的屬性；混在同一張卡裡，卡片標題就沒辦法回答單一問題 |
| 補測試進 `tests/p3_pick_toolbar.test.js` | 寫在新檔 `tests/r3_pick_flow.test.js` | 內容等效（鎖定→點整欄→標示、單格升級、停用提示都有）；p3 是 AF-7 的驗收檔，本輪的行為改變獨立成檔比較好追 |
| `ui.css` 新增摘要卡與 chip 樣式 | 寫在 `picker.html` 的 `<style>` | 這些類別只有 picker 用得到，放進 `ui.css` 會變成 site 那邊的死類別（CLAUDE.md 明文禁止） |
| 面板顯示「已換成這一格（復原）」 | 「已換成新的選取（可按復原或 Ctrl／⌘＋Z 還原）」 | 取代也可能來自「整批被換掉」，寫「這一格」在多值情境是錯的 |
| 視窗 560×820（暫定） | 600×820 | 三張卡的欄寬在 560 會讓時段那一列換行 |
| popup「怎麼用？」連結 | 不做 | 見〈定案〉節，已於實作時推翻並寫進 BACKLOG |
