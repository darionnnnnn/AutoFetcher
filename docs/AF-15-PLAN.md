...
- **子單位只在「複合格」裡才有**（實作前收窄，理由見執行紀錄「過程中發現」）。複合格＝外層格符合任一條：
  1. 格內含表格（`cellWrapsTable`）；
  2. 格內（不含外層格自己）「自己擁有非空白文字節點」的元素兩個以上。`<td><span>31.5</span></td>` 不是複合格；`<td><div><b>MAX</b><span>462</span></div></td>` 是。
- **子單位是誰**：
  - 規則 1（格內含表格）→ `cellOf(滑鼠元素)`，且它不是外層格、在外層格之內；滑鼠停在外層格自己（padding）或內層表的縫隙上 → 沒有子單位（整格）。
  - 規則 2（多個帶文字的元素）→ 從滑鼠元素往上（含自己、不含外層格）第一個「自己擁有非空白文字節點」的元素；停在沒有自己文字的包裝元素上 → 沒有子單位。
  - 兩條都成立用規則 1；子單位 = 外層格本身時等同沒有 `inner`。
  - 判定只寫一個函式；適用於 HTML 表、ARIA 表與 CSS 假表格。
- **Enter 快速路徑**記住 hover 的元素（`currentHoverEl`），不用外層格算候選，否則會丟掉子單位。
- **`↑`／`↓` 落在非表格時清掉明確選定的鎖**（離開表格是明確意圖；否則 q9「↑ 到非表格再 ↓ 回表格」會卡住）。
- 「某一列、網格欄 c、沿路徑取元素」一律呼叫 `table.js` 的 `resolveInnerAt`，與擷取端同一份。
...
委派模型：agy `gemini-3.8-flash-high`（開工時 Gemini 組週額度 75%、五小時 54%）。
收尾輪由 Opus 5 接手規劃後的實作與驗收（Fable 5.1 做規劃與四角度複檢）。
...
| C | agy 兩次假成功 → **使用者決定改由 Claude 實作** | 通過 | v4 14 綠、相關 163 綠、全套 2009 綠、突變 14 發皆紅、BOM/NUL/CR 與 dev 基準一致 | agy 兩次都 exit 0、stdout 零輸出、工作樹零改動：第一次秒退，第二次同模型重派 93 秒；額度正常（Gemini 五小時 35%→33%），同模型最小提示 6 秒回應正常——agy 本身沒壞，是這份任務讓它無聲結束，原因未明。依委派規則停下來問使用者，選「改由 Claude 實作」。實作時把三份會重複的小判定收成一份：`putInner`、`blockRowsOf` 進 `table.js`（選取模式原本的本地 `putInner` 移除改匯入、擷取端改用 `blockRowsOf`），`withInnerLabel` 進 `describe.js`（七個入口的組字只有一份）。v4 第 5 條存檔鏈結案原本用了為命名入口編的欄標題 `PORT:443`，監控頁外層表沒有欄標題，擷取端照規則 `not_found`——是測試資料錯，改用實況欄位。**派工前的規格修正**：規格原寫「Picker 會把 inner 自然流進規格、不要改 buildSpec」，探針實測單值與多值存檔後規格都沒有 `inner`（`applyPosToCell` 重建儲存格、單值整欄只挑三個欄位重組），改寫規格並補一條單值整欄存檔的測試；多值存檔案例原本漏給 `nameHint` 存不出任務，是測試寫錯，已修 |
| D | Claude | 通過 | 全套 2009 綠、煙霧 Chrome／Edge 全部通過（含新增三案）、錨點乾跑全中、BOM/NUL/CR 與 dev 基準一致 | SPEC §2（明確選定的表要鎖、複合格子單位、三句提示）§3（`innerProbe`）§7（`inner` 形狀與擷取規則、網格索引、舊任務影響）；CLAUDE.md 三條慣例與基線 2009；BACKLOG 七項；版本 0.14.0 兩處同步。煙霧新增：5h 擷取端整欄子路徑 min=460／單格 462、5i 真實滑鼠 ↑ 鎖外層後選子單位、5j colspan 表點 c1 的網格索引 |
...
- **B1 寫測試時發現定案 2 套在一般表格上會造成大範圍退化**：「沒有巢狀表就取滑鼠下的元素本身」若適用每一張表，
  匯率表的 `<td><span>31.5</span></td>` 滑鼠停在 span 上就會多帶 `inner`，網站只要改個包裝標籤就 `not_found`——
  使用者從來沒要求過子單位，卻被換上一條更脆弱的定位。收窄成「複合格」才有子單位（見 B1 定案），
  一般表格的包裝標籤維持整格，並以測試守門。這是縮小範圍、不擴大行為；既有測試掃描只命中 `<td><img> 美金 (USD)</td>`（`img` 不帶文字，不算複合格），零影響。
  已知代價：臺銀那種 `<td><div>美金 (USD)</div><div>美金</div></td>` 的列標題格會被判成複合格，只有使用者點那一格本身時才會長出 `inner`（通常點的是數字格），先接受。
- **Enter 快速路徑用外層格算候選**（`candidateAt(currentCellEl)`），會丟掉子單位；B1 改記 hover 元素。
- **`↑`／`↓` 落在非表格時要清鎖**：規劃只寫了設定時機，沒寫落在非表格時怎麼辦。
- B1 規格的「DOM 索引取格」grep 驗收補上 `getCellText` 那一處（現況命中 5 處，規劃只列到 4 處的樣式）。
- **定案 8 再收窄：訊息層也不帶顯示標籤**（推翻 v2 的「`PICKED` 頂層帶 `innerLabel`、選取端用 DOM 算小表欄標題」）。
  寫 C 批時核對到 Picker 把 `pick.cell`／`pick.block` 整個物件抄進清單列與 `buildSpec`，`inner` 會自然流進規格；
  訊息頂層再帶一份 DOM 算出的標籤，就是 AF-14 剛刪掉的 `rawHeader` 側通道翻版（多一條要逐欄挑、要測「不得進規格」的欄位）。
  使用者的小表沒有表頭，DOM 版標籤用不上。改為所有顯示處一律用純函式 `innerLabel(inner)`，訊息、ctx、規格都不帶顯示欄位。
- **`innerLabel` 從 C 批提前到 B2**：B2 的 chip 名稱原規劃先用「格內第 n 格」暫代、C 批再換，等於刻意寫一份會被刪掉的第二份判定。提前後 C 批只接消費端。
- **命名入口是七個不是六個**：一鍵命名「用欄標題」（`renameFields` 的 `col` 分支）也組名稱，AF-14 正是漏了這種入口；C 批的「掃到的入口數」斷言改為 7。
- **B2 寫規格時再核對到 `Ctrl+A` 與 `Shift`＋方向鍵也用 DOM 索引**（原測試沒涵蓋 colspan），併進 B2 修正並補測試。
...
- 體檢時請特別看：
  1. **複合格判準**（實作前收窄）：臺銀式 `<td><div>美金 (USD)</div><div>美金</div></td>` 會被判成複合格，只在使用者點那一格本身時長出 `inner`，目前接受。
  2. **`colspan` 修正對舊任務的影響**：有 `colspan` 資料列的表，舊任務存的是 DOM 索引，修正後擷取結果不變（擷取端一直照網格取），但重選會存成網格索引。
