...
- Picker：`[data-skip-row]` 內加勾選框 `#skip-blank`（標籤「自動略過頭尾空白」），**新建預設勾**；`skipFromForm` 回 `{ head, tail, blank }`；編輯既有任務時從 `skipOf(skipSource).blank` 回填（缺省＝不勾，顯示真實狀態）；換目標時 `render` 重跑不重設它（與 `#skip-head` 同待遇）。**不加進監聽 id 清單**（實作時修訂：原寫「加進去」，但摘要卡與儲存摘要依定案都不提空白，改勾選不會改變任何畫面文字，綁了只是空轉；x2 斷言摘要不出現「略過／空白」）。儲存摘要 `skipNote` 不變。
...
- **`items` 的形狀（暫定）**：整欄／整列（非 `pos` 取格）結果一律帶 `items: [{ index, header, raw?, number?, use }]`，順序即清單順序：
  ...
  - **只要清單已經產生，失敗回傳也帶 `items`**（以作業 A「實作前核對補充」為準共五種：略過後沒有剩下、全部空白、全被排除、剩下的全部找不到子路徑、全部非數字）——這正是使用者最需要看表的時候；處置照當時判定得到的填（例如全部 `skipHead`／`skipTail`／`trimmed`）。清單為空的既有失敗（本來就沒有格子）不帶。
...
## 體檢輪修正（Fable 5.1，換模型後）

規劃比對逐條（含「明確不做」七項、「實作前核對補充」）由 Opus 5 在切換前親做一次、Fable 再核一次，程式部分全部對得上；抓到 1 個 bug、1 處重複、2 句規格漏寫：

1. **換目標時明細表殘留**（bug，`src/ui/picker/picker.js`）：`render` 重畫會清預覽、狀態色、診斷，卻沒清 `#test-detail`——右鍵改選另一張表後，舊表的整欄明細（甚至展開著）留在新目標的預覽底下，與 AF-7 `pickedTableEl`、AF-9 `undoSnapshot` 同型的跨目標殘留。修法：抽 `resetTestDetail()`（藏起、收合、清空）一份，`render`、`handleTestNow` 開始、`renderTestDetail` 畫表前三處共用，順帶收掉終檢指出的「畫表前重複清空」。迴歸測試 x3 兩條：換目標後藏起且清空；立即測試丟例外時開始就收掉（沒有這條，「開始時歸零」那一行的突變會存活）。
2. **`classifyUse` 重複分支**（`src/shared/extract.js`）：「略過數超過剩餘長度」那個 if 的結果與後面兩個判斷完全相同（k < head → skipHead；否則 strippedLength − tail ≤ head ≤ k 必成立 → skipTail），刪掉並留一行註解說明；x1 既有的略過太多案例全綠，突變把 skipTail 判定改壞 3 紅。
3. **SPEC 漏兩句**：0.15 以前匯入帶 `skip.blank` 的任務會忽略（規劃有要求）；`items` 與 `blank` 不進診斷包（定案寫「不進紀錄、不進 diag」，SPEC 只寫了前者）。
4. 不改、記 BACKLOG：`render` 重畫同樣沒清 `#errors` 與 `#test-note`（既有行為，非本輪引入，症狀輕）。

突變 3 組全紅。全量 **2181 綠**（基線 2117，+64）。

## 體檢交接

- **模型**：規劃 Claude Fable 5.1；實作程式碼 agy（gemini-3.8-flash-high）三段；驗收測試、既有測試修改、終檢修正、文件 Claude Opus 5。兩份終檢是 Opus low subagent。**體檢請用 Opus 5 以外的模型**（Opus 5 寫了測試與驗收，自己驗自己會沿用同一套假設）。
- **最沒把握的地方**（體檢優先看，但不可只看這幾處）：
  1. `extract.js` 的 `classifyUse`：處置分類與實際移除清單是兩份獨立計算（分類用位置算、移除用 slice／filter），四條口徑不變式靠測試與終檢 14 組輸入撐著，沒有結構上的保證。
  2. Opus 5 自己改的既有測試（`v1_inner_extract` 守門、`w4_picker_skip` 三條）：判斷為「預期變化」是 Opus 5 的判斷，沒有第二人看過。
  3. 換目標時勾選框保留使用者的選擇：保留是因為 `render` 新建分支本來就不碰 skip 欄位，不是明確寫的邏輯；以後有人在那裡加重設就會靜默改變。
  4. 排程抓取也會算整欄明細再丟掉（定案「一律回傳」的代價），是否要收斂留給體檢判斷。

- **全量測試**：`npm test` 2179 綠、0 紅（基線 2117，+62；終檢前 2176，終檢補 3 條）。版本號 0.16.0 只動兩個 JSON，未新增測試。
- **新增測試**：`x1_skip_blank` 29、`x2_blank_picker` 18、`x3_test_detail` 14、`m2_chain` +1。
...
- **突變**：終檢補測 4/4 紅（整列兩則訊息、換目標重設勾選框、多值失敗讀 fields 不防空）；A-1 11/11 紅；A-2 9 組 8 紅（1 組等價）；B-1 12 組 11 紅（1 組等價）＋紀錄白名單 2 組紅。等價突變的理由見執行紀錄。
...
- **文件終檢**（Opus low 獨立一份）：定案逐條對實作、SPEC 逐句對程式碼皆相符；全量實跑 2176 綠；中 2、低 4，處理如下。
  1. 中｜BACKLOG 漏了規劃要求的「ARIA／CSS 假表格空白判準」→ 已補並附觸發條件。
  2. 中｜勾選框未加進監聽清單，與 PLAN 定案不符且未記錄 → 確認不綁才對（摘要不提空白），PLAN 定案改寫並記理由。
  3. 低｜整列版兩則訊息沒有測試 → x1 補兩條（全空白、剝完不夠略過），逐字比對訊息。
  4. 低｜多值整包失敗時畫表沒有測試 → x3 補一條（先顯示再整包失敗，確認收掉、不丟例外）。
  5. 低｜SPEC 舊句「兩個都 0 不放鍵」與消費端預覽字串未同步 → 已補。
  6. 低｜PLAN 作業 B 寫三種失敗、補充寫五種 → 已標注以補充為準。
- **程式碼終檢**（Opus low 獨立一份）：判定可放行，無高、中。以 14 組輸入實跑 `extractValue`（排除指到略過範圍、指到被剝的空白、重複排除、inner 解析不到落在頭尾、整列 inner 加 colspan、head+tail 剛好等於剝完長度、全空白、全非數字等），四條口徑不變式在所有成功回傳成立；搬運點普查（重選、`stripPos`、描述句、兩份紀錄白名單、多值擷取白名單、設定匯出入、Report）皆正確。低 3 條：
  1. 勾選框沒綁摘要重算 → 與文件終檢第 2 條同一件，確認不綁。
  2. 排程抓取也算整欄明細再由 fetcher 丟掉；`classifyUse` 裡「略過太多」分支與後兩個 if 大致重疊；整欄模式一律計算列標題 → **不改**：「一律回傳、不走 dryRun 旗標」是本輪定案（避開訊息欄位改行為），成本是每次抓取多一次逐列取標題與陣列組裝，表格規模下可忽略。留給換模型體檢判斷是否收斂。
  3. 換目標測試只斷言勾選框與略過欄位「同進退」，不鎖定保留或重設 → 規劃寫明「不重設」，已改成斷言明確值。
- **留給體檢者注意**：畫表函式開頭與 `handleTestNow` 開始各清空一次明細容器（重複但無害）；`classifyUse` 參數多達八個，若要收斂屬體檢取捨，不影響行為。

## 終檢輪

終檢無新發現。Opus low 掃描因額度上限中止，改由 Fable 親做：體檢修正 commit `4151f52` 的 `classifyUse` 刪分支以五組「略過數 ≥ 剝完長度」輸入實跑，skipHead／skipTail 標記與定案一致；`resetTestDetail` 三個入口（`render`、測試開始、畫表前）在編輯既有任務與丟例外路徑都只會多藏不會漏藏；合併後全量 2181 綠。
