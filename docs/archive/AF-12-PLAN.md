...
- 實作：Opus 5（分支 `r12`，commit `4f8c7a6`、`6ea8a8b`）。體檢：Fable 5.1（事實盤點派 scan-low／Opus low）。
...
1. **`docs/BACKLOG.md` 殘留待辦**——症狀：第 80 行還掛著「`RUN_TASK` 仍吃 `msg.__testOpts`（AF-1 留下）」，
   本輪就是在做這件事，卻沒有人結案；規劃的「現況核對」也漏了這條（那一節只查了 SPEC，沒查 BACKLOG）。
   修法：刪除該列。教訓：核對現況文件時 BACKLOG 要一起查——它本來就是「已知但沒做」的清單，本輪做的事最可能在裡面。
2. **`tests/d2_fetcher.test.js`「擷取一直不回時逾時失敗」的計時器斷言太鬆**——症狀：`assert.ok(timers <= 1)`
   沒有進場前的基準；逾時路徑的計時器本來就已到期，這條斷言任何實作下都會過（CLAUDE.md「把門檻設在本來就不會命中的位置」那一型）。
   修法：改成與進場前等值比對。該測試真正的守門（逾時要 reject、要寫錯誤紀錄）不變，突變仍紅。
3. **判定無事的兩處**：`main.js:466` 整包展開 `msg` 是把 `PICKED` 轉發給面板，不進任何執行選項；
   src/ 內同型的「`Promise.race` 賽跑計時器不清」只有 fetcher 那一處（`sleep` 樣板與 debounce 都是會到期或已清除的），沒有第二個洩漏。
4. 其餘角度無新發現：規劃逐條都有對應 diff，計畫外的計時器修正理由充分且量測在案；`...runOpts` 放在 `slot` 之後、
   `reason` 之前，與 `handleAlarm` 的 `{ slot, ...testOpts }` 同序；正式接線仍只傳兩個參數（src/ 零處傳第三參數）。

測試：**1852 綠**（基線 1848 + 4）。

## 終檢輪

終檢無新發現（併 dev 後 1852 綠；文件掃描只補 CLAUDE.md 歸檔清單 AF-1~AF-12。煙霧測試在 r12 最後一次改 src 之後已跑過全過，體檢修正只動文件與測試斷言，未重跑）。
