...
- **A-2 `z-index` 跟著它代表的那個 iframe 走**(`getComputedStyle(frame).zIndex`,非數字取 `0`;
  **體檢改為取 iframe 最外層有數字 z-index 的祖先**,見「體檢輪修正」):
...
- `npm test` 1846 綠（1834 + 12）；體檢後 **1848 綠**（+14）。
- **突變測試 10 個，全部各自紅**（對定案改寫後的堆疊順序版本重跑；改寫前那一版跑的五個已作廢，
  因為它守的是不同的判定）：放回 overlay、z-index 拉到最高、z-index 不跟著 iframe、拿掉讓路、
  讓路後沒關掉代理層、移到別處不裝回去、拿掉 `mouseout` 出口、內部移動誤判為離開、
  不重算矩形、離開沒清代理層。
...
- 實作：Opus 5（commit `03985af`、`0b9c544`）。體檢：Fable 5.1。
- 實作方自陳最沒把握處：讓路的重新武裝時機、`z-index` 只看 iframe 自己是否夠。

## 體檢輪修正

1. **`picker-mode.js` `proxyZIndexFor`**——症狀：iframe 包在有 `z-index` 的定位容器裡
   （`.content { position: relative; z-index: 2 }`，很常見）時，代理層只看 iframe 自己拿到 `0`，
   在 body 層級輸給整個容器，**iframe 從此選不到**。這是 AF-11 之前沒有的退化（以前代理層在最高層永遠贏）。
   真實瀏覽器探針證實：容器 `z-index: 2` 時代理層 `0` 命中的是 iframe，`2` 才命中代理層。
   修法：沿 iframe 往上到 `<body>` 掃一遍，取**最外層**那個有數字 `z-index` 的值（那才是在 body 層級比高低的值；
   取最大值會拿到內層的 999 把頁面選單全蓋掉），負值仍取 `0`。
   迴歸測試：`o9` 新增四條（容器 z 2 → 代理層 2；巢狀 1/999/5 → 取 1；整條鏈 auto → 0；負值 → 0），
   兩個突變（只看 iframe 自己、取最大值）各自紅；煙霧案 (3) 改把 iframe 包進 `z-index: 2` 的容器，
   斷言代理層 `zIndex === '2'` 且仍能下鑽。
2. **設計上限記進 BACKLOG**（不是退化、本輪不做）：選單與 iframe 同在一個有 `z-index` 的容器裡時，
   代理層在 body 層級贏過整個容器，只剩讓路。正解與代價都寫在 BACKLOG 那一列。
3. 其餘角度無新發現：規劃逐條都有對應 diff，「明確不做」沒有混進來；`allProxies`／`stillOnYielded` 各有多個呼叫端，
   沒有單一呼叫者的抽象；新監聽有對應移除、狀態在 `exitPickMode` 重設；`overlay` 與代理層同為 body 子節點，
