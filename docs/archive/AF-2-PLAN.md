...
> 狀態:**全案完成**(2026-09-06),已併入 dev。測試 293 → 671 綠,煙霧測試通過。基線 `npm test` 293 綠 + `./run_smoke.sh` 通過。
...
| 階段 | 執行者 | 結果 | 驗收 | 落差與處置 |
|---|---|---|---|---|
| G0 基礎整備 | Claude + agy | 綠 | 309 測試 + 3 突變 + 煙霧 | 順手修掉既有時區缺陷:d6/d5c 測試用 UTC 取「今天」,紀錄日期卻取自當地時區的 slot,跨 UTC 午夜就紅 |
| F3 任務頁 | agy | 綠 | 337 + 4 突變 + 煙霧 | REPICK 走規格允許的退化路徑;終檢發現按了完全沒反應,R1b 才真正接上 |
| F4 設定頁 | agy | 綠 | 362 + 3 突變 | 看門狗時間讀了三個全 repo 沒有寫入端的鍵(終檢抓到,R1a 改讀診斷紀錄) |
| G1 格線數學 | forge 失敗 → agy | 綠 | 399 + 4 突變 | 先派地端 LLM:Qwen 端點回 507 記憶體不足,改 gemma-4 跑 926 秒四輪未收斂;照路由表(>200 行新模組)改派 agy 一次過 |
| G2 版面儲存 | agy | 綠 | 427 + 4 突變 | prune 漏掉 status 卡片的 `options.taskIds`(終檢抓到,R1b 修) |
| H1 資料序列 | agy | 綠 | 458 + 4 突變 | — |
| H2 SVG 圖表 | agy | 綠 | 486 + 4 突變 | — |
| H3 卡片渲染 | agy | 綠 | 518 + 5 突變 | 成功狀態只認 `ok`,`fallback` 被當失敗顯示破折號(終檢抓到,R1a 統一) |
| G3 儀表板接線 | agy | 綠 | 541 + 4 突變 + 煙霧 | 我寫的測試假設新增卡片會沿用指定座標(實際會自動配位),Claude 修測試;另有偷改儀表板 id 與復原競態(R1b 修) |
| I1 設定抽屜 | agy | 綠 | 559 + 5 突變 | 每次變更會靜默刪掉「目前型別看不到」的設定(終檢抓到,R1a 修) |
| I2 範本與頁籤 | agy | 綠 | 583 + 4 突變 + 煙霧 | 套用範本直接清空卡片、無確認也無法復原(終檢抓到,R1b 加確認框) |
| J1 Picker 加入儀表板 | agy | 綠 | 596 + 4 突變 | 我寫的「編輯不重複加卡片」測試是同義反覆(區塊沒填值時本來就不會加),Claude 補強後重測突變才紅 |
| J2 歷史查詢進階 | agy | 綠 | 622 + 6 突變 + 煙霧 | `renderPivot` 做完但沒有任何呼叫端,畫面切不到樞紐表(終檢抓到,R1a 接上 `#table-mode`) |
| K1 獨立 HTML 報表 | agy | 綠 | 639 + 4 突變 | 把 theme.css 的 20 個色碼逐字複製進 JS(終檢抓到,R1b 改成執行期讀 theme.css) |
| I3 抽屜補選項 | agy | 綠 | 644 | 併回前跨段 grep 抓到 `compare`/`mode`/`limit`/`taskIds` 四個選項有人消費、沒人設定得到 |
| R1a 終檢修復一 | agy + Claude | 綠 | 662 | 篩選監聽累加、刪除確認疊加、抽屜刪掉隱藏欄位、成功狀態統一、樞紐表接線、UI 不再直讀 chrome.storage、看門狗改讀診斷;agy 重構時漏掉一個變數造成回歸,Claude 補修三行 |
| R1b 終檢修復二 | agy | 綠 | 671 + 5 突變 + 煙霧 | 儀表板 id 不再被偷改、復原歷史只在真有變更時推、下次執行統一問 background、status 卡片連動、報表調色盤、範本確認、重新選取 |

## 併回前終檢(兩份獨立審查)

程式碼審查列出 15 項、文件審查列出 4 類。逐項自行驗證後:
- **真實缺陷 14 類全部修掉**(見 R1a / R1b 兩列)。
- **不採納**:`filterRecords` 同時吃 `min/max` 與 `valueMin/valueMax`、`pivot` 的第三參數、
  `autoArrange` 的 `widthByType`、`latest` 的 `today` —— 這些是 AF-1 既有契約或本輪規格明訂的簽章,
  既有測試依賴它們,移除是破壞性變更而非改善。
- **`report.js` 模組載入時執行 `loadAndRenderPage()`**:AF-1 既有做法,有 `chrome.runtime.id` 守衛,
  測試環境不會觸發,本輪不動。

## 回顧

- **有效**:測試先寫 → 委派 → 突變驗收這套維持零返工;14 段沒有任何一段需要回饋重跑。
- **測試品質才是天花板**:三次突變沒紅都是我的測試同義反覆(G3 座標假設、J1 編輯守門、I1 欄位分組),
  不是實作有問題。突變測試是唯一抓得到這件事的手段。
- **逐段驗收看不到段與段之間的斷鏈**:`renderPivot` 沒呼叫端、四個卡片選項設定不到、
  status 卡片沒被 prune —— 三件都是各段全綠、串起來才壞。便宜的跨段 grep(匯出對呼叫端、
  選項對消費端與設定端)抓到兩件,兩份獨立終檢抓到其餘。
- **地端 LLM 這台仍不堪用**:Qwen 載不進記憶體(507),gemma-4 在 166 行的純函式模組上四輪未收斂,
  應照路由表直接給 agy。

## 體檢方式(與 project-closeout §0 的差異,如實記錄)

本輪**沒有做「換模型再體檢」**。實際做法是:同一個 session 在併回前開**兩個獨立的
唯讀審查子代理**(一個審全 diff 的程式碼、一個審文件與規格一致性),各自只拿到分支 diff
與文件、不帶本輪對話脈絡,結果見上方「併回前終檢」與 R1a/R1b 兩列。
差異在於**換的是脈絡不是模型**。若要補做正式體檢:切換模型後對
`git diff <併入前的 dev>..dev` 重跑 project-closeout §1 的四個角度。
