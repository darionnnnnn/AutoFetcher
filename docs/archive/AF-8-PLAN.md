...
2. `docs/SPEC.md` §2。
3. 既有測試改寫：實際改到的是 `b4_pick_mode`、`p3_pick_toolbar`、`l7_batch_b3`、`p5_audit_fixes`、
   `p1_nested_cell`（規劃列的 `b6_pick_wiring` 走的是 background 訊息層，不受點擊語意影響，未改；
   `m2_chain` 改的是新增 `pos` 的鏈結測試而非點擊語意）。
...
- **Picker UI**：`#block-section` 加兩個下拉 `#row-pos`「列定位」與 `#col-pos`「欄定位」，選項「依表頭／第一筆／最後一筆／倒數第二筆」，預設「依表頭」。
  ~~進入時若使用者點的格子就在最後一個資料列，`#row-pos` 自動帶「最後一筆」~~
  → **實作時改為「只給建議、不改設定」**：兩列的匯率表點第一列（美金）與每日成交表點最後一列
  在資料上長得一樣，猜錯就是默默換掉定位方式。建議寫在 `#block-summary`（`role="status"`），決定權留給使用者。
  整欄／整列模式（`block.axis`）同樣顯示；~~`pos` 非「依表頭」時聚合下拉隱藏~~
  → **終檢時修正為**：同一軸的定位對整欄／整列沒有意義（整欄的位置來自列定位），該軸的下拉**停用並說明**；
  聚合下拉只在「實際生效的那一軸」被位置取代時才隱藏。
  編輯既有任務時從規格回填（多值取第一個值的 `pos`）。
- **`#block-summary` 與即時預覽**：實際文案是「每次抓取取最後一列，不看標題」（欄名已在上方的值清單裡，
  不重複）；還沒設定位置但看起來用得上時，這一行改放建議。「立即測試」走 background 真擷取，不另算。
...
- ~~**選取模式的預覽**：`getCellText`／`getBlockPreview` 對帶 `pos` 的規格算當下位置。~~
  → **不需要做**：這兩個函式吃的一律是已選清單的項目，而已選清單只存索引——選取當下建立的 pick
  本來就沒有 `pos`，帶 `pos` 的 preselect 也在 `applyPreselect` 就換算成當下的索引了。
  加一條走不到的分支只是死碼；已在 `picker-mode.js` 的函式上方註明理由。
...
併 dev 後全量 1677 綠；體檢修正 commit（`9487e93`）逐行自審無新發現；文件稽核：SPEC 已無輪次敘事、
CLAUDE.md 基線與地雷已更新、BACKLOG 補兩項。終檢無新發現。

## 體檢交接

- **全量測試**：`npm test` **1666 綠 / 0 紅**（上輪基線 1558，本輪 +108）。
