# AF-14 第 14 輪規劃：純數值列標題定位收緊＋試抓失敗匯出診斷包
...
> **讀這份之前**：批次 A 的「定案／改動／測試驗收」三節是**原定案，已於 2026-09-11 收尾輪推翻**。
> 原定案是「純數值標題不當錨點：選取端過濾、擷取端視同空字串、顯示靠 `rawHeader` 側通道」；
> 終檢發現年度欄（`2024`/`2025`）的表會從黃燈警示變成靜默抓錯欄，於是改為
> 「**純數值標題只在當下唯一出現時才拿來定位**」，選取端原文照存、`rawHeader` 與 `anchorHeader` 都不存在。
> **現況以〈終檢後定案〉與 SPEC §7 為準**；下面被推翻的段落保留，是為了留下「為什麼不那樣做」。
...
| 批次 | 內容 | 規模 | 相依 | 執行者 |
|---|---|---|---|---|
| A | 純數值列標題定位收緊（原定案「不當錨點」已推翻，見〈終檢後定案〉；判準一份、舊任務零遷移、失敗訊息帶現況、選取端當場提示） | 中 | 無 | agy |
...
1. 作業 A 走「**嚴格純數值**列標題不當錨點 ＋ 選取端當場提示 ＋ 失敗訊息帶現況列標題」。**（「不當錨點」已於收尾輪推翻，見〈終檢後定案〉）**
...
## 批次 A：純數值列標題不當錨點（原定案；定位規則已於收尾輪推翻）
...
### 定案（原定案——「選取端過濾」「擷取端視同空」「`anchorHeader`」「`rawHeader`」四項已推翻，其餘仍有效）
...
- **失敗訊息帶現況**（`headerGoneMessage`）：「標題「X」找不到；目前這張表的列標題是：A、B、C（最多列 5 個，超過加「…共 N 個」）；若這張表每天新增一列，請到任務設定改用位置定位（…）」。
...
- **選取端當場提示**：Picker 摘要卡（`#summary-target`）在「任一被選格子的列（或欄）標題被判為純數值」時多顯示一句：
  ...
  「列定位」下拉本輪**不搬出進階區**（動版面的收益不確定）。
  **規劃時的事實錯誤（實作時更正）**：那兩個下拉根本不在進階區，而是在「抓什麼」區（`#block-section`），
  所以不需要展開任何東西。實作改成摘要卡旁一顆捷徑鈕（`#goto-rowpos`），
  點了把焦點送到 `#row-pos` / `#col-pos`（被擋下的是欄就指向欄）。
...
### 測試 / 驗收（Claude 先寫，執行端自驗；「header === ''」類斷言已於收尾輪改為原文照存）
...
| 終檢修正（Opus 5） | Claude | 通過 | 全套 1929 綠、突變 3 發皆紅 | 修 `rawHeader` 從重選漏進規格、多值試抓拿不到診斷、匯出鈕狀態殘留、命名鏈第四份、兩條假通過測試 |
| 逐條比對補驗收（Opus 5） | Claude | 通過 | 全套 1932 綠、突變 2 發皆紅 | 補「連續選兩表」「例外出口診斷」「紀錄帶新訊息」三條規劃列了但沒守門的驗收 |
| 收尾定案（Fable 5.1） | Claude | 通過 | 全套 1932 綠、煙霧 Chrome/Edge 全過、突變 4 發皆紅 | 定位規則改「唯一出現才定位」；選取端回到原文照存、preselect 共用 `locateByHeader`、刪 `rawHeader`/`anchorHeader` |
| 收尾體檢（Fable 5.1，兩個 Opus low 代理） | Claude | 通過 | 全套 1936 綠、突變 4 發皆紅 | 程式碼六條全修：提示句不看 pos（使用者照做後警語關不掉）、整欄整列的捷徑鈕指向被停用的下拉、一鍵命名是命名鏈第四個入口沒套判準、Picker 自己算了第二份「哪一軸」判定（收成 describe.js 匯出一份）、例外出口的診斷包永遠寫「找不到框架」（`loc` 提到 try 外）、preselect 退回索引時欄索引無上界。文件五條全修：PLAN 前半段加「已推翻」標記、數字、執行紀錄補收尾輪、BACKLOG 一項前提錯誤改寫、補「唯一出現才定位」的刻意取捨 |
...
程式碼與文件各派一個獨立代理審全 diff。**抓到三個真 bug（其中兩個是本輪引入的）與一個假通過的測試**。

| # | 發現 | 嚴重度 | 處置 |
|---|---|---|---|
| 1 | `rawHeader` 從**重選路徑**漏進 `task.spec`：`background/main.js` 的 `pickSpecOf` 對 `cell` 整包照抄，繞過 `buildSpec` 的過濾。連帶讓 `sameSpec` 的全等比對永遠對不上 → key 重生、歷史序列斷掉、使用者改過的名稱被覆蓋 | 高 | 已修：`pickSpecOf` 改逐欄挑。補鏈結測試（`m2_chain`：重選存回後規格不得含 `rawHeader`） |
| 2 | 多值任務試抓失敗**拿不到診斷**：`extractValue` 對多值回 `ok:true`（SPEC §7），content 與 fetcher 都只看整體 `ok` → 本輪主打的情境反而沒有匯出入口 | 高 | 已修：兩端都改以「有沒有值失敗」判斷。補測試 B1-4/B1-5/B2-4/B2-5 |
| 3 | 匯出鈕在 `render()` 後不收：換目標後按下去會匯出**上一頁**的網址與 HTML 片段（與 AF-7 `pickedTableEl`、AF-9 `undoSnapshot` 同型的狀態殘留） | 中 | 已修：`render()` 一併清掉。補測試 B3-5 |
| 4 | `defaultPickName` 是命名鏈裡唯一沒套判準的一份 | 中 | 已修：四份收斂成同一個 `anchorOnly` 薄包裝 |
| 5 | 測試假通過：B2-3 的替身回應根本沒有 `debug`，斷言「紀錄裡沒有 debug」在任何實作下都成立 | 中 | 已修：替身改回真實形狀（content 會附 `debug.page`），突變後確認會紅 |
| 6 | 弱斷言：驗「診斷讀分頁實際網址」時，任務網址與分頁網址寫成同一字串，開新分頁也會過 | 中 | 已修：改成真的建分頁、網址帶 query，兩者可區分 |
| 7 | 規劃寫了但漏掉：`m2_chain` 鏈結測試、`defaultFieldName` 的守門、「改用列定位」的可點入口 | 中 | 已補齊。**規劃的事實錯誤**：那兩個定位下拉不在進階區而在「抓什麼」區，實作改成捷徑鈕 |
| 8 | 文件落差：SPEC §2.1 命名鏈、§4 立即測試流程沒跟上；失敗訊息的「沒有標題」分支與例外出口的 `raw` 未描述 | 低 | 已補進 SPEC |

修正後又跑了三發突變（`pickSpecOf` 整包照抄、`render` 不清診斷、多值失敗不組診斷），**都紅**。
其中「多值失敗不組診斷」第一次突變是**存活**的，追下去正是第 5 條那類問題——
測試造了現實中不會出現的回應，斷言到的其實是 content 原樣轉發的欄位；改成真實形狀後才有訊號。

## 終檢後定案（2026-09-11，Fable 5.1 收尾輪）

**裸年份當欄標題的表會失去警示**是真的退化。收尾輪重新推演後採**「先比對、找不到才退回索引」的收緊版**：
純數值標題**只在當下唯一出現時**才拿來定位；不見了或重複出現都視同沒有標題、走索引、狀態 `ok`。
「唯一出現」這一條是原本選項 2 沒有的——第一欄是 `0`/`1` 這種小整數的表，拿去比對會跳到最近的那一列，
加上唯一性就收掉了。四種情況與行為見 SPEC §7 的表。

這個定案讓設計**變小**：
- 選取端不再過濾標題（回到基準版寫法），A-2 的過濾整段拿掉；
- `rawHeader` 這條顯示用側通道（終檢抓到會從重選路徑漏進規格的那個）整個消失，
  摘要卡與捷徑鈕改從規格裡的 header 本身判定；
- preselect 改直接呼叫 `extract.js` 匯出的 `locateByHeader`，「畫面勾到的」與「擷取抓到的」保證同一格；
- `table.js` 的 `anchorHeader` 沒有呼叫端，刪除；`isAnchorText` 只剩命名與提示兩個用途。
突變四發（不見了不退回、一律走索引、重複也比對、preselect 自己比對）全部紅；
其中「重複也比對」第一次存活，原因是測試造的表讓「最近的一列」剛好就是原列——改成原列的值已變、
前兩列重複之後才有訊號。

## 體檢交接

- **全量測試**：`npm test` → **1936 綠 / 0 紅**（基準 dev@7e47e1e 是 1874 綠，本輪 +62）。
- **真實瀏覽器**：`./run_smoke.sh` **Chrome 與 Edge 全過**，新增兩案（純數值標題舊任務取到 38605、擷取失敗附得出診斷現況）皆通過。
- 版本 0.13.0（`src/manifest.json` 與 `package.json` 同步）。
- 分支 `feature/AF-14`，commit：A-1 `6e516f7`、A-2 `208ab65`、A-3 `0fc34a6`、B `ea2f182`、C `6c8fbb2`、
  終檢修正 `d2d2f7a`、補驗收 `59be27d`、收尾定案 `5efa47b`。
- 真實瀏覽器煙霧（Chrome + Edge）在收尾定案之後**重跑全過**。
