...
併入 dev@56c553b 後全量 2445 綠；Chrome for Testing 煙霧全部通過。Edge 在收尾當下瀏覽器行程啟動失敗（`Failed to launch the browser process`，重試一次相同，屬環境問題）；實作完成時同一份煙霧在 Edge 全過，之後的體檢修正由 Chrome 煙霧與 jsdom 迴歸測試覆蓋。
合併無衝突、`feature/af-19` 與 dev 無差異；規劃比對與文件稽核無新發現。
待使用者畫面實測：任務頁多選與整批動作、整批改排程面板、選取模式按兩次 Esc、拆成每個值一個任務、面板自動換角與捲動。
