...
- **前置動作的複合型**與**結構 A 解析改善**：見上。（面板寬度記憶已改列 BACKLOG）
...
|---|---|---|---|---|
| B-0 探針 | Claude | 完成 | 14 項框架事實實測（見上表） | **推翻兩項原設計**：面板無法自行判斷 tabId（改 windowId + visibilitychange）、草稿由「可選」變「必做」 |
| D 表格判準合一 | Claude | 完成，1763 綠 | `s1_table_unify` 14 條；突變 4 項全紅 | M4（格的巢狀過濾）第一次沒被抓到 → 補一條 ARIA 巢狀退路測試才守得住 |
| C 已選就鎖表 | Claude | 完成，1778 綠 | `s2_lock_table` 15 條；突變 5 項全紅 | M3（col 模式鎖定非表格）第一次沒被抓到 → 補「不得出現已鎖定」斷言。改寫三條舊語意測試（m2/n1/r3） |
| A 前置動作 | Claude | 完成，1791 綠 | `s4_preaction_hover` 13 條；突變 7 項全紅 | 兩條測試原本把中文化斷言放錯層（中文化在 background，只有它知道第幾步）→ 改斷言層級並補 background 鏈結測試。改寫 f5 兩條舊單位契約 |
| B side panel | Claude | 完成，1807 綠 | `s3_side_panel` 16 條；突變 9 項全紅 | 六處舊測試改寫成面板契約；Edge 相容封鎖清單移除 `sidePanel`（查證 Microsoft 官方 API 支援表）；`tabs.getCurrent` 取代查作用分頁 |
| E 收官 | Claude | 進行中 | 版本 0.9.0、SPEC/BACKLOG/CLAUDE.md/README 已更新 | 煙霧 Chrome + Edge 全過 |

### 實機驗證（headful，真實擴充功能）

| 項目 | 結果 |
|---|---|
| Report 任務頁「編輯」→ side panel 開啟 | **通過**（真實點擊手勢） |
| 面板寬度 | **360px**，無橫向捲動 |
| 等待態／表單切換 | 正確（編輯直接顯示表單） |
| 編輯 ctx 由 `storage.session` 帶入 | 正確（標題與名稱都是該任務） |
| 面板與 Report 頁的 console 錯誤 | 0 |
| `contextMenus.onClicked` 內的 `sidePanel.open` | **未能自動化驗證**（原生選單無法程式點擊）。Chrome 文件明列該情境合法，且 `openPanel` 失敗會退回彈出視窗並記 `panel_fallback` 診斷；**留給使用者實測**，不成立則改走 popup 入口 |

## 併回前終檢（兩份獨立審查，2026-09-09）

程式碼與文件各開一份獨立審查，**共抓到 17 項**，全部處理完畢。其中五項是真缺陷，
不修就會在使用者眼前壞掉：

| # | 發現 | 嚴重度 | 處置 |
|---|---|---|---|
| 1 | `picker.js` 的換目標呼叫了**不存在的 `collectValues()`** | 阻斷 | 改用真正存在的欄位快照；補一條**跨到面板端渲染**的鏈結測試（原本只驗 background 寫了什麼，崩潰完全逃掉） |
| 2 | `site.js` 仍讀網址參數，**站台登入面板實質不可用**（生產端改了、消費端沒改） | 阻斷 | 改讀 session ctx，網址參數留作舊版退路 |
| 3 | 面板的 `pagehide` 被當成「面板關閉」，但**切換分頁也會卸載面板文件** | 設計級 | **移除 pagehide 通道**：它會把草稿與頁面標示清掉，正是本輪要修的症狀。通道剩 `onClosed` 與 `tabs.onRemoved`，代價寫進 SPEC 與 BACKLOG |
| 4 | 送出時把**別的用途**的保留標示改群（前置動作送出一次，任務目標的藍框就會被下一次 `Esc` 抹掉） | 高 | 只標「這一輪選的」（`:not([data-af-held])`）；補「preaction 送出成功」的案例（原本只測到「進去就 Esc」） |
| 5 | 存檔後**不清草稿也不關面板** → 下一個新任務被上一個的設定灌進去 | 高 | 接上 `CLOSE_PANEL`（連同 ctx 一起清） |
| 6 | 版本升到 0.9.0 後**測試是紅的**（`r5` 把版本號寫死成 0.8.0） | 高 | 改成驗格式與兩處一致，不釘死數字 |
| 7 | 舊版退路的彈出視窗**不帶參數**，開出來是空表單 | 中 | `openPanel` 多收一個退路 query |
| 8 | `closePanelFor` **不冪等**，每次觸發都廣播一輪 `EXIT_PICK` | 中 | 以「暫存還在」當旗標；原本的冪等測試是恆真斷言，一併改寫 |
| 9 | repick **取消時**不收分頁；`repickTabs` 放模組級 Map（worker 被回收就整張歸零） | 中 | 取消也收；改放 `storage.session` |
| 10 | `RESOLVE_PANEL_TAB` 查不到時**猜**目前作用分頁 | 中 | 改回 `null`，面板下次可見時自癒 |
| 11 | 啟動時沒有全域停用面板 → 工具列右鍵可開出空表單 | 中 | 補上（B-0 #9 已證實分頁層仍可開） |
| 12 | `CELL_SELECTOR` 漏了 `role="rowheader"`（定案寫了、實作漏了） | 中 | 補上＋測試（漏掉會讓整列欄索引位移） |
| 13 | 工具列判定來源仍在 `updateToolbar` 裡看 hover 目標（行為靠 `upgradeTarget` 間接達成，那條突變無處可改） | 中 | 判定寫成明確一行；順帶發現 **`upgradeLastPickTo` 判定不同源** → 「按鈕亮著、按下去沒反應」，一併修 |
| 14 | `open` 之前夾了 `await setPanelCtx`（手勢跨非同步等待有失效風險） | 中 | 改成**先開面板再寫 ctx** |
| 15 | `shared/table.js` 內部仍有兩份表頭列判準 | 低 | 內部那份改成薄包裝 |
| 16 | 三條假斷言（`<= 2`、恆真的冪等、只驗 background 的換目標） | 低 | 全部改成真的守得住的斷言 |
| 17 | 文件：SPEC 仍寫「視窗尺寸 600×820」、README 仍寫「跳出設定視窗」、CLAUDE.md 結構圖漏兩個新檔、BACKLOG 少一條 | 低 | 全部更正 |

**一條定案被另一條取代並記錄**：C-5「`pendingMode` 兌現時同時 `upgradeLastPickTo`」在 C-3
（工具列改以 `pickedTableEl` 判定）之後**不可達**——有已選時三段一律可用，
不可能走到「點到停用的整欄」那條路。已移除該行並在程式碼註解寫明理由，不留死碼。
...
- **待使用者實測**：右鍵選單開面板（見上表最後一列）、hover 型前置動作對真實站台的選單。
