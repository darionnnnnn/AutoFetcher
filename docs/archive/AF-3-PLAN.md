...
| B1 區塊型別偵測 | forge(地端 gemma-4) | 綠 | 15 | Qwen 載不進記憶體(507),換 gemma-4 兩輪過。**弱斷言遮住一個 bug**:ARIA 表格的 `role="cell"` 沒被計入欄數,我原本的測試剛好只斷言到表頭那一列所以照樣綠;補強測試後才紅 |
| B2 選取模式 | agy | 綠 | 24 | 一次過。它加了 `MSG?.PICKED \|\| 'PICKED'` 這種防禦 fallback(規格明文禁止),拿掉之後才暴露出它其實相依於還沒加的訊息型別;另外列索引寫成 1-based 而欄索引是 0-based(我的測試斷言寫得含糊,它照著填),以及每次滑鼠移動都重跑 `detectKind`(大表格上會卡),三處由 Claude 修 |
| B3 三端接線 | agy | 綠 | 10 | 一次過。連帶要改四條寫著舊契約的既有測試(右鍵開視窗、REPICK、任務頁自己開分頁),逐條改寫成新契約而不是刪掉 |
| B4 Picker 區塊與配色 | agy | 綠 | 8 | 一次過。驗收時發現「選了表格但沒點任何一格」會顯示「取第 1 欄」並存下 `index: null`——`Number(null)+1` 剛好是 1,測試沒蓋到;補測試後修掉 |
| E1 告警判定 | forge(地端 gemma-4) | 綠 | 20 | 一輪過。Claude 修兩處:`deltaPct` 的分母沒取絕對值(前一筆是會計負數時算不出變動)、`eq` 用絕對 EPSILON 對大數等同精確比較 |
| E2 告警接線 | agy | 綠 | 12 | 一次過。`prevRecords` 的過濾條件寫得繞(排除「本筆」但本筆根本還沒寫入),Claude 簡化 |
| E3 告警介面 | agy | 綠 | 11 | 第一次呼叫 agy 直接 exit 1、零輸出(額度正常),重跑就過。驗收時三個突變存活,原因是我寫了三條「grep 原始碼字串」的假測試;改成真的跑行為,並把月曆統計抽成純函式 `buildDateStats` 才測得動 |
| C1 聚合 | forge(地端 gemma-4) | 綠 | 12 | 第一次四輪全敗,**是我的測試錯**:我假設 `parseNumber` 認全形括號的會計負數,它其實不認。修 `parseNumber` 後重跑一輪過 |
| C2 表格解析 | agy | 綠 | 16 | 一次過,`rowspan`/`colspan` 的網格展開寫得正確(規格有指名做法) |
| C3 block 擷取 | agy | 綠 | 14 | **越界修改**:規格明寫「不要動 parseNumber」,它為了過我的測試加了「整串像日期就不當數值」。稽核後判定這個行為對繁中頁面是對的(把 `09-02` 讀成 9 是看不見的錯誤),於是保留、補測試釘住邊界、寫進 SPEC |
| D1 密碼加解密 | forge(地端 gemma-4) | 綠 | 8 | 兩輪過。`decryptSecret` 複製了一份金鑰載入邏輯,Claude 收斂成共用的 `loadKey()` |
| D2 登入流程 | agy | 綠 | 12 | 一次過(第一次因為我的 `&&` 指令鏈斷掉根本沒送出)。突變驗證時發現我一條斷言是同義反覆,改寫成「就算網址巧合跳到成功頁,content 說填不進去就是失敗」 |
| D3 站台檢查與管理 | agy | 綠 | 16 | 一次過 |
| D4 站台設定視窗 | agy | 綠 | 10 | 跨段 grep 抓到的斷鏈:右鍵「設定此站台登入」會進入選取模式但 `PICKED` 的 `login-*` 什麼都沒做,使用者走進死路 |
| F1 前景與前置動作 | agy | 綠 | 17 | 一次過 |
| F2 前置動作介面 | agy | 綠 | 24 | 一次過。突變驗證抓到 `Number('')` 是 0,空白的等待欄會被存成 `ms: 0` |

**突變驗證**:每一段都做,共 50 個突變。全部由綠轉紅,除了四個判定為**等價突變**
(移除的檢查在下游另有等效攔截,行為不變)。有效突變存活的只有六次,**六次都是我的測試太弱**,
不是實作有問題——與 AF-2 的結論一致:補強測試後才紅。

**跨段 grep 抓到、逐段全綠看不到的斷鏈**(三件):
`PICKED` 的 `login-*` 分支沒有去處(D4 補);`alertCooldownMin` 與 `siteCheckTime`
有消費端沒有設定端(補進設定頁);`login_failed` 是新的紀錄狀態但歷史頁篩選不到(補進清單)。

**自查專案硬規則**時另外抓到兩件(這輪之前就有):`popup.js` 直接讀 `chrome.storage`
(已改走 `shared/storage`);`background/health.js` 的 badge 色碼是必要豁免
(`setBadgeBackgroundColor` 只吃色碼字串),已記錄並寫進守門測試。

## 體檢交接

- 實作模型:Claude Opus 5(全部 32 段規格、驗收、突變、第一輪體檢修正皆由它主持;委派端為 agy 與地端 gemma-4)。
- 體檢模型:Claude Fable 5.1(使用者 `/model` 切換後下「進入收尾流程」)。
- 第一輪體檢(下一節)是 Opus 5 用**兩個不同模型的審查子代理**做的,但**修正本身沒有被任何獨立視角看過**;
  第二輪(再下一節)由 Fable 5.1 針對那三個手改 commit(`d3683a6`、`49bb3fb`、`5259889`)逐行讀 diff。

## 體檢輪修正

體檢由**兩個不同模型**的獨立審查代理進行(程式碼 diff / 文件一致性),只給它們
`git diff dev..r3` 與文件、不帶本輪對話脈絡。逐項自行驗證後修掉的:

| # | 症狀 | 為什麼會發生 | 修法 | 迴歸測試 |
|---|---|---|---|---|
| 1 | **block 模式對真實使用者是壞的**:Picker 存出來的任務去抓,得到 9(整張表 textContent 的第一個數字)而不是 42 | `buildTask` 沒寫 `spec.mode='block'`,而 `extract` 是看 `spec.mode` 分派。我的測試**全部自己手組 spec**,連煙霧測試也是手塞,所以逐段全綠也看不到 | 補一行 `spec.mode = 'block'` | `c5` 新增「從 `buildTask` 走到 `extractValue`」的端到端測試 |
| 2 | **含密碼的設定匯出/匯入整條是壞的**,而且會在 storage 留下明文 | 匯出只認早已不存在的明文 `password` 欄位、把 `passwordEnc` 直接刪掉(secrets 永遠是空的);匯入把明文寫回 `password`,而登入端只讀 `passwordEnc`。既有測試**把「明文被寫回 storage」當成正確行為在斷言**,所以抓不到 | 匯出先解密再以密語加密;匯入以本機金鑰重新加密;`e2` 舊測試改寫 | `d11` 九條:往返、密語錯誤不寫入、不含密碼時的行為 |
| 3 | **schemaVersion 2 遷移沒做**(規劃明列):舊站台的自動登入必定失敗,三次後整個站台被停用 | 規劃寫了但實作段沒排進任何一段的驗收標準 | `storage.init` 做一次性遷移:`loginPageUrlPrefix` → `loginCheck`、明文 `password` → `passwordEnc` | `d11` 四條(含「只做一次」與「新格式不受影響」) |
| 4 | **站台健康沒進燈號**(規劃批次 D 改動第 9 項漏做):每日檢查失敗只跳一次通知,燈號永遠不變色 | 同上 | `computeHealth` 納入 `site:` 前綴的健康項 | `d9` 四條 |
| 5 | 告警通知點擊只定位到日期,篩不到任務 | 產生的是 `task=`,報表解析的是 `taskIds=` | 改成 `taskIds=` | `e4` 改成用 `parseHash` 真的解析一次 |
| 6 | 保留天數的偏好無效(`trimOldRecords` 零呼叫端) | 既有缺陷 | `writeRecord` 之後清一次 | `d12` 兩條 |
| 7 | popup 的「最後值」永遠是破折號(`lastValues` 無寫入端) | 既有缺陷 | 成功時寫 `lastValues`,失敗不覆蓋 | `d12` 兩條 |
| 8 | 看門狗清理卡住的執行**從未觸發過** | 比對 `typeof startedAt === 'number'`,但寫進去的是 ISO 字串 | 兩種型別都吃 | `d12` 兩條 |
| 9 | 任務頁的「下次執行」印毫秒 timestamp | 既有測試餵字串進去,把錯的假設寫死了 | 格式化;測試改用真實契約 | `d12` 一條 + `f3` 改寫 |
| 10 | 設定頁 HTML 匯出還是「下一段完成後啟用」的佔位文字 | AF-2 做完沒回頭改 HTML | 拿掉 disabled 與佔位字 | `d12` 一條 |
| 11 | `popup.js` 直接讀 `chrome.storage`;`CLAUDE.md` 三條規則重複 | 自查與審查各抓到一半 | 改走 `shared/storage`;合併重複條目 | `a4` 新增兩條守門 |

**文件如實化**:SPEC 有六處寫了但沒實作(燈號來源、popup 按鈕、通知點擊範圍、重試次數、
`daily` 用 `periodInMinutes`、重新選取),改成現況;落差記進 BACKLOG(10 項,附觸發條件)。
README 重寫成「怎麼建立第一個任務」——原本教的兩個選單項目已經不存在。

測試數 909 → 931。

## 體檢輪修正(第二輪,換模型)

範圍:`origin/dev..dev` 共 48 個 commit,重點是第一輪體檢之後、沒被獨立審查看過的三個手改 commit。

| # | 症狀 | 為什麼會發生 | 修法 | 迴歸測試 |
|---|---|---|---|---|
| 1 | **效能回歸**:第一輪把 `trimOldRecords` 接進 `writeRecord`,而它做 `storage.local.get(null)`——每抓一筆值就把整個 storage(含全部歷史紀錄)載進記憶體 | 修「保留天數沒人讀」時只看到「要有人呼叫」,沒看它的成本 | 移到看門狗,且 `trimOldRecords` 自己以 `lastTrimDate` 保證一天只做一次;寫入路徑零全量掃描 | `d12` 三條(看門狗會清、一天一次、寫入不掃) |
| 2 | 站台停用或刪除後 `health['site:<origin>']` 留著,燈號**永遠紅、沒有任何途徑清掉**(popup 的已讀只對任務 id) | 第一輪把站台項納入燈號時,審查明講「對應站台仍 enabled 的項目」,實作漏了這個條件 | `deleteSite` 與設定頁的停用一併移除該健康項(新增 `storage.deleteHealthEntry`) | `d9` 兩條 |
| 3 | 匯入舊版設定檔時站台的 `loginPageUrlPrefix` 不會轉成 `loginCheck`(規劃寫「匯入舊版設定檔也要走同一段搬移」) | 遷移只接在 `init`,匯入端沒接 | 抽出 `normalizeSiteShape`,`init` 遷移與匯入共用 | `d11` 一條 |
| — | 疑慮解除:`storage.init`(含遷移與金鑰產生)只在 service worker 執行,UI 不呼叫,不存在雙端同時產生金鑰的競態 | — | — | — |

另外核對:三個手改 commit 內的其餘變更(`spec.mode='block'`、通知 `taskIds=`、密碼匯出入往返、`startedAt` 型別、`ensureSiteCheck`、任務列標記)逐行讀過,邏輯與測試一致,未見新問題。
五個突變全部由綠轉紅;947 綠;煙霧測試九段全過。
