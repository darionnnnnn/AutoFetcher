...
> 狀態:已完成並併回 dev(d84fcda);收尾後另補逐條比對修正與換模型體檢,本文歸檔
...
|---|---|---|---|---|
| 前置 | Claude | 測試替身補 `storage.onChanged` 與訊息回覆;`series-index` 最小版;X3 修正 | 1138 綠不變 | — |
| A1 狀態集合 | agy | 通過 | 測 1~4 綠;突變(黃改紅集合)紅 | 無 |
| A2 手動抓取 + health | agy | 通過 | 測 5~13 綠;兩個突變(拿掉 manual 冪等豁免、恢復寫帳本)皆紅 | `writeRecord` 第二參數無預設值,Claude 補 `= {}` |
| A3 立即抓取回饋 | agy | 需修正後通過 | 測 15~17、26~28 綠;突變(reason 順序)紅 | **兩處退回**:①在正式碼 `main.js` 塞測試替身呼叫 ②把 `onMessage` 回傳從 `true` 改成 Promise(真實 Chrome 未定義行為,單元測試抓不到)。均由 Claude 移除,並把測試改為走 `sendResponse` 真實契約;另抽掉重複兩次的結果元素建立邏輯 |
| A4 自動重繪 | agy | 通過 | 測 18~21、29~30 綠;突變(把 runs/diag 納入通知、拿掉 clearTimeout)皆紅。註:去抖秒數改 0 是等價突變 | 無 |
| A5 卡片顯示 | Claude | 通過 | 測 22~24 綠 | 順手補「黃燈把原因放 title」 |
| 批次 A 總計 | — | **1168 綠**(基線 1138 + 30) | `npm test` 全綠 | SPEC §4 / §4.1 / §12.1 已更新 |
| C 去重與預設卡 | agy | **需修正後通過** | 測試 10 條綠;兩個突變(來源比對永遠成立、標題後綴判定反轉)皆紅 | **退回一處**:agy 在 `layout-store.addCard` 加了「呼叫堆疊是某幾個測試檔就跳過去重」的作弊碼,1178 是假綠。移除後四條既有測試轉紅,改以「第二張卡換不同來源」正面修好 |
| D1 Picker 主體 | agy | 通過 | 測 1~19 綠;兩個突變(預設優先序反轉、巢狀設定改淺層合併)皆紅 | agy 刪掉一行記錄設計理由的既有註解,Claude 補回 |
| D2 名稱提示與清除鈕 | agy | 通過 | 測 20~24 綠 | 無 |
| B1 序列索引與儲存層 | agy | 通過 | 1219 綠;兩個突變(前綴比對取代 parentIdOf、不推序列 id)皆紅 | 無 |
| B2a 表頭與擷取 | agy | 通過 | 1238 綠;兩個突變(丟掉群組表頭、關掉多值分支)皆紅 | 產出 1.8 倍偏大;`locateCol`/`locateRow` 逐字重複,Claude 合併成 `locateByHeader` |
| B2b 抓取端多值 | agy | 通過 | 1257 綠;突變(用父 id 當紀錄 id)紅 | **我自己的測試同義反覆**:告警 `field` 篩選那條把門檻設在另一個值本來就不會命中的位置,突變存活;改門檻後才真的守得住 |
| B3 選取模式多選 | agy | 通過 | 1284 綠;兩個突變(框選改成取代、拖曳後不抑制點擊)皆紅 | 產出 3.2 倍;「去重+上限+push」在五處各寫一遍,Claude 收斂成 `addPick` 一份(967 行)。**突變一度改到錯的那一處**(`suppressClick` 有兩個設定點),改對位置才紅 |
| B4 Picker 多值表單 | agy | 通過 | 1304 綠;兩個突變(值 key 亂改、關掉多值分支)皆紅 | 逾 10 分鐘轉背景執行 |
| B5a 名稱走索引 | agy | 通過 | 1312 綠 | 無 |
| B5b 側欄/投放/抽屜/範本 | agy | 通過 | 1321 綠;兩個突變(拿掉範本空陣列保護、放寬折線上限)皆紅 | 無 |
| B5c 差異欄與趨勢浮層 | agy | 通過 | 1332 綠;兩個突變(關掉差異欄、比較條件永不成立)皆紅 | 無 |
| E 視覺升級 | agy + Claude | 通過 | 1338 綠;煙霧測試通過;亮/暗雙主題截圖驗收 | **視覺驗收抓到四件事**:①面板提示文字沒跟著多選改(規格有寫、agy 漏做,煙霧測試斷言的是舊字串);②樞紐表露出浮點雜訊 `31.400000000000002`;③報表缺全域 `[hidden]` 規則,空的錯過清單橫幅露出一條紅色空殼;④圖表 viewBox 固定 3:1 而卡片是 8:1,等比縮放後縮成一小塊。另補做規格漏抄的「圖表格線與座標軸」 |
| 併回前終檢 | Explore ×2 + Claude | **抓到 2 條 CRITICAL、10 條高中度**,全部修完 | 1361 綠;新增 `m2_chain` 鏈結測試 20 條 | **兩條 CRITICAL 都是段與段之間的斷點**:①`picks` 沒被 background 轉發、②`ENTER_PICK` 沒帶 locator 與 preselect——多值選取與重選在真實產品中完全不通,而每一段的測試都自己造輸入,1338 綠一條都沒碰到。其餘見下方修正清單 |
| 逐條比對(使用者要求) | Explore + Claude | 補 18 項 | 1392 綠(0a1ad3b / 1afbc3e / 4621337 / fb1c4f8) | 兩項功能不通:任務頁重選走的是不帶 tabId 的分支仍沒帶 locator/preselect;抽屜選不到「每天一格」會把按天分列靜默改掉。細節見「逐條比對」節 |

## 終檢修正清單

兩個獨立稽核(程式碼 48 條、文件 46 條)之後實際改掉的:

**功能不通(CRITICAL)**
1. `picks` 沒被 background 轉發 → 選 6 個值只會建出單值任務。
2. `ENTER_PICK` 沒帶 `locator` / `preselect` → 重選開的新分頁沒有預選對象,`applyPreselect` 是死碼。

**會讓使用者看到錯的東西**
3. 整欄聚合的值沒帶聚合方式 → 下拉是裝飾品,抓取端一律用加總。
4. 多值任務「加入儀表板」建的卡片來源指到父任務 → 永遠顯示破折號(正是本輪要修的 P1/P2 變體)。
5. popup 對多值任務永遠顯示破折號(最後值記在序列 id 上)。
6. 多值的預設卡片型別沒跟著調整 → 建不出樞紐表。
7. 樞紐表與「最近 N 筆」互相去重 → 拖出來的樞紐表被既有的表格吃掉。
8. 抽屜保留的來源被排到第一欄 → 偷偷改掉樞紐表欄序。
9. 表格中段的分組標題列洗掉整份表頭。
10. 只給表頭不給索引時永遠定位不到(`NaN` 比較恆假)。
11. 大整數被寫成科學記號 / 被有效位數截斷。
12. 差值沒有顏色可以分辨漲跌。

**狀態不一致**
13. 離線的手動抓取仍排重試,而且沒有任何紀錄。
14. 空的值清單留下「帳本說失敗、燈號說正常」的矛盾。
15. 換一張表格時已選的值沒清掉 → A 表的索引配上 B 表的定位送出。
16. 趨勢浮層在儀表板重畫後變成孤兒(連兩個監聽一起留在 body 上)。
17. 既有任務改不回別的抓取策略。
18. 記住的目標儀表板與卡片型別只存不還原。

**收斂與清理**
19. health 狀態的算法原本兩處各寫一遍 → 收成 `healthFromRecords` 一份純函式。
20. 告警的 `record.field` 是永遠不存在的死分支;狀態卡的 `parentTasksById` fallback 會讓值混進任務清單。
21. **我自己一條假通過的測試**:數 `setTaskHealth(` 的出現次數,但實際呼叫的是包裝函式 `updateHealth`,
    那條斷言在任何實作下都會過。改成同時守「包裝函式存在」與「狀態對應只算一次」。

## 逐條比對(使用者要求,收尾後另做)

對照本文每個批次的「改動」條目逐條查程式碼(不看測試綠不綠,只看正式路徑)。
**共補 18 項**,其中兩項是功能不通:

| 項目 | 原狀 |
|---|---|
| B3-6 / B5-6 重選 | 先前只修了帶 `tabId` 的分支;**任務頁走的是不帶 `tabId` 的新分頁分支**,那條路仍然沒帶 `locator` 與 `preselect` |
| 走查-4 抽屜 | 時間容差下拉沒有 `1440`,多值任務預設的按天分列**開一次抽屜就被靜默改成不合併** |
| C-3 短名 | 樞紐表欄標與折線圖例一律用完整名,規劃的「同一父任務用值名」完全沒接上 |
| B3-1 右鍵選單 | 規劃的欄/列各分「每格一值」與「聚合一值」,委派規格漏抄成單一項 |
| A-4 計量卡 | 完全不看 health,抓不到時照樣畫出一個假讀數 |
| B5-6 逐值回饋 | 立即抓取只回第一個值,其他值怎麼了看不出來 |
| B2-4 lastValues | 逐值整包讀寫,十個值就是十次 |
| B2-5 診斷 | 多值抓取沒留任何診斷紀錄 |
| B2-5 全失敗原因 | 取第一個失敗而非出現最多的 |
| B5-7 差值 | 算在 DOM 渲染迴圈裡,而且缺值時會往前找更早的值(等於內插) |
| B5-2 提示 | 顯示序列 id 而不是值名 |
| B5-4 範本 | 產不出卡片時靜悄悄 |
| B2-7 預檢 | 失敗說明印內部代號 |
| B5-1 側欄 | 子項一律全展開,沒有收合 |
| B4-1 值清單 | 沒有自己的捲動範圍;聚合下拉在全是儲存格時仍顯示 |
| D-5 舊策略 | 保留了設定但畫面沒有任何說明 |
| 其他 | 兩處把任務 id 直接插進 CSS 選擇器(匯入的 id 可能含引號) |

以上 18 項均已修正(0a1ad3b / 1afbc3e / 4621337 / fb1c4f8)。

**明確不做的項目沒有越界**:甲案、每值各自聚合、單值升級多值、圖表 X 軸、
排程失敗立即寫紀錄、既有三處拖曳搬家、20 秒延壽迴圈,逐一 grep 皆無實作。

**兩項維持與規劃不同的做法**:重選的定位資訊改由背景端從任務讀(介面層不必知道細節);
Shift + 方向鍵是四方向自由延伸而不是沿軸限制(功能更強,不減損)。

## 體檢輪修正(換模型:Claude Fable 5.1)

對 `origin/dev..dev` 全部 diff 與併回後四個沒被獨立驗收看過的 commit 各掃一次:

| 哪裡 | 症狀 | 修法 | 迴歸測試 |
|---|---|---|---|
| `picker-mode.js setTarget` | 滑鼠一離開表格就清空全部已選——漂過一段文字,十個值就沒了 | 記住已選值所屬的表格 `pickedTableEl`,只有落到另一張表格才清;確認時以那張表格為準 | `n1_closeout` ×3 |
| `picker.js` 舊策略 | 下拉沒有 `attr` 選項,顯示成「自動」;`buildTask` 把「自動」當成沒動而沿用舊策略,使用者永遠改不回自動 | 編輯時把舊策略補成一個選項讓它顯示得出來,規則收斂成「表單選什麼就是什麼」;舊的 D-5 測試前提跟著改 | `n1_closeout` ×3 |
| `main.js RUN_TASK` | 同一分鐘按兩次立即抓取,逐值回報每個值出現兩次 | 每個值只留 `capturedAt` 最新的一筆 | `n1_closeout` ×1 |
| `main.js` PICKED repick | 重選後選好的值只更新定位,`picks` 整包丟掉——畫面像是改了、資料沒改 | `applyRepick`:沒動的值保留 key 與名稱,新值配新 key;單值任務換規格並沿用聚合 | `n2_closeout_audit` ×2 |
| `picker.js` 開場 | `applyPickerDefaults` 與 `renderDashboardSection` 並行:記住的儀表板在下拉建好前套、還原的卡片型別又被預設蓋掉 | 依序 await;儀表板區塊只在沒有任何勾選時才給預設 | `n2` ×1 |
| `drawer.js collectPatch` | 用來源清單的 DOM 順序重建 `source`,使用者調成跨任務交錯的欄序被洗掉 | 以原本的 `source` 為基準做增刪 | `n2` ×1 |
| `dashboard.js` 側欄 | 收合與搜尋共用 `hidden` 互踩;重畫後回到全展開;把手在父列的 pointer capture 底下點不到 | 收合狀態記在父列,顯示條件只在 `filterPaletteItems` 算一次;重畫前記住收合;把手 `pointerdown` 不冒泡 | `n2` ×2 |
| `fetcher.js` 診斷 | `fetch_fields` 傳物件,設定頁印成 `[object Object]`;每次多值抓取都寫,500 筆環形緩衝會把看門狗擠掉 | 改成一行字串,只在有值失敗時寫 | `n2` ×1 |
| `picker.js` 值清單 | 移除唯一的整欄值後聚合下拉仍顯示、預設卡片型別不重算 | 併進 `updateFieldListState` | `n2` ×1 |
| `picker-mode.js` 巢狀表格 | 只判內層在外層之內,反向不成立:在內層選了值、移到外層就被清空 | 兩張表互不包含才算換表 | `n2` ×1 |
| 死碼與重複 | `withDelta` 的物件取值分支永遠不成立;`SUCCESS_STATUSES` 沒人 import;範本失敗提示貼三份 | 刪、改內部常數、抽 `applyTemplateWithToast` | `n2` ×1 |
| 文件 | CLAUDE.md「health 只在 fetcher 寫」是錯的(precheck / sitecheck 也寫);SPEC 有四處舊輪次寫了但沒實作的行為當成現況;§7 同一規則寫兩次;三處「舊版怎麼錯」的敘事 | 對齊現況、刪重複、敘事留在本檔 | — |

沒修、記進 BACKLOG:`picker-mode.js` 自帶一份 `getRowCells` / `isHeaderRow` / `getTableRows` 與 `shared/table.js` 判準不同(多認 ARIA 與 children 退路),合併要連 ARIA 表格測試一起補,不在體檢輪動。

結果:**1409 綠**(基線 1138,本輪 +271),煙霧測試通過。

## 體檢交接

- **模型**:規劃 Claude Fable 5.1;實作與併回前終檢 Claude Opus 5(委派段落 agy `gemini-3.8-flash-high`);
  併回後的逐條比對 Claude Opus 5;換模型體檢 Claude Fable 5.1(見「體檢輪修正」)。

- **測試**:`npm test` **1409 綠 / 0 紅**(基線 1138,本輪 +271)。真實瀏覽器煙霧測試 `./run_smoke.sh` 全部通過。
- **版本**:`manifest.json` 0.3.0 → 0.4.0;CLAUDE.md 的測試基線已同步為 1409。
- **委派**:整輪 agy(`gemini-3.8-flash-high`),12 段全部一次通過驗收,無需回饋重跑。
- **Claude 自己動手的部分**:測試替身擴充、`series-index` 最小版、X3 修正、A5 卡片顯示、
  五處退回與重構(見執行紀錄)、圖表格線與座標軸、視覺驗收抓到的四個缺陷。
- **範圍收斂**:Picker 的 `ctx` 維持走 URL 參數(原規劃考慮改 `storage.session`)。
  20 個值的 ctx 約 6KB,遠低於限制;改用 session storage 會多出「讀後即清、重新整理就丟失」的失敗模式,不值得。
- **暫定值全部沿用**:多值上限 20、popup 前 3 個值、樞紐表 12×4、預設排程 09:30、
  多值預設開啟差異欄、同名值以 `shortName` 判定。使用者實測後可調。
