...
> 狀態：全案完成，已併 dev 與 master（1558 綠；版本 0.6.0）
> 補完輪：收尾後依規劃逐條比對，發現批次 D 有四項版面沒做到，已補（見文末「補完輪」）
...
- **實作**：agy 委派產出（批次 A 三段、B、C 用 `gemini-3.8-flash-high`；批次 D 在 Gemini 組額度用盡後
  改用 `claude-sonnet-4-6`，起點就是批次 D，之後未再切回）。Claude 負責規劃、寫測試、驗收、修補與文件。
- **體檢**：不是由實作方自驗。開了兩份**獨立**審查各看一次全 diff（一份程式碼、一份文件），
  兩份都不帶本輪對話脈絡，只讀 PLAN、現況文件與 diff。共回報 12 類發現。
- **處置紀律**：每一項先自己寫探測腳本實測，確認機制真的成立才修；有一項（非 task 用途 chip 不累積）
  實測後判定是設計如此，不改行為只補文件。
- **驗證**：全量測試 1541 綠（上輪基線 1464）；真實瀏覽器煙霧測試 Chrome + Edge 全過；
  體檢輪九處守門各做一次突變，全部殺得死對應測試。

## 補完輪（收尾後的規劃比對）

收尾之後依 PLAN 逐條比對實作，批次 A／B／C 全數落實，批次 D 有四項只寫在規劃、沒做到。
原因是批次 D 的規格把版面細節寫在散文裡，而驗收條件只寫了「id 不變」「零色碼」「載入 ui.css」，
**規格寫了、驗收沒寫的條目等於零訊號**——逐段驗收只驗驗收條件列出的東西。

| 規劃項目 | 補完內容 |
|---|---|
| 頂部標題列（任務名稱／目標主機） | 新增 `[data-picker-header]`：`#picker-title`（編輯時顯示任務名稱）與 `#target-host`（主機名，次要色、等寬字、過長截斷） |
| 內容以卡片分節 | 「目標與模式」與「即時預覽」補上分節容器；「固定為預設值」移到動作列上方。內容區不再有裸欄位 |
| 預覽數值卡的狀態色 | `#preview` 測過後帶 `data-state`，成功用 `--ok`、失敗用 `--danger`；每次 render 先清掉 |
| 移除重複的內嵌樣式 | 儲存鈕改掛 `.btn-primary`，刪掉 picker.html 自己那份主色樣式；`ui.css` 移除六個沒人使用的類別 |

補完輪五處守門各做一次突變。其中兩個一開始存活，都是測試寫太鬆：
「還沒測過沒有狀態」在全新 DOM 上本來就成立（改成驗「測過後重新渲染要清掉」）、
死規則偵測只抓行首的類別選擇器（改成掃整份選擇器清單，並先剝掉註解避免把檔名當類別）。
補強後兩者都殺得死。全套 **1555 綠**。

## 終檢輪（換模型體檢，Fable 5.1）

實作與前兩輪體檢由 Opus 5 主持（agy 產出）；本輪切換模型後以 PLAN 基準 `98836d4` 到 HEAD 的完整 diff
重新掃一次，不沿用先前對話的假設。規劃比對：A／B／C 全數落實，D 含補完輪四項也已落實；
「明確不做」六項未混入。抓到的都是同型漏修與重複邏輯，沒有功能性 bug：

| 發現 | 處置 |
|---|---|
| `site.html` 的儲存鈕仍自己寫一份主色樣式（補完輪只修了 picker，同型只修一半） | 改掛 `.btn-primary`、刪重複規則；E-4 測試改成兩個視窗都驗 |
| 「同一個目標頁」判定寫了兩份：`frames.js` 第二層與 `fetcher.js` 的 `sameTarget` | 收成 `frames.js` 匯出的 `sameOriginPath`，fetcher 改用它；新增 E-5 守門（fetcher 不得再碰 `.pathname`） |
| `ui.css` 的 `.btn` 沒人用，死類別測試沒抓到（`\bbtn\b` 把 `btn-primary` 算成有人用） | 刪 `.btn`；偵測改成 class token 精確比對 |
| `ui.css` 兩個連續的 `fieldset {}` 區塊（刪 `.card` 別名後的殘留） | 合併成一個 |
| `picker.html`／`site.html` 檔尾少了換行 | 補回，並加測試 |
| A-6 鏈結測試放在 `p1_nested_cell` 而非慣例的 `m2_chain` | 不搬（本輪沒有新增跨模組欄位；`TEST_TASK` 的鏈由 p2 從 Picker 斷言到 background）|

確認過但沒問題的：dryRun 的 inflight 在 finally 清除、立即測試自己開的分頁會在佇列收尾時關掉、
`role="table"` 包真表格的列數、選取模式所有狀態變數在 `exitPickMode` 重設、
`Tab` 循環對非 task 用途只剩單格、面板與工具列的點擊不會落到 `confirmPick`。
三處守門各做一次突變，全部殺得死。全套 **1557 綠**、煙霧測試 Chrome + Edge 全過。

## 併 master 前的版本盤點

使用者要求併回 master 並確認版本號。查到三件事：

1. **`master` 停在開案那個 commit**（只有 `.gitignore`／`CLAUDE.md`／`README.md`／`docs`，
   連 `src/` 都沒有），dev 領先 244 個 commit、master 領先 0，所以是單純的 fast-forward。
   AF-1～AF-7 全部是這一次才進 master。
2. **版本號確實沒更新**：`src/manifest.json` 停在 `0.4.0`，那是 AF-5 收尾升的（2026-09-06）；
   AF-6 與 AF-7 兩輪都漏升。慣例是每輪 minor +1，因此一次補到 **0.6.0**。
3. **`package.json` 的版本從開案起就沒動過**（`0.1.0`），與 manifest 各漂各的。已對齊 0.6.0。

兩處版本號沒有任何地方比對過，才會各自漂好幾輪。已加 `a4_conventions` 的 D3b 守門
（突變成不一致會紅），並把「每輪收尾 minor +1、兩處同步」寫進 CLAUDE.md 慣例。
