...
| 右鍵選單的「整欄/整列」再分「每格一值」與「整欄一值」 | 目前只有整欄聚合那一種;使用者要一次挑一整欄的每一格時 |
| 樞紐表差值的計算搬進 `series.js` 純函式層 | 目前算在 `cards.js` 的 DOM 層;要在別處重用或加統計時 |
| 折線超過 8 條的拒絕邏輯收成一份 | 目前 `drop-rules` 與 `dashboard` 各有一份(前者判定、後者提示);再多一個呼叫端時 |
