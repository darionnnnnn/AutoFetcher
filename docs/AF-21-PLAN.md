...
> 狀態：全案完成，已換模型體檢並併 dev（2973 綠、Chrome＋Edge 煙霧全過，v0.20.0）
...
## 定案逐條比對（2026-09-19，Claude 親做）

批次 1～9 的「定案」逐條到程式與文件比對（腳本 103 條字串核對＋7 條細項補查，行為面由各段的驗收測試與突變負責）：**全部已實作，沒有漏做**。
與原定案字面不同、實作時改過的地方（原因都已在執行紀錄或本節說明）：

| 批次-定案 | 原定案 | 實際 | 理由 |
|---|---|---|---|
| 2-2 | runTask 依時鐘判斷遲到 | 呼叫端算 `isLateStart` 傳 `markLate` | runTask 自己看時鐘會讓 32 則直接呼叫它的測試與路徑被時鐘影響 |
| 2-4 | 續跑門檻看「slot 距今 ≤10 分鐘」 | 看登記時刻 `at`；同 worker 卡住看 `runningAt` | 重試項目的 slot 本來就早於 10 分鐘，看 slot 會把正常重試判成中斷；排隊久的項目看 `at` 會被誤判卡住 |
| 3-2 | 紀錄匯入 `status` 必須是已知列舉 | 沒有 `status` 欄位的照收，帶了未知的才拒絕 | 既有 `za2` 去重測試用的是不帶 status 的紀錄 |
| 3-2 | 外來設定檔一律丟密碼、列入要重輸 | 本機已有該站台密碼時沿用、不列入 | 同一台機器再匯入會把能用的密碼洗掉 |
| 4-2 | 名稱保底「〈主機名〉的值 N」 | 「〈主機名〉的值」，不編號 | 規格階段簡化（同名不影響存檔，任務頁可就地改名） |
| 6-3 | 去抖 300ms | 維持 50ms | `za2`、`l1` 以 60ms 斷言通知；收益主要來自「沒交集不動」與輕量重畫 |
| 7-4 | shadow DOM 說「封閉元件」 | 說「網頁元件」 | 一般使用者不懂「封閉」；只抓得到 open shadow root（closed 的記 BACKLOG） |
| 9-5 | BACKLOG 劃掉「排程觸發失敗立即寫紀錄」 | 保留 | 本輪只做了 `interrupted`，排程失敗仍走重試策略，那一條沒做完 |
| 9-5 | BACKLOG 新增「保留天數永久時舊日鍵永遠留著」 | 不加 | 前提不存在：保留天數範圍 1～3650，沒有永久 |

規格外另做（終檢或實作時發現）：排程健康的保護次數（規劃把程式工作寫在文件批次，沒派進任何一段，Claude 補做）、儀表板刪除改對話框、站台 `updateSite` 加鎖、repick 核對分頁（含取消）、通知建立失敗撤回冷卻、forbidden 診斷節流、匯出退路 17 個自我引用 token、900px 以下卡片高度、`showModal` 置中。

## 體檢輪修正

> 實作：Claude Opus 5（協調）＋ impl-low（Opus low）。體檢：Claude Fable 5.1；掃描與修正 subagent 一律 Opus low，掃描端附 jsdom／node 探針實測、每條標「已實測重現／推論」。
> 規劃比對由體檢方親做；五個掃描代理分區找「沒被查過的狀態組合」，回報 44 條，採納 36 條修正、其餘記 BACKLOG 或判定接受。

**規劃比對抓到的缺口（體檢方親做）**
- 探針結果一節承諾「煙霧測試加一條 worker 重啟後佇列項目被續跑」，實作輪沒做 → 補進 `tests/smoke/load.mjs`（寫入別的 boot 的 runState、觸發看門狗 alarm、輪詢到紀錄＋帳本＋runState 清空＋看門狗 alarm 補回），Chrome 與 Edge 都過。補的當下抓到自己的競態（紀錄寫入後 runTask 還有收尾才移除登記），改成等清空。
- 批次 9 定案 2「SPEC 瘦身」只搬了探針事實表。體檢方重讀 §2／§4／§7 後**縮小範圍**：那些「以前……」句子多半是「不這樣做會怎樣」的失效模式，搬走等於拿掉護欄；只把純歷史的「誰推翻誰」註記（7 處）移到 `docs/archive/SPEC-decisions.md`，失效模式原地保留。規劃階段「§2 有 37% 是決策史」的判斷來自掃描摘要、沒有親讀，高估了。
- 探針表搬家後 `tests/z4_fetch_tab.test.js` 註解與 CLAUDE.md 仍指向 SPEC §4 → 已改指 archive。

**排程與資料（探針 A、B）**
- 新建的 daily 任務第一次 `refreshMissed` 被回溯成 7 天錯過（`computeMissedSlots` 沒有 `createdAt` 守衛，gap 那份有）→ 補守衛；4 個用假時間軸的既有測試補 `createdAt`。
- 離線分支的重試沒有 `attempt < 3` 上限，alarm 每 10 分鐘自我延續、把重試額度用光 → 加上限，用盡寫一筆「目前離線」的失敗紀錄。
- 錯過清單只加不減：遲到的 alarm 真的跑完後那一格仍留著；任務刪掉後項目與黃燈清不掉 → 合併時丟掉「帳本已有」與「任務不存在」的項目；`deleteTasks` 收尾呼叫 `pruneOrphanEntries`（一併清 `missed`／`alertLog`／`notifyLog`／`health`，不等一天一次）。
- gap 的 slot 每輪延伸，畫面拿舊 slot 送 `SKIP_ONE` 比不到而靜默無效 → gap 只比任務、UI 帶 `kind:'gap'`，移除 0 筆回 `ok:false`。
- 恢復正常的任務仍出現在下一則同站台合併通知 → `clearNotifyLog` 連帶清 `failMerge`。
- 續跑前若同格已有重試 alarm 就不續跑（避免兩條重試鏈）。
- 設定匯入同 id 任務整筆取代，匯入檔沒帶的 `enabled`／`foreground`／`suggestForeground`／`notFoundStreak`／`createdAt` 被洗掉（停用的任務復活）→ 沒帶就沿用本機。
- 密語錯誤顯示 WebCrypto 英文原文 → 中文訊息。取鎖逾時要先等 diag 寫完才照做（diag 鎖也塞住時等待加倍）→ 診斷不 await（`za1` 對應改成稍等再查）。
- `updateTasks` 的 mutator 改 id 會多出一筆任務 → 丟例外。`getRecordsInRange` 日期格式不合會退去掃整庫 → 回空。`deleteRecord` 同一筆同時在舊鍵與小時鍵只刪一筆 → 全刪。`pickerDefaults` 壞形狀寫得進去 → 拒絕。
- 擷取短等待每次 DOM 變動都跑一次解析（anchor 層是全文件掃描，會自己重繪的監控頁 3 秒內上百次）→ 合併成最多每 100 毫秒一次；已知壞掉的任務（`notFoundStreak ≥ 1`）不再每次多等 3 秒。

**面板與選取模式（探針 C、E）**
- 面板重畫簽章沒含批次的 `items`：第二輪批次選取被當成沒變、「全部儲存」存的是舊目標 → 納入；連帶讓「全部試抓／全部儲存」進行中的重畫延後到結束再補。
- 守門原因、`aria-invalid`、「還差 N 項」在換目標、單任務↔整批改排程、批次↔單任務之間殘留（名稱已保底填好卻還寫「名稱不可空白」）→ 每個畫面切換入口清掉。與 AF-7／9／17 的「清單清了、旁邊那份沒清」同型。
- 前置動作「已設定 N 步」計數殘留到批次畫面（清單已清空）→ 同步更新。
- popup「知道了」不看回覆也不接例外（背景沒存成，畫面照樣變已知悉）→ 比照 gap 列處理。
- 站台測試登入進行中切分頁，A 站的結果畫在 B 站表單上並說「可以用」→ 以輪次丟棄過期結果。
- 診斷包匯出失敗靜默 → 顯示原因。iframe 被移除後代理層仍算有效 → 改判它代表的 iframe。中鍵自動捲動與側鍵 `auxclick` 沒擋 → 補。`staleNotice` 滑鼠一動就消失 → 加選一格才清。

**Report（探針 D）**
- 多序列折線圖抽樣後各序列保留不同時間戳，而 X 軸是「首見順序」→ 座標亂跳、線段回頭（兩條 1500 點實測回頭 63 次）→ X 軸依時間排序（非時間標籤維持原樣）。
- 抽屜「套用」寫入失敗：抽屜已關、草稿丟掉、畫面留著沒存的值、零提示 → 先寫入成功才關，失敗留著並說原因。
- 編輯模式切儀表板，undo 堆疊沒清，`Ctrl+Z` 改到沒在看的儀表板 → 切換時清空。
- 卡片與頁籤拖曳沒接 `pointercancel`，`isDashboardBusy()` 永遠為真、之後的重畫全被延後 → 補收尾與 flush。
- 匯入確認框開著時再選壞檔，對話框節點被覆寫掉但 `modal.js` 仍認為開著，抽屜的 `Esc`／點外關閉從此失效 → 寫結果區前先收掉；`modal.js` 發現對話框已不在文件上就當取消。
- 抽屜開著切到別的分頁仍算「開著」→ 離開儀表板前先請它關閉（選繼續編輯就不切頁）。手動抓取結果的 Map 不隨任務刪除清 → 補。
- 設定頁保護次數每次讀 7 天紀錄只為數 `interrupted` → 背景寫紀錄時同時寫一筆診斷，全部從 diag 數。

**架構契合**：移除沒人用的 `plus` 圖示。新模組的匯出逐一比對使用點，其餘沒有只剩測試在用的新抽象。

**判定接受、記 BACKLOG**：原地插列的重繪不算選取失效；工具列折行時可能與面板重疊；擷取短等待的 3 秒沒計入總時限；站台面板判斷不出分頁時儲存鈕仍用原生 disabled；closed shadow root。

測試：**2973 綠 / 0 紅**（體檢前 2914，+59：`zk1` 23、`zk2` 22、`zk3` 14）；每條修正各做一次突變皆紅；煙霧 Chrome、Edge 全過。

## 終檢輪

併 dev 後全量測試與煙霧重跑；體檢修正 commit 逐檔 `node --check`、BOM／CR／NUL 掃描。結果見 commit 紀錄；終檢無新發現。

## 體檢交接

- **實作模型**:Claude Opus 5（協調、驗收、文件）＋ impl-low（Opus low）實作；**體檢模型**:Claude Fable 5.1。
- **全量測試（實作輪收官）**:`npm test` **2914 綠 / 0 紅**(基線 2529,+385)。本輪曾出現一次無法重現的單則失敗(7-B 收尾,之後連跑多次全綠),已記 BACKLOG。
