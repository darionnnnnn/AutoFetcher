...
- 按下前兩項**不會直接開設定視窗**,而是讓目標頁進入**選取模式**(`content/picker-mode.js`):
  ...
  - `Ctrl`/`⌘` + `A` 全選這張表的資料格(列標題那一格不算,受 `maxPicks` 截斷並提示);
    `Ctrl`/`⌘` + `Z` 先還原快照,沒有快照才「移除最後一項」(與 `Backspace` 同一條:清單空的時候不攔);
...
- **設定畫面是側邊面板(`chrome.sidePanel`),不是彈出視窗**(AF-10 推翻先前的 `windows.create`):
  ...
  - **參數一律走 `storage.session` 的 `panel:<tabId>`**,`setOptions.path` **不得帶查詢字串**:
    ...
    形狀是 `{ kind: 'waiting'|'new'|'edit'|'site'|'saved'|'batch', ctx?, taskId?, origin?, draft?, retarget?, batch?, items?, text?, notice? }`
...
- **面板的畫面狀態**:**等待態**(`#panel-waiting`,右鍵剛開、還在頁面上選)——
  ...
  **已儲存**(`saved`,回饋區);**多任務清單**(`batch`,見〈多任務的面板〉)。
