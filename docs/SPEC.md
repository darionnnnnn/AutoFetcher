...
| `bar` | 長條(每日聚合) | 同上 |
| `table` | 最近 N 筆(`mode: recent`)或樞紐表(`mode: pivot`,列=時間、欄=任務) | `limit`(兩種模式都吃)、`rowHeader`(樞紐表第一欄標頭,預設「時間」)、`bucketMinutes`(時間容差,見下) |
...
- 每筆紀錄的「有效時刻」= `slot`,沒有 slot 就用 `capturedAt` 截到分鐘(補抓與手動觸發的紀錄因此也進得了樞紐表)。
...
- `limit` 對樞紐表是「保留最新 N 列」,顯示順序仍由舊到新。
...
9. **資料來源側欄與拖曳投放**(編輯模式才顯示,`#source-palette`):列出啟用中的任務,可搜尋,每項都是拖曳來源。
   ...
   | 目標 | 行為 |
   |---|---|
   | `table` | 依放開的 X 位置插入到對應欄之前(表頭中線判定,列軸那一欄不算);已存在則搬移 |
   | `line` / `bar` | 追加並去重;超過 8 條(`--chart-1~8`)拒絕並在 `#dnd-toast` 說明原因 |
   | `number` / `gauge` | 取代唯一來源;標題等於原任務名稱時跟著換,使用者自訂的標題不動 |
   | `status` | 加入任務篩選清單,去重 |
   | `text` | 不接受 |
   | 空白格線 | 建新卡:`text` 模式的任務建 `table`,其餘建 `number`;位置由 `layout-store.addCard` 找空位 |
   | 指標壓在不肯收的卡片上 | 什麼都不做,**不可在它底下偷偷長出新卡片** |

10. **拖出移除**:編輯模式下,表格的欄標與折線/長條的圖例各有一個移除把手(`data-remove-source`),
    拖到來源卡片矩形之外放開即移除該來源(放在別張卡片上也算);`number`/`gauge` 沒有把手(至少留一個來源)。
    表格移除到零欄時**卡片保留**並顯示「拖進來」的空狀態。
