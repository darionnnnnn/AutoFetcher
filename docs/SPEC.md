...
- **AF-22 群組建立流程**(`af-pick-batch`):右鍵開右側 Picker 後先按「新增第一個群組」，輸入群組名稱或按「從頁面取名稱」，再確認名稱並開始選值。頁面取名只讀取使用者點到的文字或可見表單值回填名稱，不加入值；空白結果不覆蓋名稱。每組可從不同表格、獨立元素及可存取 frame 選值；群組不按表格自動合併，群組名稱可重複。可以新增、切換、改名、移除群組或值、復原，再按「完成選取」進共用設定；「返回選取」回到草稿。只有一組也照此流程保存為 multi 任務。
