...
- 按下前兩項**不會直接開設定視窗**,而是讓目標頁進入**選取模式**(`content/picker-mode.js`):
  ...
  - **滑鼠語意照一般電腦操作習慣**(試算表與檔案總管),`task` 與 `repick` 適用:
    ...
    **雙擊是結果式**:瀏覽器的順序是 click、click、dblclick,前兩下會加再取消;雙擊結束時那一格一定在清單裡,然後送出。
  ...
  - **面板永遠以一句「現在該做什麼」收尾**(`instructionLine`,唯一一份,固定在文字區最後一行、
    ...
    已選 N → 「已選 N 個值，再點其他格可加選、點已選的可取消，好了按完成（或雙擊、Enter）」;
    批次模式沒有組 → 「點你要抓的內容；不同的表格或元素會各自成為一個任務」,有組 → 「已選 N 個任務、共 M 個值，好了按完成（或雙擊、Enter）」;
  ...
  - **送出前檢查選取仍在文件上**(AF-21):完成、雙擊、`Enter` 三條路徑共用一個檢查;目標或已選所屬的表 `isConnected === false`(SPA 重繪)時不送出,面板說「頁面剛剛更新過，請重新點選」並清掉失效的選取(連同索引)。`describe` 對沒連到文件根的節點不產生 `path`／`xpath`。
  ...
  - **面板底部固定「完成」與「取消」兩個鈕**(`data-af-done` / `data-af-cancel`):
    ...
    面板的一行提示**跟著狀態換**:沒選時教怎麼開始(點一格/`Shift` 拉範圍/點表頭選整欄),
    選了之後教怎麼送出(雙擊或 `Enter`/再點取消/`Backspace` 移除最後一項)。
  ...
  - **一個任務只抓一個元素,但那個元素裡可以挑多個值**(§7):
    ...
    同一頁要抓四張**不同的表格**,才是四個任務(各自命名)——用右鍵「一次建立多個任務」一次選好、一次存(見下一條)。
  - **一次建立多個任務**(AF-18,右鍵 `af-pick-batch`;選取模式的旗標是 `ENTER_PICK`／`DESCEND_FRAME` 的 `batch: true`,`purpose` 仍是 `'task'`):
    **是不是多任務在入口就決定**,選取中不提供切換。資料模型不變——建出來的就是 N 個一般任務。
    - **組**＝一個定位目標(一張表,或一個非表格元素)＝一個任務。點不相干的另一張表的格子＝切到那張表的組(沒有就開新組,**不取代、不要求確認**);
      回頭點先前那張表＝回到那一組。表格內的所有既有操作都作用在目前這組。**能升到外層的先升**(監控頁別列的小表併進同一組,不是六個任務);
      升上去之後若外層本來就有一組,**合併成一組**(同一張表只能是一組,否則會建出兩個定位相同的任務)。
      **切過別的組之後也一樣**:鎖表的 anchor 是「目前這組」的表,換組後觸發 2 不會自己發生,所以換組(`batchFollowTarget`)時先找「同一張外層表的親屬組」——
      點的是某組外層表底下的小表、或某組小表的外層／兄弟小表,就回到那一組並升到外層;那一組換算不了才各自成組。
      所有加值路徑(點擊、雙擊、拖曳、右鍵加值、`Ctrl+A`、**`Shift`＋方向鍵**)加值前都過同一套守門:升到外層 → 批次換組 → 目標不是已選那張表就拒絕。
    - 非表格元素:點一下＝加成一組,再點＝移除那一組;**雙擊是結果式**(click、click 會加再移除,雙擊結束時它一定是一組);
      點在頁面空白處(`body`)不加,說「點在頁面空白處了：把滑鼠移到要抓的數字或表格上再點」。整欄／整列模式點非表格照舊拒絕。
    - 值被移光的組自動消失;**組數上限 20**,第 21 組說「一次最多建立 20 個任務；要再加請先完成這一批」。
      `selectedCount()` 在批次模式是所有組的值總數(元素組算 1)。
    - 已選標記與送出後的 `held` 藍框涵蓋**每一組**;面板清單依組分段(`data-af-group`,標題「任務 N：名稱提示（M 個值）」,整組移除鈕 `data-af-group-remove`)。
    - 復原快照涵蓋**所有組與目前是哪一組**;整組移除是移除類動作。
    - 一批只能在同一個框架:已有組時確認的目標是 iframe(代理層)→ 不下鑽,說「進入框架會離開這一頁的選取；請先完成這一批，再對框架內的內容另開一批」。
    - 元素組的 `nameHint`(AF-19,`elementNameHint`:文字收空白取前 20 字,與面板組標題同源)**只在真的送出 `batch` 陣列時補**;恰好 1 組走單任務訊息,不帶。
    - 送出:payload 的組裝只有 `buildPickPayload` 一份;**組數 ≥2** → `PICKED{ purpose, batch: [payload…] }`(頂層不放單任務欄位);
      **恰好 1 組** → 與非批次模式選同樣東西的訊息逐欄相同;0 組照單任務流程。
    - background 把 `batch` 每項補上 `tabId`／`url`／框架身分與穩定鍵 `key`(`b1`…),寫成面板 `kind:'batch'`(見〈面板的畫面狀態〉)。
...
- **多任務的面板**(AF-18,`kind:'batch'`):顯示 `#batch-section`(每項一列 `[data-batch-item][data-batch-key]`:名稱輸入框、`describeTarget` 的「抓什麼」、
  ...
  - 已知取捨:卡片型別整批共用(不像單任務依值的數量換預設);逐任務進階設定不提供(存完到任務頁編輯,BACKLOG)。
...
- **手動抓取(`reason: 'manual'`,來自任務頁與 popup 的「立即抓取」)**與排程觸發走同一個 `runTask`,但四點不同:
  ...
  任務頁與 popup 逐值就地顯示。
- **錯過清單**:`onStartup` 與看門狗每輪都呼叫 `refreshMissed`(AF-21;以前只在啟動時算,闔上筆電再打開不會列出)。只算 slot 早於「現在 − 20 分鐘」的格子(`MISSED_SETTLE_MS`,剛到點還在跑或重試中的不誤報),`lastSeenAt` 也只推進到那裡。`catchUpAll`／`catchUpOne` 略過 `gap`;gap 的白話只在 `shared/describe.js` 的 `gapTextOf`。任務帶 `createdAt`(`saveTasks` 對新 id 寫入、更新時保留),gap 不算建立之前的格子。
- **失敗通知**(AF-21):`background/notify.js` 的 `notifyFailure(key, status, options)`——同一 key、同一狀態 24 小時內只跳一次(帳本 `notifyLog`),恢復正常時 `clearNotifyLog`;用在找不到元素(key＝任務 id)、預檢(`<id>:precheck`)、站台檢查(`site:<origin>`)。同一站台 5 分鐘內的失敗合成一則(`notifySiteFailure`,通知 id `fail:<origin>`,累計放 `session.failMerge`)。燈號不受冷卻影響。
...
- **content 端**:訊息路由在分派處統一 try/catch,例外回 `{ ok:false, error:'content_exception', detail }`(紀錄寫「頁面上的程式發生錯誤：…」);擷取找不到目標時用與 `waitFor` 同一份觀察實作等最多 3 秒再判 `not_found`(延遲渲染的頁面不必等 2 分鐘的重試)。
...
| 風險 | 現象 | 對策 |
|---|---|---|
| service worker 被殺 | 閒置 30 秒或執行 5 分鐘就被回收,抓到一半消失;同站台記憶體佇列裡排隊的任務一起蒸發 | **`session.runState`**(AF-21,取代 `inflight`):到點進佇列前登記 `{ '<taskId>@<slot>': { state:'queued'|'running', at, boot, attempt, reason } }`,結束(含例外)才移除;手動與試抓不登記。worker 每次啟動(模組頂層)與看門狗呼叫 `recoverRunState`:只處理 `boot` 不是自己的項目,**先在鎖內拿走再處理**(兩個復原同時跑時同一格只處理一次);`at` 在 10 分鐘內 → 續跑;超過 → 寫 `interrupted` 紀錄(**不寫帳本**,紅燈),daily 另進錯過清單可補抓。啟動時同時清一次孤兒抓取分頁。純等待每 20 秒續命一次(§4) |
...
- 設定匯出/匯入(換機):
  ...
  - 匯入**先驗後寫**(AF-21):`previewSettingsImport` 零寫入地解析、解密、驗證並回摘要(新增／覆寫／略過與原因／要重輸密碼的站台／被拒的設定與原因),設定頁以對話框顯示,使用者確認後 `applySettingsImport` 才寫;任一步失敗把已寫的鍵還原成快照。同 `taskId` 覆蓋、新 id 新增;成功後重建 alarms、重畫設定頁並套用主題。
...
- **刪除保護**:共用對話框(`ui/modal.js`,真正的 modal:焦點在安全的那顆、`Esc` 取消、關閉後焦點回觸發元素;危險確認用 `btn-danger`,不是主要按鈕)顯示「將一併刪除 N 筆紀錄」,並提供「先匯出再刪除」(先下載成功才刪)。紀錄刪除、站台刪除、設定匯入確認、保留天數調低都用同一個。
...
- **即時生效＋就地回饋**(AF-21):每一欄寫入成功顯示「已儲存」(`role="status"`);數值欄空白或超出範圍不寫入、欄位下說明原因、離開焦點回到上一個有效值;**調低保留天數先用對話框確認**「將會刪除 N 天以前的紀錄（約 M 筆）」。「排程健康」另列近 7 天的被瀏覽器中斷、取鎖逾時、擋下網頁訊息、背景錯誤次數(`countGuardEvents`)。
...
- **卡片設定抽屜是草稿**(AF-21):變更只改草稿並預覽在那張卡上,底部「套用」才一次 `updateCard`、「取消」丟掉;有變更時關閉(✕、`Esc`、點外面、開另一張卡)問「套用／捨棄／繼續編輯」(取消與 `Esc`＝繼續編輯)。抽屜開著時投放到同一張卡、或從同一張卡拖出移除把手,都併進草稿不直接寫。
...
- **下一步**(AF-21):每個異常列有「知道了」,清單上方有「全部知道了」;紅項知道了之後顯示「已知悉・尚未修復」與「停用這個任務」。依狀態給下一步(重選目標 → `report.html#view=tasks&task=<id>`、前往登入頁);站台紅燈畫成一列(前往登入頁＋「在登入頁按右鍵 → AutoFetcher → 設定此站台登入」提示,side panel 不能從 popup 代開)。零任務時三步引導;頁尾有「使用教學」(推翻 AF-18 的「教學入口只放右鍵」)、開啟報表、全部暫停。同一畫面只有一顆主要按鈕(「在這個頁面選取」)。
