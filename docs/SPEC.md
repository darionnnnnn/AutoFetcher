...
- 按下前兩項**不會直接開設定視窗**,而是讓目標頁進入**選取模式**(`content/picker-mode.js`):
  ...
  - **表尾合計預設排除**(AF-16):**建立**整欄值的當下,資料列中位於這張表 `<tfoot>` 的列自動進 `exclude`,
    面板提示「已自動排除表尾 N 列（合計），右鍵可取消」——**只在那個整欄值真的加進清單時說**(`candidateAt` 只記下待提示數,由 `addPick` 加入成功才說;`Ctrl` 點取消已選的整欄、候選值重複時都不說)。判定只有 `withFooterExclude` 一份,**六個建立入口都呼叫它**:
    `candidateAt` 的點表頭與整欄模式點格、右鍵「整欄聚合」、單格升級成整欄(`upgradeLastPickTo`)、沒選值直接送出的兩處。
    **`applyPreselect` 帶回來的值不呼叫**(使用者取消過的排除不能復活)。整列沒有表尾;ARIA 與 CSS 假表格沒有 `tfoot` 語意,不自動排除。
    不在擷取端偷偷跳過 tfoot:使用者看不到、`count` 聚合會少一筆卻沒人說。
    頭尾**空白格**的自動略過(AF-17)是擷取端的 `skip.blank`,與表尾排除互不取代:空白格本來就不進聚合、`count` 也不算它,數值零變動,而且預覽會說出空白幾格、明細表逐格看得到、Picker 有開關(見 §7 略過段)。
  - **排除的標示**:被排除的格子帶 `data-af-excluded`、`COLORS.warn` 虛線外框,不帶 `data-af-picked`;
    每次滑鼠移動隨已選標記重貼(`applyPickedMarks`),清標記時一併清掉。
  - **preselect 回勾排除項**(`relocateExcludes`):逐項以 `locateByHeader` 在另一軸定位(與擷取端同一份),
    搬家跟著標題走、找不到就略過那一項,兩種都亮「位置已變」。
  - **每格各一個值的去頭去尾**:右鍵「每格各一個值」之後,面板動作列出現「去掉第一格」「去掉最後一格」(`data-af-trim-head`／`data-af-trim-tail`,
    動作列建一次、只切 `hidden`)。可連按,每次按之前存復原快照,`Ctrl+Z` 反悔一步。
    **任何其他改變清單的動作**(加選、Ctrl 點、移除、取代、升級、復原)都讓兩顆鈕收起來(`trimReady`,`exitPickMode` 重設)。
    清單只剩 1 個時兩顆 `aria-disabled`,點了不改清單並說「只剩一個值了，不能再去掉」。
    給的是「整欄每格各一個值,第一列是欄位名稱、最後一列是合計」:以前去頭要捲回去 `Ctrl` 點第一格。
    **選取狀態只存索引與表頭字串,不存元素參照**——即時報價的表格會整個重畫,
    存參照會讓標記留在被丟掉的節點上;每次滑鼠移動重貼一次標記。
  - **右鍵選單**(攔截頁面原生選單):表格內是
    「選這一格 / 這一欄:每格各一個值 / 這一欄:整欄聚合成一個值 /
    這一列:每格各一個值 / 這一列:整列聚合成一個值 /〔排除項〕/ 完成 / 取消」——
    ...
    **〔排除項〕**(AF-16):滑鼠下那一格被某個已選的整欄／整列值涵蓋時才出現——整欄值涵蓋同一欄的每一列、整列值涵蓋同一列的每一欄;
    被好幾個值涵蓋時取**清單裡最後一個**(`blockCoversCell`;選單的涵蓋判定與「這一格是否已排除」的 `isExcludedCell` 都經它,標示則直接照排除清單貼)。
  ...
  - **`↑`／`↓` 選定的表也要鎖**(AF-15,`deliberateTableEl`):以前按 `↑` 從內層小表切到外層表,
    滑鼠一動就被升回內層小表(每次 `mousemove` 都重跑升級),「按 ↑ 再點那一格」在第一次點之前就走不到。
    鎖的判定與「已選值的鎖」**合成一份**(`upgradeTarget` 的 `anchor`:已選時是 `pickedTableEl`,否則是 `deliberateTableEl`):
    滑鼠在它的內外層之間移動不換目標、移到非表格區域不換目標;**移到不相干的另一張表才解除**;
    `↑`／`↓` 落在非表格時清掉(離開表格是明確意圖);`exitPickMode` 重設。`backStack` 在目標沒變時不清,`↓` 走得回去。
  - **複合格的子單位**(AF-15):外層格是「複合格」時,滑鼠下的子元素可以單獨選,pick 帶**格內子路徑** `inner`(§7)。
    複合格＝格內含表格(`cellWrapsTable`),或格內(不含外層格自己)「自己擁有非空白文字節點」的元素兩個以上。
    **只有一個包裝元素的格子(`<td><span>31.5</span></td>`)不是複合格**,維持整格、不長出 `inner`——
    否則網站改個包裝標籤就 `not_found`,使用者從來沒要求過子單位卻換上一條更脆弱的定位。
    子單位是誰:格內含表格 → 滑鼠下最內層的格子(停在外層格自己或小表的縫隙上＝沒有子單位);
    否則 → 從滑鼠元素往上第一個「自己擁有文字」的元素(停在沒有自己文字的包裝元素上＝沒有子單位)。
    判定只有 `subUnitOf` 一份。
    - 標示:單格只框子單位;整欄／整列對每一列解析同一條路徑,**解析得到才框**;已選標記與送出後的 `held` 藍框都落在子單位上。
    - 點一下、`Ctrl` 點、雙擊、`Enter` 快速路徑(記住 hover 的元素 `currentHoverEl`,不用外層格算候選)、
      單格升級成整欄整列都帶當下的 `inner`;`Shift` 範圍、拖曳框選、右鍵「每格各一個值」、`Shift`＋方向鍵以**錨點那一格**的 `inner`
      套到範圍內每一格,**解析不到的格不建 pick**;**`Ctrl+A` 不帶**(全選整格)。
    - `samePick` 比 `inner`:同一格的「整格」與「格內第 2 格」是兩個值。pick 只存路徑,不存元素參照。
    - preselect 回勾:列欄定位照舊,再解析路徑;解析不到就略過那一項並亮「位置已變」。
    - chip 名稱接 ` · 小表第 1 列第 2 格`(`innerLabel`,§7)。
  - **巢狀的三句提示**(面板仍只有一句指令句):目標是另一張表格子裡的小表、還沒選 →
    「這是外層表格裡的小表：要抓外層每一列的這個位置，按 ↑ 切到外層表再點那一格」;
    外層表為目標、滑鼠停在內含表格的外層格自己上 → 「這一格內含表格，會抓到整串文字；要抓裡面某一格，把滑鼠移到那一格上（只會框那一格）」
    (**有子單位時不說**——框的就是那一格,再說會抓到整串文字是假的);
    目標小表只有 1 列時點「整欄」、右鍵「整欄聚合」或「每格各一個值」→ 照做,並提示
    「這張小表只有 1 列，整欄只有 1 格；要跨外層每一列請按 ↑ 切到外層表」(整欄選到 1 格是最先會踩的)。
...
**規格裡有值帶 `inner`(§7)時,表格摘要多一個 `innerProbe`**(AF-15):每個帶 inner 的值一份
`{ key, inner, rows: [{ resolved, text }] }`(單值的 `key` 是 `''`),儲存格與整欄逐列、整列逐格(`gridStartsOf`)以 `resolveInnerAt` 探測,
`text` 取前 40 字,最多 20 筆;沒有任何帶 inner 的值時**不放這個鍵**。**欄用位置定位時沒有 `index`**(每次擷取才算得出),
該值不探測、改標 `unprobed: 'pos'`、`rows` 為空——逐列報 `resolved:false` 會是反向答案;整列的列不存在或列裡沒有格子同樣標 `unprobed: 'norow'`。
整列帶 `inner` 而取不到任何格子時,擷取訊息是「這一列在目前的頁面上取不到格子」,不會說「0 格都找不到」。「這一列解析不到」與「抓錯欄」在錯誤訊息上長得一樣,沒有它就查不出是哪一種。
...
- `task.mode = 'block'`;`task.spec` 有三種形狀,擇一:
  ...
  - **位置定位**:任一軸可帶 `pos: 'first' | 'last' | 'last-1'`,有 `pos` 就**不看 `index` 與 `header`**,
    ...
    依標題定位隔天就 `not_found`。`last-1` 是因為最後一列常是合計(要**聚合整欄又不含合計**時用 `skip`／`exclude`,見下)。
  ...
  - **格內子路徑 `inner`**(AF-15):`cell.inner` 與 `block.inner` 可帶 `[{ tag, index }, …]`——從外層那一格的直接子元素起算、
    到子單位為止的結構路徑(`tag` 小寫、`index` 是同層同標籤第幾個,從 1 起算,計數與 `selector.js` 的 `getTagIndex` 同一份)。
    給的是監控頁「外層表每一列的某一格各包一張小表」:沒有它只抓得到外層整格串接起來的文字(`42MAX:462`)。
    **缺省、`null`、空陣列都算沒有**(`hasInner`),舊規格的擷取結果一個位元都不變;**形狀不合法算「有」但解析不到**,
    一律 `not_found`,**不得靜默退回整格文字**。
    - 列欄定位(表頭、位置、純數值標題)**完全不變**;定位到 `(r, c)` 之後才沿路徑取子元素的 `textContent` 當 `raw`。
    - 「某一列、網格欄 c、沿路徑對應哪個元素」只有 `shared/table.js` 的 `resolveInnerAt` 一份,**選取端標示與擷取端取值共用**。
      **只認起點就在這一欄的格子**:被左邊格子的 `colspan` 涵蓋的不算——監控頁最後一列是 `colspan=4`、裡面包著 PublicIP 小表,
      套同一條路徑會把 `203.69.51.90` 解析成 203.69 混進整欄聚合。
    - 單格解析不到 → `not_found`,訊息「這一格裡找不到原本的位置（小表第 1 列第 2 格）；目前這一格的文字是：…」(空的就說「目前這一格是空的」;括號內是 `innerLabel`,使用者才對得回畫面)。
    - 整欄／整列:解析不到的格**不進聚合、計入 `skipped`**(`used` 是解析出數字的格數);一格都解析不到 → `not_found`(「N 格都找不到」)。
      **整列是逐「格」不逐「網格欄」**(`gridStartsOf`,每格一次的網格起點):逐欄走的話被 colspan 涵蓋的欄是同一格、會計成解析不到,`skipped` 虛報。
      選取端的整列「每格各一個值」、`Shift`＋方向鍵的最右欄、診斷探測都走同一份。
    - CSS 假表格也適用:有 inner 時列元素與選取端同源(`getDataRows` 為空時用 `cssGridRowsOf`)。
    - **規格裡只存路徑,不存任何顯示字串**:`sameSpec` 是 JSON 全等比對,顯示字串一旦進規格,下次重選算出來不一樣
      就會讓值的 key 重生、歷史序列斷掉(AF-14 刪 `rawHeader` 的同一個理由)。`PICKED` 訊息、面板 ctx、任務一律不帶。
      background 的 `pickSpecOf` 逐欄挑也要挑到 `inner`(非空陣列才抄),Picker 收集表單時的 `applyPosToCell` 與單值整欄組裝都要保留它。
    - **白話標籤只有 `shared/describe.js` 的 `innerLabel` 一份**(純函式):最後一段是 `td`/`th` 且前面有 `tr` →「小表第 r 列第 c 格」;
      否則「內層 <tag>」(index 大於 1 時加序號)。**七個命名與描述入口**(`describeTarget`、Picker 的 `singleCellName`/`defaultPickName`/
      `fieldNameText`/`fieldWhereText`/一鍵命名「用欄標題」、background 的 `defaultFieldName`)都以「既有名稱 · 格內標籤」組字,
      原本會退回「值 N」或空字串的改用標籤本身;選取模式的 chip 也用它。
    - **已知上限**:路徑固定到小表的「第 r 列」,小表有多列時只取那一列;小表內插欄會讓路徑指到別格(`inner` 是結構路徑,不看小表表頭)。
    - **設定檔相容**:匯出格式版本不變(`settings-io` 仍是 1);0.13 以前的擴充功能匯入帶 `inner` 的任務會忽略它、抓整格串接的文字,不另做遷移。
  - **略過頭尾 `skip` 與點選排除 `exclude`**（AF-16）：整欄／整列聚合的 block 可帶
    ...
    - **頭尾空白自動略過 `skip.blank`**（AF-17）：`skip` 物件可帶 `blank: true`，**只認字面 `true`**（`'true'`、`1` 都是關）；`skipOf` 一律回 `{ head, tail, blank }`；`putSkip` 在 `head>0 || tail>0 || blank` 時放鍵，放的物件一律帶 `head`／`tail`，**`blank` 只在開著時才寫**（關著時形狀與 AF-16 一字不差）。設定併在 `skip` 裡，所以重選保留、`stripPos` 剝除、Picker 收集都沿用 `skip` 既有的五個搬運點，不另開鍵。
      - 「空白」＝清單項解析得到（`ok`）且 `raw` 符合 `/^\s*$/`（含 NBSP、全形空白）；格內子路徑解析不到的**不是空白**。`0`、`—`、`N/A`、`-` 都不是空白（是非數字）。
      - **只剝頭尾連續**的空白，中段空白照舊計入 `skipped`。
    - **有 `pos` 就是取那一格**，`skip`／`exclude` 一律不看（`extractCrossCell` 不變）。
    - 擷取順序（`extract.js` 的 `extractBlockFromTable`）：定位那一軸 → 展開另一軸成有序清單（取格方式與既有三條分支相同）→
      **剝頭尾空白**（`blank` 開著時）→ **套 `skip`** → **套 `exclude`** → 剩下的進 `aggregateCells`。
      「頭尾」講的是表的形狀：`blank` 關著時 `skip` 對著完整的表算（AF-16）；**開著時對「去掉頭尾空白後的表」算**（AF-17 推翻一半），使用者要略過的「第一列文字表頭」才會是「開頭 1 列」，不必把空列數進去。
      - 剝完一格都不剩 → `not_found`「這一欄的 N 列都是空白格」（整列「這一列的 N 格都是空白格」，N 是剝之前的格數）。
      - 剝完後略過數不夠 → 既有訊息的括號改說「（這一欄去掉頭尾 B 列空白後只有 N 列）」；**沒有剝掉任何空白時訊息與 AF-16 一字不差**。
      - `skip` 以**清單項**為單位，不看解析得到與否（一列就是一列）；`head + tail` ≥ 清單長度 → `not_found`，
        訊息「略過開頭 H 列、結尾 T 列後沒有剩下的格子（這一欄只有 N 列）」(整列單位是「格」,同一句不混用「筆」)。
      - `exclude` 每項以 `locateByHeader` 在另一軸定位（規則與列欄定位同一套，搬家跟著標題走、不影響值的狀態）。
        被 `skip` 拿掉的列再被 `exclude` 指到只算一次，也不算找不到。
      - **排除項找不到不得靜默**：不排除任何東西，但 `status` 降為 `fallback`、`message`「有 N 個排除項在目前的頁面找不到（合計）」——
        合計列改名之後漏排除，就是默默加兩次。
      - 全被排除 → `not_found`「略過與排除共 K 列後沒有剩下的格子」(K 含 `skip` 移掉的,只寫「排除」會比使用者設的排除清單還大)，與「本來就沒有格子」的訊息分得出來。
    ...
    - **逐格處置明細 `items`**（AF-17）：整欄／整列（非 `pos` 取格、非儲存格）結果**一律**帶 `items: [{ index, header, use, raw?, number? }]`，順序即清單順序。
      ...
      **四條口徑不變式**：`used` 格數＝結果 `used`；`nonnumeric+blank+unresolved`＝`skipped`；`skipHead+skipTail+excluded`＝`excluded`；`trimmed`＝`blank`。
      **清單產生之後的失敗也帶**（略過後沒有剩下、全部空白、全被排除、剩下的全部找不到子路徑、全部非數字）——那正是最需要看表的時候；清單為空的既有失敗不帶。
    ...
    - **設定檔相容**：匯出格式版本不變；0.14 以前的擴充功能匯入帶這兩個鍵的任務會忽略它們、合計照舊被加進去，不另做遷移。0.15 以前匯入帶 `skip.blank` 的任務會忽略它、頭尾空白照舊計成 `skipped`，同樣不遷移。
...
- **欄索引一律是網格索引**(AF-15):資料列含 `colspan` 時,DOM 裡第幾個格子與展開後第幾欄不同。
  選取端以前存 DOM 索引、擷取端用展開後的網格,點 `<tr><td colspan=2>ab</td><td>c1</td></tr>` 的 `c1` 會存成索引 1、抓到 `ab`。
  換算只有 `shared/table.js` 的 `gridIndexOf(row, cell)` / `cellAtGridIndex(row, idx)` 一份,選取、標示、預覽、`Ctrl+A`、`Shift`＋方向鍵、preselect 全部走它。
  **只有 `td`/`th` 讀 `colspan`**;ARIA 格(`aria-colspan`)與 CSS 假表格一律寬度 1——擷取端的 `parseAriaTable`/`parseCssGrid` 本來就不展開,兩端要一致。
  只看這一列自己的格子,**不處理上一列 `rowspan` 佔位**(見 BACKLOG)。
  **舊任務的影響**:有 `colspan` 資料列的表,舊任務存的是 DOM 索引,擷取端一直是照網格取值——那些任務本來就抓錯格,
  修正後重選一次才會存對;沒有 `colspan` 資料列的表零影響。
...
- **格子裡自己包著一張表格時,面板要先說出來**(`這一格內含表格，會抓到整串文字…`):
  ...
  AF-15 之後有了正解:滑鼠移到小表裡那一格就是子單位(帶 `inner`),警語改教這條路,**有子單位時不再顯示**(§2)。
...
- **Picker 的值清單**(`#field-list`,每列 `data-field-row`):挑了 2 個以上的值才出現。
  ...
  **略過開頭／結尾 N 筆**(`#skip-head`／`#skip-tail`,容器 `[data-skip-row]`,AF-16)放在聚合方式旁邊,
  顯示條件與聚合下拉**同一個布林值**;**藏起來時規格不寫 `skip`**——多值是整包展開舊 block 再組的,編輯既有任務時帶進來的舊 `skip` 要先刪,
  否則使用者改用位置定位後,看不見的設定仍在規格裡。標籤單位整欄「列」、整列「格」、混合「筆」;任務層級一份,收集時套到每個 block 值、儲存格值不帶;
  ...
  **重選保留原本的定位方式**(`applyRepick` 的 `keepPos`),而且比對「是不是同一個值」時忽略 `pos`、`block.exclude` 與 `block.skip`
