...
- 按下前兩項**不會直接開設定視窗**,而是讓目標頁進入**選取模式**(`content/picker-mode.js`):
  ...
  - **取消要二段確認**(AF-19):`Esc`、面板「取消」、右鍵選單「取消」三處都經 `requestCancel`。
    用途是 `task`／`repick` 且已選 ≥2(批次模式算所有組的值)時,第一次只記下 `{ at, count }` 並在面板說
    「再按一次 Esc（或再點「取消」）才會取消，會丟掉 N 個已選值」,第二次才 `cancelPick`。
    **不得被連按或長按穿過**:`repeat` 的 `Esc` 一律忽略;第二次距第一次不滿 400 毫秒只當第一次(並重新計時)。
    **清單變了確認就重來**(記下的 `count` 與現在不同＝重新第一次)。`Esc` 以外的按鍵清掉確認;
...
- **設定畫面是側邊面板(`chrome.sidePanel`),不是彈出視窗**(AF-10 推翻先前的 `windows.create`):
  ...
  - **面板已經有表單時再選一次目標＝換目標,不重置**:只換 `locator`/`picks`/`blockInfo`/`preview`/`nameHint`,
    ...
    新格用預設名,對不到的舊值消失;告警綁的值若已不在清單,欄位退回「全部值」。
...
- **整批改排程**(AF-19,`kind:'bulk'`,`{ kind, taskIds }`):任務頁勾選後按「改排程」、或點某一列的排程文字進來。
  ...
  - 預填:排程正規化(`weekdays` 與 `times` 去重排序、`window` 起訖都有才算)後全等→預填並說「相同」;
    ...
    回填只有 `fillSchedule` 一份(`render` 也用它),**每一欄都確定地寫入**(沒有時段＝取消勾選並清空起訖、沒有星期＝每天):
...
- **多值清單的每一列**(`[data-field-row]`)除了序號與名稱,還有
  ...
  **「拆成每個值一個任務」**(AF-19,`#split-tasks`):新建的多值表單(兩個值以上)才顯示,編輯既有任務不顯示。
  ...
  之後的試抓與儲存就是〈多任務的面板〉那一條。值超過一批上限(`shared/messages.js` 的 `MAX_BATCH_TASKS`,20,頁面組數上限也用它)時
...
- **任務的整批 API**(AF-19):`saveTasks(list)`(**先全部驗證、任一筆不合法整批不寫**;例外訊息與單筆一字不差,另掛 `index`;
  一次讀一次寫;`order` 缺省依序補「目前最大 +1」)、`deleteTasks(ids)`(一次掃描刪任務與其紀錄,卡片逐一 `pruneCardsForTask`)、
