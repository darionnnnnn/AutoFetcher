...
- `contextMenus` 建立父項「AutoFetcher」,子項依序:
  「選取要抓的內容」「一次建立多個任務」(AF-18,`af-pick-batch`,見下方〈一次建立多個任務〉)「設定此站台登入」「開啟 AutoFetcher 報表」
  「使用教學」(AF-18,`af-open-help`,見〈使用教學頁〉;設定 `showHelpMenu` 為字面 `false` 才不建)。
  (沒有獨立的「抓取此區塊」——區塊由選取模式自動判定。)
- 「選取要抓的內容」讓目標頁直接進入選取模式；「一次建立多個任務」先開右側 Picker 的群組草稿，再由使用者命名後進入選取模式。兩者都使用 `content/picker-mode.js`:
  - Chrome 沒有 `getTargetElement`;content script 監聽 `contextmenu` 記住最後右鍵的元素,
    進入選取模式時**它就是預選**,立刻高亮。
  - 滑鼠移動改選;`↑` 擴大到父層、`↓` 沿原路縮回(到 `body` 停住);`Esc` 取消。
  - **滑鼠語意照一般電腦操作習慣**(試算表與檔案總管),下表中標註「批次模式」的舊規則只適用 AF-18 相容協定；目前 AF-22 群組模式以本節後述契約為準。單任務與 `repick` 仍採下表規則:
    ...
    | 已選一格後點工具列「整欄→一個值／整欄→每格／整列→一個值」 | 把**最後一項**那一格**取代**成該欄／該欄每格／該列(不論清單有幾個值;面板說「已把『X』換成『Y』整欄，其他已選不變（可復原）」) | 無 |
    | 點資料列的 `th`(列標題) | 選**整列**(`block.axis: 'row'`) | 無 |
    | 點別處 | — | 解除鎖定 |

    點擊與雙擊都會被攔截,不傳給頁面。單任務與 `repick` 的**送出出口**:雙擊、`Enter`、面板的「完成」鈕。
    **單任務與 `repick` 的 `Enter` 快速路徑**:已選清單是空的且滑鼠停在某一格時,`Enter` = 選那一格並送出
    (鍵盤使用者不必先點一下)。
    **拖曳框選放開後 200 毫秒內的 `dblclick` 要吃掉**——瀏覽器在拖曳結束補的 `click`
    會跟下一下湊成雙擊,那不是使用者要送出。
    **換表且已選 ≥2 時要先提示、再點同一格才真的換**(`replaceConfirmPending`,提示「再點一次才會換到這張表（會取代 N 個已選值）」):
    點一下改成加選之後,唯一會一次清光整批的就是換表(AF-18,取代 AF-8 綁在 preselect 上的那條)。
    **單任務與 `repick` 的雙擊是結果式**:瀏覽器的順序是 click、click、dblclick,前兩下會加再取消;雙擊結束時那一格一定在清單裡,然後送出。AF-22 群組模式雙擊只加入一次、不送出；完成用固定按鈕或 `Ctrl`/`⌘`+`Enter`，普通 `Enter` 僅操作焦點控制項。
    `preaction` 與 `login-*` 一次只選一個,**維持點一下就送出**;iframe 代理層點一下即鑽入
    (那是導覽不是選取)。
  - `Ctrl`/`⌘` + `A` 全選這張表的資料格(列標題那一格不算,受 `maxPicks` 截斷);
    **截斷時說出規模**(AF-19,`selectAllNotice`):掃描照數到底,面板說「這張表有 S 格，只選到前 M 格（上限）」,
    清單長度一變就不再說。
    `Ctrl`/`⌘` + `Z` 先還原快照,沒有快照才「移除最後一項」(與 `Backspace`／`Delete` 同一條:清單空的時候不攔);
    **`Ctrl+Z` 自己做的移除不存快照**(存了就是連按兩次互相抵銷)。
  - **取消要二段確認**(AF-19):`Esc`、面板「取消」、右鍵選單「取消」三處都經 `requestCancel`。
    用途是 `task`／`repick` 且已選 ≥2(批次模式算所有組的值)時,第一次只記下 `{ at, count, sig }`(`sig` 是已選清單內容的簽章)並在面板說
    「再按一次 Esc（或再點「取消」）才會取消，會丟掉 N 個已選值」,第二次才 `cancelPick`。
    **不得被連按或長按穿過**:`repeat` 的 `Esc` 一律忽略;第二次距第一次不滿 400 毫秒不算,**但不重設計時**
    (重設的話每 300 毫秒按一次就永遠取消不了)。
    **清單變了確認就重來**:比的是內容簽章不是數量——工具列把最後一格換成整欄、右鍵排除／取消排除都不改數量。
    換表的「再點一次」提示出現時取消的確認作廢(兩句「再一次」不同時掛著)。`Esc` 以外的按鍵清掉確認;
    **右鍵開選單與滑鼠移動不清**(選單裡的「取消」要能當第二步)。
    兩個旗標一起清的地方只經 `clearPendingConfirms`(改變已選集合、換組、進出選取模式);單獨清的只有兩處:
    換表真的取代那一刻清換表旗標、`Esc` 以外的按鍵清取消旗標。已選 0～1 個、前置動作與登入直接取消。
    單任務與 `repick` 送出(完成、雙擊、`Enter`)沒有確認:它不會丟資料。AF-22 群組模式另見下方規則。
  - **一次挑多個值**(`task` 與 `repick` 支援;`repick` 會帶既有的值回來勾,所以也要能多選):
    ...
    **單任務與 `repick` 換表要「點」不要「移」**:滑鼠移到另一張表只是把目標指過去(讓使用者看得到可以改點這張),
    **點下那張表的格子才換表**,而且是取代並存復原快照;`Ctrl` 點另一張表的格子**不加選**
    (兩張表的索引配不到同一個 locator),面板說明「一個任務只能抓同一張表格裡的值」。AF-22 群組模式可將不同表的值放在同一組，見「AF-22 群組建立流程」。
    **鍵盤 `↑` 已選之後照樣離開這張表**(AF-10 C-5)——那是明確意圖,不是滑鼠路過;而且**鎖改認 `↑` 選定的外層表**
    (`deliberateTableEl` 包著 `pickedTableEl` 時它才是鎖),否則滑鼠一動又被拉回已選的小表(AF-18 P1 的根因)。
    值能不能跟著換到外層見〈巢狀表格的已選值升到外層〉;換不過去時值留在原表、面板說原因,接著點外層的格子是換表。
  ...
  - **右鍵選單**(攔截頁面原生選單):表格內是
    ...
    **只在滑鼠下的表就是已選那張表時才比對**(`blockCoversCell` 只比索引,在別張表上按右鍵不得改到這一組的排除);
  ...
  - **滑鼠停在儲存格上就進入該表格的表格模式**(`upgradeTarget`,唯一一份):目標升為那一格
    所屬的**最內層**表格,工具列四段隨即可用、`nameHint` 也算得出來。
    ...
    **巢狀表格預設取內層**(不是外層目標表格的那一格):外層那一格的文字是
    ...
    例外是下一條「升到外層」——值整批換成外層座標之後,鎖跟著換到外層。
  - **`↑`／`↓` 選定的表也要鎖**(AF-15,`deliberateTableEl`):以前按 `↑` 從內層小表切到外層表,
    滑鼠一動就被升回內層小表(每次 `mousemove` 都重跑升級),「按 ↑ 再點那一格」在第一次點之前就走不到。
    鎖的判定與「已選值的鎖」**合成一份**(`upgradeTarget` 的 `anchor`:已選時是 `pickedTableEl`——但 `deliberateTableEl` 包著它時取 `deliberateTableEl`(AF-18)——否則是 `deliberateTableEl`):
    滑鼠在它的內外層之間移動不換目標、移到非表格區域不換目標;**移到不相干的另一張表才解除**;
    `↑`／`↓` 落在非表格時清掉(離開表格是明確意圖);`exitPickMode` 重設。`backStack` 在目標沒變時不清,`↓` 走得回去。
  - **巢狀表格的已選值升到外層**(AF-18,P1／P2／P4 的根因):監控頁那種「外層每一列的某一格各包一張小表」,
    小表的值 `(r, c[, inner])` 可以無損換成外層的 `(R, C)`＋「外層那一格到該元素的完整路徑」(§7 的 `inner` 本來就支援任意深度)。
    **預設目標仍是內層小表**(既有任務零變動);換算只在下列三個時機發生:
    1. **單列小表上要整欄**(目標是單列小表且可升級、用途 `task`):整欄模式點格、工具列把最後一格升級成整欄、右鍵「整欄聚合」與「每格各一個值」、
       工具列「整欄→每格」、沒選值直接送出、`Enter` 快速路徑——一律建立**外層那一欄**的值;整欄模式的 hover 標示也畫外層那一欄。
       提示改成「這張小表只有 1 列，已改選外層表這一欄的同一個位置（N 格）；只要這一格請切回單格」。
    2. **已選在小表 T,點到同一張外層表 O 的別處**(別列的小表或外層自己的格子;滑鼠移過去時目標就指向 O):加值之前先換算,
       換好了才照原語意加值。判定與處理只有 `promoteBeforeAddingInOuter` 一份(點擊、雙擊、拖曳、右鍵加值共用)。
    3. **已選之後按 `↑`**:能換就換並鎖在外層;換不了(見下)照樣到外層、值不動、說原因。
    - **外層表 O**＝從 T 按一次 `↑` 會到的那張 HTML `<table>`(`outerTableOf`)。**可升級**(`isUpgradeableTable`,以 T 為鍵快取、`exitPickMode` 清掉):
      O 的資料列中**至少另有一列**,沿「外層那一格到 T 的路徑」解析到一張與 T **同型**(資料列數相同、第一列格數相同)的表。
      並排的兩張資料表(同欄沒有第二張)、上下疊放不同型的摘要表與資料表都**不可升級**——預設不指外層的原因就是版面表格會得到脆弱的長路徑。
      ARIA／CSS 假表格的巢狀不升級(沒有實測樣本)。
    - **全有或全無**(`promotePicksToOuter`):單格值一律可換;整欄值只有 T 是單列時可換(換成 O 的整欄＋路徑,照 `withFooterExclude` 做一次表尾排除,原 `exclude` 丟棄);
      其他(多列小表的整欄、任何整列值、元素解析不到)換不了——**整批不換、狀態不動**,面板說「已選的值裡有小表的整欄或整列，換不到外層表；要抓外層每一列請先移除它」。
    - 換算存復原快照(連同 T),**這份快照活過觸發它的那一次加選**:升級＋加選之後 `Ctrl+Z` 一步回到升級前。
    - **`repick` 不升級**(值的 key 會重生、歷史序列斷掉):`↑` 照樣到外層但值不動,`Ctrl` 點外層範圍被擋,都說「重選既有任務時不能換到外層表（歷史紀錄會接不上）；要抓外層整欄請建立新任務」。
    - 不可升級時 `↑`:照樣到外層、值不動,面板說「外層不是每一列重複同一種小表的表格，已選的值留在原表；點外層的格子會換表」。
    - 已升級(已選那張表就是目前目標)時按 `↓` 回小表:不換,說「已選的值在外層表；要回小表請先移除已選」。
    - `↑`／`↓` 之後**立刻**以最後停留的元素重畫 hover 標示(以前要等滑鼠動,看起來像沒反應)。
    - **已選標記一律畫在「已選屬於的那張表」**(清單非空時 `applyPickedMarks(pickedTableEl)`):以前滑鼠移到別列的小表,藍框會跟著跑到那張小表的同一格。
  ...
  - **巢狀的三句提示**(面板仍只有一句指令句):目標是另一張表格子裡的小表、還沒選 →
    ...
    目標小表只有 1 列時點「整欄」、右鍵「整欄聚合」或「每格各一個值」→ **可升級時改選外層那一欄**(見上一條觸發 1);
    **不可升級時**照做並提示「這張小表只有 1 列，整欄只有 1 格；要跨外層每一列請按 ↑ 切到外層表」(整欄選到 1 格是最先會踩的)。
  - 拖曳放開後瀏覽器補的那個 `click` 要吃掉,否則會被當成「單擊確認」直接送出。
    選取期間 `body` 的文字選取停用,離開時還原。
  - **`preview` 是所選那一個值的文字,不是容器的文字**:單格取那一格;整欄/整列改為
    `「表頭」整欄 N 格` 的描述且**不帶 `previewValue`**;只有一個值且是整欄／整列時另帶 **`previewSamples`**(AF-18):
    字串:前 3 個解析得到、未被排除、非空白格子的文字以「、」串接,超過 3 格加「…」;**設定面板**的預覽接在後面顯示(頁面上的選取模式面板不顯示——那裡已經看得到藍框)。
    **取樣不併進 `preview`**:`preview` 是任務名稱的最後退路,每天會變的值不得進名稱(AF-14 同一個坑)。多值取第一個值再加「(共 N 個值)」;
  ...
  - **會改掉清單的動作都留一步反悔**(單步快照,`takeUndoSnapshot`):
    取代類——換表、單格升級成整欄／整欄每格／整列、去頭去尾、升到外層;
    **移除類(AF-18)**——點一下／`Ctrl` 點取消已選、「整欄→每格」整組取消、chip 的 ×、`Backspace` 與「移除最後一項」、`Ctrl+A`(它其實是取代整批);
  ...
  - **面板永遠以一句「現在該做什麼」收尾**(`instructionLine`,唯一一份,固定在文字區最後一行、
    ...
    單任務／`repick` 已選 N → 「已選 N 個值，再點其他格可加選、點已選的可取消，好了按完成（或雙擊、Enter）」;
    舊版 AF-18 批次模式沒有組 → 「點你要抓的內容；不同的表格或元素會各自成為一個任務」。AF-22 群組模式顯示目前作用組與值數；完成使用固定按鈕或 `Ctrl`/`⌘`+`Enter`，雙擊只加一次。
  ...
  - **送出前檢查選取仍在文件上**(AF-21):單任務／`repick` 的完成、雙擊、`Enter`，以及 AF-22 群組模式的完成出口共用有效性檢查;目標或已選所屬的表 `isConnected === false`(SPA 重繪)時不送出,面板說「頁面剛剛更新過，請重新點選」(加選一格才消失)並清掉失效的選取(連同索引);iframe 代理層看它代表的 iframe。**只認節點脫離文件**:表格節點沒換、原地插列的重繪不算(見 BACKLOG)。`describe` 對沒連到文件根的節點不產生 `path`／`xpath`。
  ...
  - **選取單位由畫面右上角的工具列決定**(`pickMode`,overlay 內、`pointer-events: auto`):
    四段「單格 / 整欄→一個值 / 整欄→每格 / 整列→一個值」(AF-18 加第三段),屬性 `data-af-tool="cell|col|colEach|row"`,
    各帶 `title`,目前那一段帶 `data-af-active`,**預設是單格**;容器 `flex-wrap: wrap`,窄視窗換行。`Tab` 在四段之間循環(停用的跳過)。
    對外的 `currentAxis()` 維持相容:非表格回 `null`、`row` 回 `'row'`、`cell`／`col`／`colEach` 都回 `'col'`。
    **「整欄→每格」與右鍵「這一欄：每格各一個值」共用同一份展開**(`resolveColEachCells`／`expandColEach`):
    點資料格或表頭列的格＝**整組切換**(該欄解析得到的每一格全都已選 → 整組移除並存快照;否則補上缺的、不重複、受上限截斷);
    **被上限截斷(補不進去了)而這一欄已有值時同樣算整組取消**,否則再也點不掉;提示的格數是實際在清單裡的格數。
    一格都解析不到說「這一欄在目前的頁面上取不到格子」;已選最後一項是單格時切到這一段＝展開成它那一欄每格;`Enter` 快速路徑與沒選值直接送出也是每格各一個值。
    **停用規則**:**已經選了值時以 `pickedTableEl` 判定**(那永遠是表格,所以四段一直可用,
    滑鼠在頁面上任何地方都走得到工具列);
    **`upgradeLastPickTo`(把最後一格升級成整欄／整列)也用同一個判定來源**,
    不然會出現「按鈕亮著、按下去卻沒反應」;還沒選任何值時才看 hover 目標——
    目標不是表格時四段全停用,面板一併顯示「非表格:抓整個元素」說明為什麼點不動;
    `preaction` 與 `login-*`(一次只選一個)時 `col`、`colEach` 與 `row` 停用——`repick` 不在此列,
  ...
  - **點一格而它已經算在某個已選的整欄／整列值裡**:加成獨立的值是允許的(合計那一格單獨當一個值),但要說「這一格已經算在「X」裡；現在另外加成獨立的一個值（再點一次取消）」——那一格本來就是藍框,指令句又說「點已選的可取消」,無聲加選會被當成壞掉。
  - **點一個已經選過的格/欄/列 = 移除它(留復原),而且不送出**(判定走 `samePick`,加減一律走 `addPick`/`togglePick`);
    有沒有按 `Ctrl`/`⌘` 都一樣(AF-18)。
  ...
  - **面板底部固定「完成」與「取消」兩個鈕**(`data-af-done` / `data-af-cancel`):
    ...
    面板的一行提示**跟著狀態換**:單任務／`repick` 沒選時教怎麼開始(點一格/`Shift` 拉範圍/點表頭選整欄),
    選了之後教怎麼送出(雙擊或 `Enter`/再點取消/`Backspace` 移除最後一項);AF-22 群組模式只提示固定完成出口、作用組與同步狀態。
  - **面板是可互動的已選清單**:每個值一個 chip(`data-af-chip` 帶序號,內含 `data-af-chip-remove` 的 ×),
    ...
    **已達上限時點格子不送出**,停在原地提示,不能把使用者剛點的那一格靜靜丟掉。
    ...
    清單空的時候面板顯示原本的說明文字。
  - **單一 block 任務**只用一個定位目標,可選該表中的多個值(§7)。跨多來源的任務使用 AF-22「一次建立多個任務」群組流程,見下方。
  - **一次建立多個任務**(AF-18,右鍵 `af-pick-batch`;選取模式的旗標是 `ENTER_PICK`／`DESCEND_FRAME` 的 `batch: true`,`purpose` 仍是 `'task'`):
    AF-22 目前由設定面板管理群組與命名，選值階段的組不再依表格自動合併或自動切換；同表可分組、同值可出現在不同組。一組或多組均以各自穩定 field key 保存，完整流程見「AF-22 群組建立流程」。舊版 AF-18 `batch` 訊息仍由相容路徑處理，不代表目前新建 UI 仍採舊分組規則。
...
- **設定畫面是側邊面板(`chrome.sidePanel`),不是彈出視窗**:
  ...
  - **`sidePanel.open()` 只能在使用者手勢裡呼叫,而且手勢不跨 `sendMessage`**
    ...
    五個入口(右鍵三項:選取、一次建立多個任務、站台登入;popup 的選取鈕、任務頁的編輯鈕)各自在**自己的**處理函式裡呼叫,
  ...
  - **參數一律走 `storage.session` 的 `panel:<tabId>`**,`setOptions.path` **不得帶查詢字串**:
    ...
    形狀是 `{ kind: 'waiting'|'new'|'edit'|'site'|'saved'|'batch'|'bulk', ctx?, taskId?, taskIds?, origin?, draft?, retarget?, batch?, items?, text?, notice? }`
    (AF-18 加 `saved`〔`text` 是回饋那一行〕、`batch`〔`items` 每項同單任務 ctx 加穩定鍵 `key`〕、等待態的 `batch: true`、入口被擋時的 `notice`)。
  ...
  - **面板已經有表單時再選一次目標＝換目標,不重置**:只換 `locator`/`picks`/`blockInfo`/`preview`/`nameHint`,
    名稱、排程、儀表板、進階設定全部留著。**「留著」的範圍明寫**(AF-19,`applyRetarget`):
    `DRAFT_FIELDS` 只認有 id 的欄位,多值清單、告警與前置動作要另外抄下再貼回——
    多值列經 `shared/field-match.js` 的 `reconcileFields` 對回:**同一格沿用原本的 key 與使用者改過的名稱**(key 重生會讓歷史序列斷掉),
    新格用預設名,對不到的舊值消失;**使用者上下移過的順序也留著**(沿用的值照原本相對順序、新值接在後面);
    告警綁的值若已不在清單,欄位退回「全部值」。多值換成單值時提示句改說「現在只抓一個值，原本各個值的名稱不再適用。」。
    提示句「已換成新的目標；名稱、排程、告警、前置動作都留著。」,有值被移除時接「對不到新目標的值移除了 N 個。」。
    **右鍵與 popup 兩個入口在面板已有表單時都不得把 ctx 蓋成等待態**(蓋了草稿就沒了,
    選完也認不出是換目標);只有面板沒開、或停在等待態／站台設定／**剛存完(`saved`)**時才寫等待態。
    「能不能開始一輪選取」只有 background 的 `canStartPick` 一份(右鍵兩項與 `ENTER_PICK` 訊息共用)。
    **單任務與多任務互不插隊**:面板是 `new`／`edit` 時按「一次建立多個任務」、或面板是 `batch` 時按單任務入口,
    都不動 ctx 與草稿、不進選取模式,只寫 `notice`(右鍵與 `ENTER_PICK` 訊息——popup 送完就關視窗——同一條,判定只有 `pickEntryOf`)(「有一個任務設定到一半，請先儲存或取消，再開始多任務」／「多任務設定到一半，請先全部儲存或取消，再選單一任務」),
    面板在 `#panel-notice` 顯示(簽章沒變也要更新這一句)。
...
- **面板的畫面狀態**:**等待態**(`#panel-waiting`,右鍵剛開、還在頁面上選)——
  一句「正在頁面上選取…」加一顆「取消選取」(多任務時標題與說明句換成多任務版);**表單**(選好之後);
  **換目標提示**(`#retarget-note`,「已換成新的目標，其他設定都留著。」);
  **已儲存**(`saved`,回饋區);**多任務清單**(`batch`,見〈多任務的面板〉);**整批改排程**(`bulk`,見〈整批改排程〉)。
  等待態時表單與底部動作列一併隱藏——一開面板就看到一整頁空欄位,使用者不知道自己該做什麼。
- **面板的表單有草稿**(`panel:<tabId>.draft`,鍵是元素 id,debounce 300ms 寫回;`DRAFT_FIELDS` 是名稱、排程各欄、合成方式、定位、儀表板、正規表達式,
  星期與卡片型別這兩個沒有 id 的勾選群組以 `weekdays`／`cardTypes` 陣列存;還原後要重跑排程欄位的連動):
  切換分頁會重載面板文件(實測),沒有草稿就會「切去看一眼別的分頁,回來表單被清空」。
  **儲存或取消時連同 ctx 一起清掉**——不清的話,下一個新任務會被上一個的名稱與排程灌進去。
- **多任務的面板**(AF-22,`kind:'batch'`):右側的 `#group-draft-section` 負責建立和編輯未儲存群組；完成選取後 `#batch-section` 顯示待建立任務(每項一列 `[data-batch-item][data-batch-key]`:名稱、`describeTarget` 的「抓什麼」、
  試抓結果、移除鈕)與共用的「多久抓一次」「抓完放哪裡」、共用合成方式 `#batch-aggregate`(有整欄／整列值才顯示);
  藏起「抓什麼」「先試抓看看」「進階」、「回頁面重選目標」與頁首名稱;`#save`／`#test-now` 改稱「全部儲存」「全部試抓」;錯誤訊息 `#errors` 暫時搬進批次區。
  - 預設名稱照單任務的命名規則(`defaultTaskName`,同一份)逐項算,**同一批撞名依序加「 (2)」「 (3)」**;使用者改過的(草稿 `draft.batchNames[key]`)原樣還原。
  - 移除一列以 `key` 更新 session 的 `items`,其餘各列名稱不錯位;**移到 0 列＝取消**。
  - **全部試抓**:逐項走與儲存同一條「`render` → 收集 → `taskFromForm`」送 `TEST_TASK`,結果回到各自那一列(失敗寫原因);進行中按鈕顯示「試抓中 k／N…」且不可連按。
  - **全部儲存**:逐項 `render(item)` → 名稱 → **貼回畫面上的共用設定**(`render` 會把合成方式等洗回預設)→ `getFormData` → `validateForm` → `saveTaskFromForm`
    (與單任務 `handleSave` 共用的存檔核心;**批次與單任務存出來的規格逐欄相同**)→ `addCardsForTask`;`REBUILD_ALARMS`／`GET_NEXT_RUNS`／`pickerDefaults.last` 整批各一次;
    回饋「已儲存 N 個任務。下次抓取：HH:mm」;**全部存好之後排程重建或記預設值才失敗**不算存檔失敗——照樣顯示回饋並加一行警告、不自動關面板。**第 k 個失敗**:說「已儲存 k−1 個；第 k 個「名稱」失敗：原因」,已存的從清單與 session 移除(再按一次不重複建立),畫面仍是批次清單。
  - **存檔順序:存任務 → 重建排程 → 記預設值 → 加卡片**(單任務);批次是每項「存任務＋加卡片」、整批最後重建排程。**加卡片失敗不算存檔失敗**:任務已存、排程照樣重建,
    回饋區加一行警告且不自動關(卡片與存任務綁在一起的話,卡片寫不進去會讓已存好的任務永遠沒有排程)。
  - 「全部試抓」與「全部儲存」進行中**互鎖**(兩條流程逐項 `render` 同一份表單);被擋時留下的 `notice` 在使用者動表單或移除一列時收掉。
  - 已知取捨:卡片型別、排程與一般前置動作整批共用；各值可另存來源狀態動作。其他進階欄位仍需到任務頁逐項編輯(BACKLOG)。
- **面板可以「回頁面重選目標」**(`#repick-target`):不必關面板、也不必回頁面按右鍵,
  按了直接進選取模式並把目前已選帶回去勾(`preselect`)。
  **編輯既有任務時隱藏**(AF-19,`kind:'edit'` 與 `?taskId=` 兩條):編輯的面板開在報表分頁旁,那裡沒有目標頁可選,
  而且選完回來會被當成新表單、存出一個副本;要換目標一律走任務頁的「重選」(它會自己開目標頁)。
- **整批改排程**(AF-19,`kind:'bulk'`,`{ kind, taskIds }`):任務頁勾選後按「改排程」、或點某一列的排程文字進來。
  ...
  - 預填:排程正規化(`weekdays` 與 `times` 去重排序、`window` 起訖都有才算)後全等→預填並說「相同」;
    ...
    回填只有 `fillSchedule` 一份(`render` 也用它),**每一欄都確定地寫入**(沒有時段＝取消勾選並清空起訖;
    沒有星期:interval＝每天全勾,daily 的星期是必填、缺了就明確回到表單預設的週一～五):
    同一份面板文件從編輯 A 切到整批改排程不會重載,只寫有的鍵會把 A 的時段或星期殘留下來、再套用到所有被選的任務。
    **不套也不寫 `pickerDefaults`**(與編輯同一條)。
  - 套用(`handleBulkSave`):只跑 `validateSchedule`(排程規則只有這一份,`validateForm` 也呼叫它;整批沒有名稱可驗);
    **以當下 storage 為準**重新取任務,每筆**展開既有任務只換 `schedule`、不經 `buildTask`**,一次 `saveTasks`、一次 `REBUILD_ALARMS`;
    回饋「已更新 N 個任務的排程。」接啟用中任務最早的下次抓取時間;**全部停用**時說「所選任務都停用中，不會排程；到任務頁啟用後才會抓」;
    有任務已被刪掉時接「（k 個已不存在）」;全部找不到時面板說「找不到要修改的任務（可能已經被刪掉了）。」且沒有套用鈕;
    開面板時還在、按套用時全部不見了則在 `#errors` 說「這些任務都已經不存在了。」。進行中按鈕顯示「套用中…」,例外時說「套用失敗：…」、不關窗。
  - 入口守門:`pickEntryOf` 對 `bulk` 一律擋下並寫 `notice`「有一批任務的排程改到一半，請先套用或取消，再開始選取」。
...
- 確認後 content 送 `PICKED` 給 background,由它決定去處(`purpose`):
  ...
  **同一套狀態機,只有確認後的去向不同。**訊息接收者要明確限定用途：Picker 的前置動作 listener 只處理 `purpose:'preaction'`，其他 `PICKED` 必須同步回 `false`／不接管回應通道；不得讓無關訊息回傳 `Promise<undefined>`，以免它先於 background 的持久化回覆被 content 當成 ACK。前置動作內容更新完成後再保存表單草稿。
...
- **多值清單的每一列**(`[data-field-row]`)除了序號與名稱,還有
  ...
  **「拆成每個值一個任務」**(AF-19,`#split-tasks`):新建的多值表單(兩個值以上)才顯示,編輯既有任務不顯示。
  按下把每一列變成 `kind:'batch'` 的一項(`key` 為 `b1`…;共同欄位 `locator`/`blockInfo`/`url`/`tabId`/框架身分/`nameHint` 照搬,
  `picks` 只放那一列的規格,**整批的預覽欄位不帶**),名稱寫進 `draft.batchNames`、共用設定經 `snapshotForm` 帶過去,
  之後的試抓與儲存就是〈多任務的面板〉那一條。解不到面板所屬分頁時在按鈕旁說明,不靜默。值超過一批上限(`shared/messages.js` 的 `MAX_BATCH_TASKS`,20,頁面組數上限也用它)時
  `aria-disabled`,點了說「一次最多 20 個任務；請先移除一些值再拆」。反向「合併成一個任務」不做(BACKLOG)。
- **儲存成功當下 ctx 收成 `kind:'saved'`**(AF-18,不帶表單內容與草稿,待寫回的草稿計時器取消):以前那 1.5 秒內 ctx 仍是 `new`,
  右鍵再選會被當成換目標、對著已被回饋區換掉的表單重畫,接著計時器到期又把剛開始的新一輪連面板一起關掉。
  **關窗計時器到期時 session 已不是 `saved` 就不關**;面板文件在 `saved` 態被重載畫回同一個回饋區;
  **同一份面板文件從 `saved` 進下一輪(等待態或新表單)一律重載面板文件**(表單節點與監聽都不在了,在原文件上 render 畫不出來)。
  **新建**的單任務回饋多一行「同一頁還要抓別的？下次在右鍵選「一次建立多個任務」」(編輯既有任務與批次不給)。
  **提示與警告都放在 `saved` ctx 裡(`hint`／`warning`)**:session 一寫面板就照 ctx 重畫回饋區,只 append 在 DOM 上的會被洗掉。
  回饋區的「開啟報表」在側邊面板裡走 `finishPanelSession`(面板沒有 `window.close()`),退路視窗才 `window.close()`。
...
- **使用教學頁**(AF-18,`ui/help/help.html`):入口是右鍵「使用教學」、設定頁開關旁固定的連結、popup 頁尾與零任務引導、任務頁空狀態(AF-21 加後兩者)。
  ...
  - **寫法契約**(`tests/y5_help_page.test.js` 擋):首屏只有「三步開始」與「這一版改了什麼」;每節(`[data-help-section]`)一句目的 ≤40 字、1～5 步每步 ≤40 字、
    一張 HTML/CSS 示意圖、一個預設收合的「遇到這些狀況」;摺疊區以外不出現內部名詞(locator、inner、block、spec、preselect、frame、iframe、聚合、定位器、子路徑、規格);
    **提到的按鈕與選單名稱標 `data-ui-label="<src 下的檔案>"`,文字必須出現在那個檔案裡**——改介面字串時要同步教學頁。
  - 示意圖是頁內 CSS 畫的迷你表格／工具列／面板(顏色只用 `theme.css` 變數),不用截圖。
- **主題設定全站共用**(AF-18):`ui/theme-apply.js` 的 `applyTheme`(唯一一份)與 `applySavedTheme`(讀不到設定時跟系統走、不拋錯);
  Report、Picker、站台設定、popup、教學頁都在各自的正式接線初始化時套用(以前只有 Report 理會使用者選的主題)。設定頁切換主題時,已開著的其他頁到下次載入才換。
...
- **編輯既有任務要保住表單管不到的執行期欄位**(AF-19,`picker.js` 的 `RUNTIME_FIELDS`):
  `enabled`、`foreground`、`suggestForeground`、`notFoundStreak`、`precheckLeadMinutes` 由 `buildTask` 從既有任務原樣帶過,
  **既有任務沒有的鍵不憑空加**;新建仍是 `enabled: true`。以前寫死 `enabled: true`,停用中的任務一改就復活、
  按過「改用前景抓取」的設定也默默消失。這份清單與任務頁 `duplicateTask`(複製時要丟掉哪些)是同一份口徑的兩面。
...
- **一次建立多個任務的畫面共用一份前置動作**(AF-20):同一份 `#preaction-section`(AF-21 起在主區),標題改成「抓之前要先點什麼嗎？（套用到每一個任務）」。
  ...
  **批次畫面沒有單一的 ctx**(逐項 `render` 之前 `currentCtx` 是 null):前置動作列的「在頁面上選取」要用 `batchTabId()`,不帶 `tabId` 的話 background 靜默回 `ok:false`。
  ...
  共用前置動作列與批次設定同屬表單草稿：`snapshotForm` 保存完整列資料，`restoreDraft` 在面板文件重載後還原；同一瀏覽器工作階段內可恢復。進入頁面選取前先等待草稿保存確認，若保存失敗則停止選取並提示，不以未保存狀態繼續。

- **AF-22 群組建立流程**(`af-pick-batch`):右鍵開右側 Picker 後先按「新增第一個群組」，輸入群組名稱或按「從頁面取名稱」，再確認名稱並開始選值。頁面取名 hover 時會顯示文字範圍與預覽；可取文字及可見的文字型 input／textarea 值，password 欄不顯示或讀取。長文預覽會截短，點擊仍取完整文字回填名稱；不加入選值，空白結果不覆蓋名稱。每組可從不同表格、獨立元素及可存取 frame 選值；群組不按表格自動合併，群組名稱可重複。可以新增、切換、改名、移除群組或值、復原，再按「完成選取」進共用設定；「返回選取」回到草稿。群組模式雙擊一個值只加入一次、不完成；固定「完成選取」按鈕與 `Ctrl`/`⌘`+`Enter` 是完成出口。只有一組也照此流程保存為 multi 任務。
  - R01 的 Picker 選取守門由 `shared/route.js` 判定，與 §3 frame 定位及 §4 fetch-tab 的 `sameOriginPath` 各司其職，不改寫後兩者契約。僅忽略明確 tracking query（`utm_*`、`gclid`、`dclid`、`gbraid`、`wbraid`、`fbclid`、`msclkid`、`mc_cid`、`mc_eid`、`_ga`、`_gl`）的增刪或重排；其他 query 及所有 hash 變化保守暫停選取、保留草稿並要求重新核對。`document`／`session` gate 不放寬。路由判定通過也不代表選值仍有效：每個已選來源仍以當前 DOM 節點及其所屬表格的連接狀態核驗；同 URL 的 SPA 若替換掉來源節點，完成會保留草稿並要求重新選取，不能只靠 URL 相同送出舊目標。
  - 關閉側邊面板會暫停本輪選取並保留草稿；同一瀏覽器工作階段內重開可選擇繼續或放棄。瀏覽器重啟會清除 `storage.session`，未儲存的草稿無法恢復。
  - 群組名是任務名稱；每個選值有穩定 field key、來源與單值擷取規格。名稱不是定位或資料身分。每值可以有自己的「來源狀態動作」，其內容取自共用前置動作列，只在擷取該值前執行。來源動作變更納入執行規格快照；執行期間改動來源或動作時，舊結果不發布到新設定。
  - 編輯 multi 任務時，可按「修復同一個值」重新定位並保留 key、名稱、告警與歷史序列；「改成不同指標」需確認，建立新 key／歷史序列並移除原告警設定，舊歷史仍保留。單值修復與替換入口僅在多值 multi 編輯表單顯示。G1 的 CfT 瀏覽器流程已驗證重開編輯、同指標修復保留 key／歷史，以及不同指標替換建立新 key 並清理舊 lastValue。
  - multi 試抓逐值回報成功與失敗；部分成功標示部分結果並保留失敗值原因，全部失敗回報失敗。試抓使用正式來源協調與擷取流程，但不寫紀錄、不更新 lastValues、不進排程帳本、不儲存任務。正式執行每個宣告值各有結果；成功與失敗值一併提交，health 依完整結果判定。
...
AF-22 multi 任務不是單一 `task.locator`／`task.frame`：每個 field 的 `source` 分別保存 `locator` 與可選 `frame: { url }`，同一頁可混用主文件和可存取 frame。擷取時逐值重新定位來源，再把該 field 的單值 `spec` 交給既有擷取器；frame 身分仍以穩定 URL 描述，不保存易變的 `frameId`。Chrome 不允許注入的頁面／frame，以及 closed shadow root，仍不可選取。
...
- **抓取頁面開在哪裡**(設定 `fetchTabMode`,設定頁「排程抓取的頁面開在」):
  - `tab`(**預設**,AF-20 使用者定案「不閃、背景無感」):目前視窗的背景分頁(`tabs.create({ url, active:false })`,不切過去、抓完就關)。
    沒有任何視窗時改走 `window`。
  - `window`:擴充功能自己的**專用視窗**,分頁列完全不動,但建立到最小化之間可能閃一下。建立方式照下面的探針事實,**只有這一種組合可用**:
...
- 主資料:`chrome.storage.local`,`schemaVersion` 目前為 **4**(AF-22 R16;0.20 的 v3 資料由初始化遷移)
  ...
  - 其他鍵:`missed`、`health`、`diag`、`layout`、`alertLog`、`notifyLog`、`cryptoKey`、`lastValues`、`lastSeenAt`、`lastTimezone`、`settings`,以及各一天一次守衛的日戳(`lastTrimDate` 等);`storage.session`:`runState`、`failMerge`、`repickTabs`、`fetchTabs`、`panel:<tabId>`、`pickDraft:<tabId>`。`pickDraft` 是工作階段暫存，不屬於設定／歷史備份；瀏覽器重啟即清除。
  ...
  - **升級與回退**(AF-22 R16):schemaVersion 拒收只保護設定檔匯入,不保證舊程式讀取已升級的同一份 `chrome.storage.local` 安全。升級前在設定頁分別手動匯出設定檔與 JSON 歷史日檔,並確認檔案可開啟、內容日期/任務正確。設定檔不含抓取紀錄;歷史檔不含任務完整設定。
    ...
    0.20.0 使用 schema 3；AF-22 0.21.0 使用 schema 4 並新增 multi 任務格式。schema 4 設定匯出應由舊版拒收，但這不代表 0.20.0 能安全共用已升級的本機 storage。回退前依上文分別備份設定與歷史，並安裝能讀取 schema 4 storage 形狀的相容版本。
...
- **任務的整批 API**(AF-19):`saveTasks(list)`(**先全部驗證、任一筆不合法整批不寫**;例外訊息與單筆一字不差,另掛 `index`;
  一次讀一次寫;`order` 缺省依序補「目前最大 +1」)、`deleteTasks(ids)`(一次掃描刪任務、其紀錄與 `lastValues` 裡它們的鍵,卡片逐一 `pruneCardsForTask`)、
  `countRecordsForTasks(ids)`(一次掃描回 `{ total, byId }`)、`deleteLastValues(keys)`。
  `saveTask`／`deleteTask`／`countRecordsForTask` 都只是包裝整批版。**改多個任務一律走整批版**(任務頁整批動作、popup 全部暫停、拖曳排序)。
- **值被移除時的序列**(AF-19):多值任務少掉的值(編輯時按「移除」、任務頁重選時沒再選到)——
  **紀錄一律保留**到保留天數自然到期(刪紀錄不可逆);儀表板上指向它的來源以 `layout-store` 的 `pruneSeries(seriesIds)` 清掉
...
- 新群組保存為一般排程任務，資料形狀為 `task.mode = 'multi'` 與 `task.spec.mode = 'multi'`；`task.fields` 與 `task.spec.fields` 依穩定 `key` 一一對應。每項規格含 `source: { locator, frame? }`、單一值 `spec`，可選 `mode` 與 `stateActions`。不得遞迴 multi；每任務最多 100 值，任務與 session 草稿另受共用大小上限約束。驗證與舊任務正規化集中在 `shared/task-source.js`。
- 舊單值與舊同表 block 任務維持舊序列化形狀；共用正規化只供讀取與執行，不會因讀取而批次重寫舊任務。multi 紀錄 `taskId` 是 `父任務 id#field key`，排程帳本與 health 用父任務 id，`lastValues` 與告警依子序列。刪值保留既有歷史；修復同一值保留 key，改成不同指標則建立新 key，舊歷史留存且原告警不沿用。
- multi 來源依 `spec.fields` 順序取得。相鄰值若有相同的 `stateActions`，狀態動作只執行一次，再依序抓這批值；動作序列改變時先執行新序列，再抓後續值。這些動作是目前頁面的操作，不是各自獨立的頁面快照：空的 `stateActions` 表示不操作並沿用前一值留下的頁面狀態，不會自動重載或還原。若先前執行過狀態動作、下一值沒有動作，Picker 會提醒可能仍停在前一個頁籤或篩選；提醒不阻止儲存。需要回到初始狀態時，請為下一值明確設定切換／還原動作。動作受既有前置動作型別與來源定位驗證約束；跨 frame 的前置動作仍依 frame 可達性限制。
- multi 執行按 field 彙整結果：一值失敗不丟棄其他值；部分成功結果標為 partial，全部失敗仍回報每個值的失敗。排程結果按穩定提交身分復原，避免中斷後重複追加；若任務規格在執行中改變，舊結果不發布至新規格。這些保證不代表紀錄、帳本、health 等不同 storage key 是單一原子交易。
- multi 試抓（`dryRun`）沿用來源定位與擷取流程並回傳逐值結果，不寫正式紀錄、帳本、`lastValues` 或 health。G1 的 CfT 瀏覽器驗收涵蓋兩組命名、一組兩個主頁值加一個跨來源 frame 值、第二組頁面取名和值、部分與全部 dryRun、保存、首次抓取、`taskId#fieldKey` 子序列歷史、重開編輯、同指標修復與不同指標替換。另驗證切換到另一個真實分頁後建立第二輪、錯誤分頁的 `COMPLETE` 被拒、第二輪 dryRun／保存不改動前一輪任務與歷史。這些結果只適用於該 CfT fixture；任意網站的取名／框架組合與使用者自己的實際資料備份尚未驗證。R15 效能量測未提供各階段拆分，最差案例的中位數約 1.3 秒。
...
- **父任務 id 與序列 id 的分工**(記錯會讓冪等失效或畫面永遠空白):
  ...
  多值任務的某個值被移除時,用序列 id 的這一欄裡只清 `lastValues` 與卡片 `source`,紀錄保留(§5〈值被移除時的序列〉)。
...
- 使用者選的單位有三種:**單一儲存格(預設)、整欄、整列**——**直接在頁面上點**
  (§2 選取模式,右上角工具列切換,`Tab` 在四段之間循環),不是在視窗裡填數字。
...
- **多選與整批動作**(AF-19):每列最前面一顆勾選框(`[data-action="select"]`,`aria-label`「選取『名稱』」,被選的列帶 `.selected`);
  ...
  工具列 `#task-select-all` 全選／全取消**目前篩選出來的列**(部分選取時 `indeterminate`)。
  **選取狀態存在模組層、跨重畫保留**(每抓一次 health 就變、任務頁整份重畫),重畫時只剔除已不存在的任務。
  動作列 `#task-bulk-bar`(沒選時隱藏):「已選 N 個」,**有已選不在目前篩選中時接「（k 個不在目前篩選中）」**
  (看不到的已選也會被整批動作影響);按鈕「啟用」「停用」「改排程」「刪除」「取消選取」。
  - 啟用／停用:展開既有任務只換 `enabled`,一次 `saveTasks`、一次 `REBUILD_ALARMS`;進行中整條動作列鎖住、被按的那顆顯示「啟用中…」／「停用中…」;
    完成後 `#task-note` 說「已啟用 N 個任務。」／「已停用 N 個任務。」;寫入失敗(整批全有全無)說「停用失敗，沒有任何任務被改動：…」並還原按鈕。
  - 改排程:寫 `panel:<tabId>` 為 `{ kind: 'bulk', taskIds }` 再 `openPanel`,**在點擊手勢裡直接呼叫**(與編輯鈕同一寫法);見 §2〈整批改排程〉。
  - 刪除:與單一刪除同一個對話框。筆數以 `countRecordsForTasks` 一次掃;多個任務的訊息是「確定要刪除這 N 個任務嗎？此操作將一併刪除合計 M 筆歷史紀錄。」;
    確認走 `deleteTasks`;「先匯出再刪除」仍是下載成功才刪(匯出的是全部任務的紀錄,既有行為,見 BACKLOG),
    **下載沒完成(使用者在另存視窗按取消)就不刪**,對話框留著並接一句「匯出沒有完成（原因），所以還沒有刪除。」。
    **對話框開著時選取變了就收掉**(由動作列開的才算;`#task-note` 說「選取已經變了…請再按一次『刪除』」)——
    確認鈕刪的是開啟當下那幾個,不收掉的話畫面勾的與實際刪的會對不上。
  - 動作列、全選與搜尋的監聽只綁一次(重畫好幾次後按一下仍只做一次)。
- **就地改名**(AF-19):名稱旁「改名」鈕把名稱換成輸入框(`input[data-rename-input]`,剛打開時原值全選),`Enter`／失焦存檔、`Esc` 放棄;
  空白不存並在 `#task-note` 說「名稱不能空白。」;存檔取最新任務只換 `name`(不重建排程)。
  **改名中遇到重畫**:改名狀態(`{ id, value }`)存在模組層,重畫時那一列重建成輸入框、帶回打到一半的字並還原焦點。
  輸入框與改名鈕的指標事件不啟動拖曳排序。**同一時間只有一列在改名**:改名中按另一列的「改名」,上一列先存檔再開這一列;
