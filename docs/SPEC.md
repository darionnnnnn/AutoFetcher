...
- 按下前兩項**不會直接開設定視窗**,而是讓目標頁進入**選取模式**(`content/picker-mode.js`):
  ...
  - **面板第一行永遠是一句「現在該做什麼」**(`instructionLine`,唯一一份),隨狀態換:
  ...
  - **選取單位由畫面右上角的工具列決定**(`pickMode`,overlay 內、`pointer-events: auto`):
    ...
    **停用規則**:目標不是表格時三段全停用,面板一併顯示「非表格:抓整個元素」說明為什麼點不動;
    ...
    停用者帶 `aria-disabled="true"` 且點了不改模式,但**不得靜默無事**:
...
- **Picker 設定視窗的版面**:整個視窗只回答三個問題——**抓什麼 / 多久抓一次 / 抓完放哪裡**。
  ...
  **任務名稱 `#name` 就在標題列**(開窗即可改,不必先找到某個欄位)。
  ...
  `#summary-dashboard`),**任何欄位變動即時重算**,文字一律取自 `shared/describe.js`
  (`describeTarget` / `describeSchedule` / `describeDashboard`,與任務頁、popup 同一份;
...
- **多值清單的每一列**(`[data-field-row]`)除了序號與名稱,還有
  ...
  另有一鍵命名 `#rename-col` / `#rename-cell`,**只改沒有被使用者手動改過的列**
