...
- 按下前兩項**不會直接開設定視窗**,而是讓目標頁進入**選取模式**(`content/picker-mode.js`):
  ...
  - **`preview` 是所選那一個值的文字,不是容器的文字**:單格取那一格;整欄/整列改為
    ...
    前 3 個解析得到、未被排除、非空白格子的文字以「、」串接,超過 3 格加「…」;面板預覽接在後面顯示。
  ...
  - **一次建立多個任務**(AF-18,右鍵 `af-pick-batch`;選取模式的旗標是 `ENTER_PICK`／`DESCEND_FRAME` 的 `batch: true`,`purpose` 仍是 `'task'`):
    **是不是多任務在入口就決定**,選取中不提供切換。資料模型不變——建出來的就是 N 個一般任務。
    ...
    - 非表格元素:點一下＝加成一組,再點＝移除那一組;點在頁面空白處(`body`)不加,說明原因。整欄／整列模式點非表格照舊拒絕。
...
- **設定畫面是側邊面板(`chrome.sidePanel`),不是彈出視窗**(AF-10 推翻先前的 `windows.create`):
  ...
  - **面板已經有表單時再選一次目標＝換目標,不重置**:只換 `locator`/`picks`/`blockInfo`/`preview`/`nameHint`,
    ...
    **單任務與多任務互不插隊**:面板是 `new`／`edit` 時按「一次建立多個任務」、或面板是 `batch` 時按單任務入口,
    都不動 ctx 與草稿、不進選取模式,只寫 `notice`(「有一個任務設定到一半，請先儲存或取消，再開始多任務」／「多任務設定到一半，請先全部儲存或取消，再選單一任務」),
...
- **多任務的面板**(AF-18,`kind:'batch'`):顯示 `#batch-section`(每項一列 `[data-batch-item][data-batch-key]`:名稱輸入框、`describeTarget` 的「抓什麼」、
  ...
  - **全部儲存**:逐項 `render(item)` → 名稱 → **貼回畫面上的共用設定**(`render` 會把合成方式等洗回預設)→ `getFormData` → `validateForm` → `saveTaskFromForm`
    ...
    回饋「已儲存 N 個任務。下次抓取：HH:mm」。**第 k 個失敗**:說「已儲存 k−1 個；第 k 個「名稱」失敗：原因」,已存的從清單與 session 移除(再按一次不重複建立),畫面仍是批次清單。
