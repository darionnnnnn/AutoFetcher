...
- 按下前兩項**不會直接開設定視窗**,而是讓目標頁進入**選取模式**(`content/picker-mode.js`):
  ...
  - **取消要二段確認**(AF-19):`Esc`、面板「取消」、右鍵選單「取消」三處都經 `requestCancel`。
    ...
    **右鍵開選單與滑鼠移動不清**(選單裡的「取消」要能當第二步)。
    確認旗標與換表旗標的清除只有 `clearPendingConfirms` 一份。已選 0～1 個、前置動作與登入直接取消。
  ...
  - **一次挑多個值**(`task` 與 `repick` 支援;`repick` 會帶既有的值回來勾,所以也要能多選):
    ...
    **chip 與格子雙向指涉**(AF-19):滑過 chip＝頁面上那個值涵蓋的格子加 `data-af-chip-hover` 加粗外框(整欄／整列值標它標示的每一格),
    移開還原;點 chip 本體＝捲到第一格並加強 1 秒(移除鈕仍是移除)。**找格子只有 `cellsOfPick` 一份**,已選標記 `drawPicksOn` 也經它;
...
- **整批改排程**(AF-19,`kind:'bulk'`,`{ kind, taskIds }`):任務頁勾選後按「改排程」、或點某一列的排程文字進來。
  - 畫面(`setBulkView`,與 `setBatchView` 互斥):只露 `#bulk-section`(唯讀的任務名稱清單＋`#bulk-note`)與 `#schedule-section`;
    標題列、抓什麼、先試抓、抓完放哪裡、進階、「固定為預設值」、回頁面重選、立即測試全部隱藏;`#save` 改稱「套用到 N 個任務」。
  ...
  - 預填:排程正規化(`weekdays` 與 `times` 去重排序、`window` 起訖都有才算)後全等→預填並說「相同」;
    ...
    回填只有 `fillSchedule` 一份(`render` 也用它)。**不套也不寫 `pickerDefaults`**(與編輯同一條)。
  - 套用(`handleBulkSave`):只跑 `validateSchedule`(排程規則只有這一份,`validateForm` 也呼叫它;整批沒有名稱可驗);
    **以當下 storage 為準**重新取任務,每筆**展開既有任務只換 `schedule`、不經 `buildTask`**,一次 `saveTasks`、一次 `REBUILD_ALARMS`;
    ...
    有任務已被刪掉時接「（k 個已不存在）」;全部找不到時面板說「找不到要修改的任務」且沒有套用鈕。
...
- **多選與整批動作**(AF-19):每列最前面一顆勾選框(`[data-action="select"]`,`aria-label`「選取『名稱』」,被選的列帶 `.selected`);
  ...
  **選取狀態存在模組層、跨重畫保留**(每抓一次 health 就變、任務頁整份重畫),重畫時只剔除已不存在的任務。
  ...
  - 啟用／停用:展開既有任務只換 `enabled`,一次 `saveTasks`、一次 `REBUILD_ALARMS`;進行中整條動作列鎖住;完成後 `#task-note` 說「已停用 N 個任務。」。
...
- **就地改名**(AF-19):名稱旁「改名」鈕把名稱換成輸入框(`input[data-rename-input]`),`Enter`／失焦存檔、`Esc` 放棄;
  空白不存並在 `#task-note` 說「名稱不能空白。」;存檔取最新任務只換 `name`(不重建排程)。
  **改名中遇到重畫**:改名狀態(`{ id, value }`)存在模組層,重畫時那一列重建成輸入框、帶回打到一半的字並還原焦點。
  輸入框與改名鈕的指標事件不啟動拖曳排序。
- **排程欄可點**(AF-19):`.task-schedule` 是 `button.task-link`(`data-action="edit-schedule"`),文字仍是 `describeSchedule`;點了走「改排程」,`taskIds` 只有這一個。
