...
- 按下前兩項**不會直接開設定視窗**,而是讓目標頁進入**選取模式**(`content/picker-mode.js`):
  ...
  - **表尾合計預設排除**(AF-16):**建立**整欄值的當下,資料列中位於這張表 `<tfoot>` 的列自動進 `exclude`,
    面板提示「已自動排除表尾 N 列（合計），右鍵可取消」。判定只有 `withFooterExclude` 一份,**六個建立入口都呼叫它**:
  ...
  - **右鍵選單**(攔截頁面原生選單):表格內是
    ...
    **〔排除項〕**(AF-16):滑鼠下那一格被某個已選的整欄／整列值涵蓋時才出現——整欄值涵蓋同一欄的每一列、整列值涵蓋同一列的每一欄;
    被好幾個值涵蓋時取**清單裡最後一個**(`blockCoversCell`,選單與標示共用這一份判定)。
    這一格不在該值的排除清單 →「從整欄聚合排除這一列」／「從整列聚合排除這一欄」(`exclude`);
    已在 →「取消排除這一列」／「取消排除這一欄」(`include`)。改的是那個值的 `block.exclude`(§7),**產生新的 block 物件替換**,
    不原地改陣列元素(復原快照裡的舊清單要保持原樣)。**不用 `Ctrl` 點**:那是「加選這一格為獨立的值」,合計那一格單獨當一個值是合理需求。
    ...
    - 擷取順序（`extract.js` 的 `extractBlockFromTable`）：定位那一軸 → 展開另一軸成有序清單（取格方式與既有三條分支相同）→
      **先套 `skip` 再套 `exclude`**（「頭尾」講的是表的形狀，要對著完整的表算）→ 剩下的進 `aggregateCells`。
      - `skip` 以**清單項**為單位，不看解析得到與否（一列就是一列）；`head + tail` ≥ 清單長度 → `not_found`，
        訊息「略過開頭 H 筆、結尾 T 筆後沒有剩下的格子（這一欄只有 N 列）」。
      ...
      - **排除項找不到不得靜默**：不排除任何東西，但 `status` 降為 `fallback`、`message`「有 N 個排除項在目前的頁面找不到（合計）」——
        合計列改名之後漏排除，就是默默加兩次。
      - 全被排除 → `not_found`「排除 K 筆後沒有剩下的格子」，與「本來就沒有格子」的訊息分得出來。
...
- **Picker 的值清單**(`#field-list`,每列 `data-field-row`):挑了 2 個以上的值才出現。
  ...
  **略過開頭／結尾 N 筆**(`#skip-head`／`#skip-tail`,容器 `[data-skip-row]`,AF-16)放在聚合方式旁邊,
  ...
  單值整欄的逐欄組裝要帶上 pick 的 `exclude`(多值走展開,本來就留得住)。
