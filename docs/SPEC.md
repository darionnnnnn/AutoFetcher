...
- **AF-22 群組建立流程**(`af-pick-batch`):右鍵開右側 Picker 後先按「新增第一個群組」，輸入群組名稱或按「從頁面取名稱」，再確認名稱並開始選值。頁面取名只讀取使用者點到的文字或可見表單值回填名稱，不加入值；空白結果不覆蓋名稱。每組可從不同表格、獨立元素及可存取 frame 選值；群組不按表格自動合併，群組名稱可重複。可以新增、切換、改名、移除群組或值、復原，再按「完成選取」進共用設定；「返回選取」回到草稿。群組模式雙擊一個值只加入一次、不完成；固定「完成選取」按鈕與 `Ctrl`/`⌘`+`Enter` 是完成出口。只有一組也照此流程保存為 multi 任務。
  ...
  - 編輯 multi 任務時，可按「修復同一個值」重新定位並保留 key、名稱、告警與歷史序列；「改成不同指標」需確認，建立新 key／歷史序列並移除原告警設定，舊歷史仍保留。單值修復與替換入口僅在多值 multi 編輯表單顯示。瀏覽器端完整修復／替換流程尚未列入本輪已確認的 G1 驗收，不應據此宣稱已通過端到端測試。
...
- multi 試抓（`dryRun`）沿用來源定位與擷取流程並回傳逐值結果，不寫正式紀錄、帳本、`lastValues` 或 health。CfT 已確認的 AF-22 縱向流程包含兩組命名、一組兩個主頁值加一個跨來源 frame 值、第二組頁面取名和值、部分與全部 dryRun、保存、首次抓取及 `taskId#fieldKey` 子序列歷史。這只確認指定 fixture 的流程；一般網站取名、任意 frame、跨分頁草稿恢復，以及完整編輯重選／替換的瀏覽器流程尚未驗收，不能以單元測試代替。
