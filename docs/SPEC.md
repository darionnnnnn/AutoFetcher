...
`{ key, inner, rows: [{ resolved, text }] }`(單值的 `key` 是 `''`),儲存格與整欄逐列、整列逐欄以 `resolveInnerAt` 探測,
`text` 取前 40 字,最多 20 筆;沒有任何帶 inner 的值時**不放這個鍵**。「這一列解析不到」與「抓錯欄」在錯誤訊息上長得一樣,沒有它就查不出是哪一種。
...
- `task.mode = 'block'`;`task.spec` 有三種形狀,擇一:
  ...
  - **格內子路徑 `inner`**(AF-15):`cell.inner` 與 `block.inner` 可帶 `[{ tag, index }, …]`——從外層那一格的直接子元素起算、
    ...
    **缺省、`null`、空陣列都算沒有**(`hasInner`),舊規格的擷取結果一個位元都不變;**形狀不合法算「有」但解析不到**,
    ...
    - 單格解析不到 → `not_found`,訊息「這一格裡找不到原本的位置；目前這一格的文字是：…」(空的就說「目前這一格是空的」)。
