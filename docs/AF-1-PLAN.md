...
> 狀態:**AF-1 已全部實作完成並驗收**(2026-09-05)。293 單元測試綠 + Chrome 端到端煙霧測試通過。
...
| **AF-2** | **儀表板自訂版面(§8.2 全部)**、Picker「加入儀表板」、樞紐表、獨立 HTML 報表 |
...
| D5 | 預檢 + 燈號 + popup | `<taskId>:pre:<i>` alarm(前 `precheckLeadMinutes`,預設 30);演練共用 D2 流程碼,`dryRun:true` 不寫紀錄,結果寫 `health[taskId]`;每日 08:00 站台健康檢查(AF-1 只做「登入頁判定」,自動登入本身在 AF-3);`shared/health.js` 匯總四級燈號並更新 `chrome.action` badge/icon/title;`ui/popup/`(§12.2:摘要、任務清單、立即重試、開啟頁面、暫停、全部暫停、開 Report);已讀機制 | 單元測試:預檢 selector_lost → health 紅、badge 文字 "1"、notifications 呼叫一次且帶任務名與時間;下次成功 → 回綠;兩任務一黃一紅 → 紅;全部暫停 → 灰「II」;預檢不得產生 record(`getRecordsByDate` 長度不變)。Puppeteer:popup 顯示紅色任務與「開啟頁面」按鈕 |
...
執行順序:A1 → A2 → B1 → B2 → B3 → E1 → E2 → C1 → C2 → D1 → D2 → D3 → D4 → D5 → F1 → F2。
...
| K 匯出 | 獨立 HTML 報表(內嵌資料 + 版面,離線可開) |

## 執行紀錄

| 階段 | 執行者 | 結果 | 驗收 | 落差與處置 |
|---|---|---|---|---|
| A1 骨架 | agy gemini-3.8-flash-high | 綠 | 15 測試 + 突變 | 首次委派用 shell `&` 背景執行,外層假回報完成(已知陷阱,改用執行環境背景機制) |
| A2 儲存層 | agy | 綠 | 31 測試 + 2 突變 | 先誤派 forge,4 輪失敗:>200 行新模組照表該給 agy。地端預設 Qwen 載不進 Metal 上限 |
| B 選擇器 | agy | 綠 | 47 測試 + 3 突變 | — |
| C2 數值策略鏈 | agy | 綠 | 63 測試 + 2 突變 | 提前到 B3 前做,因 B3 要用它 |
| E1 匯出 | agy | 綠 | 72 測試 + 2 突變 | — |
| D1 排程核心 | agy | 綠 | 90 測試 + 3 突變 | — |
| B3 content 接線 | agy | 綠 | 102 測試 + 1 突變 | — |
| D2 擷取流程 | agy | 綠 | 122 測試 + 4 突變 | 重試次數實作 1 次、SPEC 要求 2 次;規格自相矛盾,Claude 修正 |
| E2 設定匯出匯入 | agy | 綠 | 134 測試 + 3 突變 | 首次執行遇 Google 503,重試後成功 |
| D3 錯過清單 | agy | 綠 | 150 測試 + 3 突變 | 驗收抓到兩處測試同義反覆(邊界值、interval 排除),Claude 補強測試 |
| D4 看門狗與診斷 | agy | 綠 | 162 測試 + 3 突變 | — |
| C1 Picker | agy | 綠 | 182 測試 + 3 突變 | — |
| F1 報表邏輯 | agy | 綠 | 206 測試 + 3 突變 | 字串排序測試兩元素分不出比較器方向,Claude 補三元素測試 |
| D5a 健康與燈號 | agy | 綠 | 223 測試 + 3 突變 | — |
| D5b 預檢 | agy | 綠 | 239 測試 + 3 突變 | fetcher 改動 27 行,在 40 行限制內 |
| D5c popup | agy | 綠 | 252 測試 + 2 突變 | — |
| D6 總接線 | agy | 綠 | 273 測試 + 3 突變 + 端到端 | 煙霧測試抓到:重建排程後燈號沒更新,Claude 補上 |
| F2 報表檢視 | agy | 綠 | 293 測試 + 3 突變 + 端到端 | — |

### 過程中發現的真實問題(單元測試抓不到,靠真實瀏覽器)

1. **Chrome 152 起一般版已封鎖 `--load-extension`**,連 `--disable-features=...` 也無效。
   自動化測試必須用 **Chrome for Testing**(`npx @puppeteer/browsers install chrome@stable`),
   跑法見 `run_smoke.sh`。
2. **MV3 service worker 不允許動態 `import()`**。產品程式碼全用靜態 import 沒踩到,
   但測試腳本一開始寫錯。日後在 background 加模組一律靜態匯入。
3. **service worker 閒置就被回收**,`worker.evaluate` 不可靠;所有端到端檢查改從擴充功能頁面做。
4. **重建排程後沒有刷新燈號**,popup 會停在舊狀態。已修。

### 本輪未做(移至 AF-2 / AF-3)

- 儀表板卡片與自訂版面(§8.2)、樞紐表、獨立 HTML 報表、趨勢圖
- 任務頁與設定頁的**內容**(骨架已在 report.html;匯出按鈕、排程健康區、保留天數)
- 區塊聚合(§7)、自動登入的實際填入(§6,目前只做「是否停在登入頁」判定)、前置動作、告警(§10)
- Edge 實機驗證(本機未安裝 Edge,煙霧測試會自動略過)
