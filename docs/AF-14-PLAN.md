...
**裸年份當欄標題的表會失去警示**。判準把 `2024`、`2025`、純數字料號一律視為非錨點。
對「欄標題是年度」的表：

| | 表格多插一欄時 |
|---|---|
| 舊行為 | `findClosestIndex` 跟著標題走 → 抓對，狀態 `fallback`（黃燈，使用者看得到） |
| 現在 | 視同沒有標題 → 純走 `index` → **靜默抓到隔壁年度，畫面上是綠燈** |

這是判準擴大後新出現的靜默錯值路徑，你先前定案「不對年份開例外」時我們沒討論到它。三個選項：

1. **維持現況**（本輪先出，這一條進 BACKLOG）。單列數值表的問題已解，年度表的風險等有人回報再處理。
2. **改成「先比對、找不到才退回索引」**：純數值標題仍拿去比對，對得上就用（位移就 `fallback` 跟著走），
   找不到才退回 `index` 且狀態 `ok`。年度表恢復警示，單列數值表照樣好。
   代價：多列數值表若某天別列剛好出現同一個數字，會跟著跳過去（現在是穩定走 index）。
3. **只對「該軸沒有表頭」的表停用錨點**（單列無表頭那種）。最精準，但判準多一個維度，複雜度上升。

我的建議是 2：它讓「有標題可追」與「沒標題可追」各自走對的路，而年度表的靜默錯值是真的會發生的事。
