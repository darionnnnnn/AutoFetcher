...
- 測試:`npm test` **基線 2424 綠**(Node 內建 test runner + jsdom;下一輪只能增不能減)。
...
- **取消類二段確認的旗標與換表旗標一起清**(AF-19):清除只有 `clearPendingConfirms` 一份;長按(`repeat`)與 400 毫秒內的連按不算第二次,
  右鍵開選單不清(選單裡的「取消」要能當第二步)。
